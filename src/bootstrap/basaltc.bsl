...
func gen_collect_expr(id: int): void {
  ...
  if k == N_CALL then {
    ...
    if f != 0 && node_kind[f] == N_GENERIC_FUNC then {
      let aa: int = node_a[id]; let actual: int = 0;
...
func gen_type(kind: int, child: int, size: int): void {
  if kind == TY_INT then code_emit(C_KW, 1);
    ...
    if b != 0 then gen_type(node_kind[b], b, node_value[b]);
...
func gen_call_name(id: int): int {
  ...
  let actual: int = 0; let a: int = node_a[id];
  while a != 0 {
    let q: int = 0;
...
func gen_expr(id: int): void {
  ...
  if k == N_INT then {
    ...
    if node_aux[id] != 0 && node_kind[node_aux[id]] == TY_NAMED then {
...
func gen_decl(ty: int, name: int): void {
  if node_kind[ty] == TY_FUN then gen_fun_decl(ty, name);
    ...
    let inner: int = node_a[ty];
    gen_type(node_kind[inner], inner, node_value[inner]);
    code_emit(C_IDENT, sym_c_symbol(name)); code_emit(C_PUNCT, 2); code_emit(C_INT, node_value[ty]); code_emit(C_PUNCT, 3);
  } else {
...
func gen_unify_formal(formal: int, actual: int): void {
  ...
  if node_kind[formal] == TY_GENERIC && node_kind[actual] == TY_GENERIC then {
    if node_value[formal] != node_value[actual] then return;
...
func gen_function_specialized(decl: int, inst: int, cname: int): void {
  gen_bind_decl(decl, inst);
  let ret: int = gen_substitute_type(node_b[decl]);
  gen_type(node_kind[ret], ret, node_value[ret]); code_emit(C_IDENT, cname); code_emit(C_PUNCT, 6);
  let p: int = node_c[decl];
  while p != 0 { let pt: int = gen_substitute_type(node_b[p]); gen_decl(pt, node_a[p]); if node_next[p] != 0 then code_emit(C_PUNCT, 7); p = node_next[p]; }
  code_emit(C_PUNCT, 8);
...
func gen_function_signature(id: int): void {
  ...
  code_emit(C_IDENT, sym_c_symbol(node_value[id]));
  code_emit(C_PUNCT, 6);
  let param: int = node_c[id];
  while param != 0 {
    if node_kind[id] == N_EXTERN then gen_extern_param(node_b[param], node_a[param]);
    else gen_decl(node_b[param], node_a[param]);
    if node_next[param] != 0 then code_emit(C_PUNCT, 7);
...
func gen_program(id: int): void {
  ...
  si = 0; while si < gen_spec_count { if gen_spec_kind[si] == 2 then { gen_bind_decl(gen_spec_decl[si], gen_spec_type[si]); let rr: int = gen_substitute_type(node_b[gen_spec_decl[si]]); gen_type(node_kind[rr], rr, node_value[rr]); code_emit(C_IDENT, gen_spec_name[si]); code_emit(C_PUNCT, 6); let pp: int = node_c[gen_spec_decl[si]]; while pp != 0 { let pt: int = gen_substitute_type(node_b[pp]); gen_decl(pt, node_a[pp]); if node_next[pp] != 0 then code_emit(C_PUNCT, 7); pp = node_next[pp]; } code_emit(C_PUNCT, 8); code_emit(C_PUNCT, 12); code_emit(C_NEWLINE, 0); gen_bind_clear(); } si = si + 1; }
...
func ast_stmt(): int {
  ...
  if input_take(T_CONTINUE) == 1 then {
    if input_take(T_SEMI) == 0 then return (0 - 1);
    if for_step_context != 0 then {
      let s: int = clone_for_step(for_step_context);
      let c: int = ast_node(N_CONTINUE, 0, 0, 0, 0, 0);
      return ast_node(N_BLOCK, ast_link(s, c), 0, 0, 0, 0);
    }
...
func ast_params(): int {
  ...
  while 1 == 1 {
    ...
    let ty: int = ast_type();
    if ty == 0 then return (0 - 1);
    let param: int = ast_node(N_PARAM, name, ty, 0, 0, node_kind[ty]);
...
func ast_type_name(name: int): int {
  if ast_generic_param(name) == 1 then return name;
  if ast_namespace_scope == 0 then return name;
  return sym_qualified(ast_namespace_scope, name);
}
...
func lexer_next(): int {
  ...
  if is_digit(c) == 1 then {
    ...
    if source_peek() == 46 then { source_take(); while is_digit(source_peek()) == 1 { source_take(); } tok_kind = L_FLOAT; tok_length = source_pos - tok_start; tok_text = 0; tok_value = sym_intern(tok_start, tok_length, L_FLOAT, 0); return tok_kind; }
    tok_kind = L_INT;
    tok_length = source_pos - tok_start;
...
func tc_leave_scope(): void {
  ...
  while i < tc_var_count {
    ...
    if source_index < 0 then { } else if source_index < begin && tc_var_borrow_count[source_index] > 0 then tc_var_borrow_count[source_index] = tc_var_borrow_count[source_index] - 1;
    i = i + 1;
  }
  tc_var_count = begin;
}
...
func tc_substitute_type(ty: int): int {
  ...
  if node_kind[ty] == TY_PARAM then {
    ...
    if b != 0 then return tc_substitute_type(b);
...
func tc_same_full(a_kind: int, a_name: int, a_elem_kind: int, a_elem_name: int, b_kind: int, b_name: int, b_elem_kind: int, b_elem_name: int): int {
  if a_kind == TY_PARAM || b_kind == TY_PARAM then {
    if a_kind == TY_PARAM && b_kind == TY_PARAM then return 1;
    return 1;
  }
...
func sym_suffix_equal(full: int, base: int): int {
  if sym_len[full] == sym_len[base] then {
    let i: int = 0; while i < sym_len[base] { if source[sym_start[full] + i] != source[sym_start[base] + i] then return 0; i = i + 1; }
    return 1;
  }
  if sym_len[full] < sym_len[base] + 3 then return 0;
  let start: int = sym_start[full] + sym_len[full] - sym_len[base];
  if source[start - 1] != 58 || source[start - 2] != 58 then return 0;
  let j: int = 0; while j < sym_len[base] { if source[start + j] != source[sym_start[base] + j] then return 0; j = j + 1; }
  return 1;
}
...
func tc_find_struct_ctx(name: int, ns: int): int {
  ...
  if ns == 0 then {
    let broad: int = node_a[tc_root];
    while broad != 0 {
      if (node_kind[broad] == N_STRUCT || node_kind[broad] == N_GENERIC_STRUCT) && sym_suffix_equal(node_value[broad], name) == 1 then return broad;
  ...
  let item: int = node_a[tc_root];
  while item != 0 {
    if (node_kind[item] == N_STRUCT || node_kind[item] == N_GENERIC_STRUCT) && node_scope[item] == ns && sym_suffix_equal(node_value[item], name) == 1 then return item;
    item = node_next[item];
  }
  return 0;
}
...
func tc_find_enum_ctx(name: int, ns: int): int {
  ...
  if ns == 0 then {
    let broad: int = node_a[tc_root];
    while broad != 0 {
      if node_kind[broad] == N_ENUM && sym_suffix_equal(node_value[broad], name) == 1 then return broad;
      broad = node_next[broad];
    }
    return 0;
  }
  let item: int = node_a[tc_root];
  while item != 0 {
    if node_kind[item] == N_ENUM && node_scope[item] == ns && sym_suffix_equal(node_value[item], name) == 1 then return item;
    item = node_next[item];
  }
  return 0;
}
...
func tc_check_type(ty: int): void {
  ...
  if k == TY_NAMED then {
    if tc_named_exists_ctx(node_value[ty], node_scope[ty]) == 0 then tc_fail(2);
...
func tc_borrow_conflict(index: int): int {
  ...
  if index < tc_var_count && tc_var_borrow_count[index] > 0 then return 1;
  return 0;
}
...
func tc_move_value(id: int): void {
  ...
  if tc_is_owner_kind(tc_kind) == 1 then tc_move_var(tc_last_var_index);
}
func tc_record_borrow(destination: int, source_index2: int): void {
  if destination < 0 || source_index2 < 0 then return;
  if destination < tc_var_count then { } else return;
  if source_index2 < tc_var_count then { } else return;
...
func tc_require_mutable(id: int): void {
  ...
  if node_kind[id] == N_VAR then {
    if tc_lookup_var(node_value[id]) == 1 && tc_borrow_conflict(tc_last_var_index) == 1 then tc_fail(37);
  } else if node_kind[id] == N_DEREF || node_kind[id] == N_INDEX || node_kind[id] == N_FIELD_ACCESS then {
    tc_expr(id);
...
func tc_add_var(name: int, kind: int, named: int, elem_kind: int, elem_name: int, type_node: int): void {
  ...
  let i: int = begin;
  while i < tc_var_count {
    if tc_var_name[i] == name then { tc_fail(3); return; }
    i = i + 1;
  }
...
func tc_type_node(ty: int): void {
  ...
  if tc_kind == TY_PTR && node_a[ty] != 0 then {
    ...
    else if (tc_elem_kind == TY_GENERIC || tc_elem_kind == TY_PARAM) then { tc_elem_name = node_a[ty]; }
  } else if tc_kind == TY_DYN_ARRAY && node_a[ty] != 0 then {
    tc_elem_kind = node_kind[node_a[ty]];
    if tc_elem_kind == TY_NAMED then { tc_elem_name = node_value[node_a[ty]]; }
    else if (tc_elem_kind == TY_GENERIC || tc_elem_kind == TY_PARAM) then { tc_elem_name = node_a[ty]; }
  }
...
func tc_expr(id: int): void {
  ...
  if k == N_DEREF then {
    ...
    else { tc_kind = tc_elem_kind; tc_name = tc_elem_name; tc_elem_kind = 0; tc_elem_name = 0; tc_expr_borrow_source = deref_borrow; }
    return;
  }
  if k == N_INDEX then {
    tc_expr(node_b[id]);
  ...
  if k == N_FIELD_ACCESS then {
    ...
    if base_kind == TY_GENERIC then {
      ...
      while gp != 0 && ga != 0 { if tc_bind_add(node_a[gp], ga) == 0 then return; gp = node_next[gp]; ga = node_next[ga]; }
      let gf: int = node_a[sgen];
      while gf != 0 {
        if node_a[gf] == node_value[id] then { let subst: int = tc_substitute_type(node_b[gf]); tc_type_node(subst); return; }
        gf = node_next[gf];
      }
      tc_fail(11); return;
    }
    ...
    let f: int = node_a[s];
    while f != 0 {
      if node_a[f] == node_value[id] then { tc_type_node(node_b[f]); return; }
      f = node_next[f];
    }
    tc_fail(11); return;
  }
  if k == N_CALL then {
    ...
    if fun_node == 0 then {
      if tc_lookup_var(node_value[id]) == 1 && tc_kind == TY_FUN then {
        ...
        let arg_fp: int = node_a[id]; let p_fp: int = node_a[fty];
    ...
    tc_type_node(node_b[fun_node]); return;
  }
...
func tc_find_function_ctx(name: int, ns: int): int {
  ...
  let item: int = node_a[tc_root];
  while item != 0 {
    if (node_kind[item] == N_FUNC || node_kind[item] == N_GENERIC_FUNC || node_kind[item] == N_EXTERN) && node_scope[item] == ns && sym_suffix_equal(node_value[item], name) == 1 then return item;
    item = node_next[item];
  }
  return 0;
}
...
func tc_emit_field_type(id: int): int {
  ...
  if node_kind[decl] == N_GENERIC_STRUCT then {
    ...
    tc_bind_clear();
    while gp != 0 && ga != 0 { if tc_bind_add(node_a[gp], ga) == 0 then return 0; gp = node_next[gp]; ga = node_next[ga]; }
  }
  let f: int = node_a[decl];
  while f != 0 {
    if node_a[f] == node_value[id] then return gen_substitute_type(node_b[f]);
    f = node_next[f];
  }
  return 0;
}
...
func tc_emit_arg_type(id: int): int {
  ...
  if node_kind[id] == N_VAR then {
    ...
    if node_aux[id] != 0 then return node_aux[id];
  }
...
func tc_stmt(id: int, expected_kind: int, expected_name: int): void {
  ...
  if k == N_CONST then {
    ...
    if tc_ok == 1 && node_kind[node_a[id]] == N_VAR && lk == TY_PTR then {
      if lhs_borrow < 0 then { } else if lhs_borrow < tc_var_count && tc_var_borrow_count[lhs_borrow] > 0 then tc_var_borrow_count[lhs_borrow] = tc_var_borrow_count[lhs_borrow] - 1;
    ...
    tc_stmt(node_b[id], expected_kind, expected_name); tc_stmt(node_c[id], expected_kind, expected_name);
  } else if k == N_WHILE then {
...
func tc_diag(): void {
  ...
  else if tc_error_code == 23 then print "type error: return type mismatch";
  else if tc_error_code == 28 then print "type error: recursive struct definition";
  else if tc_error_code == 31 then print "type error: assignment to const";
  else if tc_error_code == 33 then print "type error: use after ownership move";
  else if tc_error_code == 34 then print "type error: use after ownership move";
  else if tc_error_code == 35 then print "type error: release requires an owned value";
  else if tc_error_code == 36 then print "type error: array element type mismatch";
  else if tc_error_code == 37 then print "type error: cannot mutate or move while borrowed";
  else if tc_error_code == 38 then print "type error: borrowed reference escapes its owner";
  else if tc_error_code == 40 then print "type error: owned value copy requires an explicit move";
  ...
  else if tc_error_code == 54 then print "type error: integer literal is out of range for its target type";
...
func tc_program(root: int): int {
  ...
  item = node_a[root];
    while item != 0 { if node_kind[item] == N_FUNC then { tc_var_count = tc_global_count; tc_scope_count = 1; tc_enter_scope(); let p: int = node_c[item];
  while p != 0 { tc_type_node(node_b[p]); let pk: int = tc_kind; let pn: int = tc_name; let pek: int = tc_elem_kind; let pen: int = tc_elem_name; tc_add_var(node_a[p], pk, pn, pek, pen, node_b[p]); if pk == TY_DYN_ARRAY then tc_var_owned[tc_last_var_index] = 1; p = node_next[p]; } tc_type_node(node_b[item]); tc_expected_elem_kind = tc_elem_kind; tc_expected_elem_name = tc_elem_name; tc_stmt(node_a[item], tc_kind, tc_name); tc_leave_scope(); } item = node_next[item]; }
...
func main(argc: int, argv: string*): void {
  ...
  let ok: int = pipeline_main(argv[1]);
  if argc > 2 && ok == 0 then {
