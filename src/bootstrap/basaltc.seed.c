...
static char* basalt_sys_out=NULL;static char* basalt_sys_err=NULL;static int basalt_sys_status_value=0;static int basalt_sys_ok_value=0;static int basalt_sys_truncated_value=0;static int basalt_sys_spawn_error_value=0;static int basalt_sys_cleanup_registered=0;static void basalt_sys_free_buffers(void){if(basalt_sys_out){free(basalt_sys_out);basalt_sys_out=NULL;}if(basalt_sys_err){free(basalt_sys_err);basalt_sys_err=NULL;}}
static BASALT_UNUSED char* basalt_sys_empty(void){char*p=(char*)malloc(1);if(!p)basalt_panic(5);p[0]=0;return(char*)basalt_track(p);}
static BASALT_UNUSED char* basalt_sys_raw_empty(void){char*p=(char*)malloc(1);if(!p)basalt_panic(5);p[0]=0;return p;}
static BASALT_UNUSED char* basalt_sys_copy_n(const char*s,size_t n){char*p=(char*)malloc(n+1);if(!p)basalt_panic(5);if(n)memcpy(p,s,n);p[n]=0;return(char*)basalt_track(p);}
#if defined(_WIN32)
...
#endif
int basalt_sys_run(const char*executable,char**args,int arg_count,int max_output){size_t cap;size_t out_len=0,err_len=0;int i;basalt_sys_free_buffers();if(!basalt_sys_cleanup_registered){atexit(basalt_sys_free_buffers);basalt_sys_cleanup_registered=1;}basalt_sys_status_value=-1;basalt_sys_ok_value=0;basalt_sys_truncated_value=0;basalt_sys_spawn_error_value=0;if(!executable||arg_count<0||max_output<0||max_output>16777216){basalt_sys_spawn_error_value=EINVAL;basalt_sys_out=basalt_sys_raw_empty();basalt_sys_err=basalt_sys_raw_empty();return -1;}cap=(size_t)max_output;basalt_sys_out=(char*)malloc(cap+1);basalt_sys_err=(char*)malloc(cap+1);if(!basalt_sys_out||!basalt_sys_err)basalt_panic(5);basalt_sys_out[0]=0;basalt_sys_err[0]=0;
#if defined(_WIN32)
{char**av=(char**)calloc((size_t)arg_count+2,sizeof(char*));int status;if(!av)basalt_panic(5);av[0]=(char*)executable;for(i=0;i<arg_count;i++)av[i+1]=args[i];av[arg_count+1]=NULL;status=_spawnvp(_P_WAIT,executable,(const char* const*)av);free(av);if(status<0){basalt_sys_spawn_error_value=errno;basalt_sys_status_value=-1;}else{basalt_sys_status_value=status;basalt_sys_ok_value=status==0;}basalt_sys_out[0]=0;basalt_sys_err[0]=0;return basalt_sys_status_value;}
#else
...
int basalt_compile_argv(const char*cc,const char*input_c,const char*output_bin,char**all,int start,int extra_count){char**av;int i,status;if(!cc||!input_c||!output_bin||!all||start<0||extra_count<0)return -1;av=(char**)calloc((size_t)extra_count+4,sizeof(char*));if(!av)basalt_panic(5);for(i=0;i<extra_count;i++)av[i]=all[start+i];av[extra_count]=(char*)input_c;av[extra_count+1]=(char*)"-o";av[extra_count+2]=(char*)output_bin;av[extra_count+3]=NULL;status=basalt_sys_run(cc,av,extra_count+3,65536);if(status!=0&&basalt_sys_stderr()[0]!=0)fputs(basalt_sys_stderr(),stderr);free(av);return status;}
...
int basalt_compile_argv(const char* compiler, const char* input_c, const char* output_bin, char**all_args, int start, int extra_count);
...
char* tc_diag_ascii = " !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";
...
if(sym_len[id]==0)return 0;
else{
}
int i = 0;
while(i<sym_len[id]){
if(ffi_header_char_ok(source[(sym_start[id]+i)])==0)return 0;
else{
}
i = (i+1);
}
...
if(ffi_header_ids[i]==id)return 1;
else{
}
i = (i+1);
}
...
if(ffi_header_seen(id)==1)return;
else{
}
...
if(((ast_parse_mode==1)&&(input_pos>0))&&(input_pos<(input_count+1)))parse_pos = input_source_pos[(input_pos-1)];
else{
}
(void)(ensure_node(id));
node_kind[id] = kind;
...
if(gen_bind_name[i]==name)return gen_bind_type[i];
else{
}
i = (i+1);
}
...
void ensure_gen_closure(int need){
if(need<gen_closure_cap)return;
else{
}
...
while(p!=0){
int src_ty = node_b[p];
int copy_ty = ast_node(node_kind[src_ty], node_a[src_ty], node_b[src_ty], node_c[src_ty], node_value[src_ty], node_aux[src_ty]);
if(args==0)args = copy_ty;
else args = ast_link(args, copy_ty);
p = node_next[p];
}
...
if(gen_closure_node[i]==id)return;
else{
}
i = (i+1);
}
...
void gen_append_uint(int value){
if(value>9)(void)(gen_append_uint((value/10)));
else{
}
...
if(node_kind[ty]==TY_INT)(void)(gen_append_text("int"));
else if(node_kind[ty]==TY_BOOL)(void)(gen_append_text("bool"));
else if(node_kind[ty]==TY_STRING)(void)(gen_append_text("char_ptr"));
else if(node_kind[ty]==TY_CHAR)(void)(gen_append_text("char"));
else if(node_kind[ty]==TY_FLOAT)(void)(gen_append_text("float"));
else if(node_kind[ty]==TY_DOUBLE)(void)(gen_append_text("double"));
else if(node_kind[ty]==TY_U8)(void)(gen_append_text("u8"));
else if(node_kind[ty]==TY_U16)(void)(gen_append_text("u16"));
else if(node_kind[ty]==TY_U32)(void)(gen_append_text("u32"));
else if(node_kind[ty]==TY_U64)(void)(gen_append_text("u64"));
else if(node_kind[ty]==TY_I8)(void)(gen_append_text("i8"));
else if(node_kind[ty]==TY_I16)(void)(gen_append_text("i16"));
else if(node_kind[ty]==TY_I32)(void)(gen_append_text("i32"));
else if(node_kind[ty]==TY_I64)(void)(gen_append_text("i64"));
else if(node_kind[ty]==TY_USIZE)(void)(gen_append_text("usize"));
else if(node_kind[ty]==TY_VOID)(void)(gen_append_text("void"));
else if(node_kind[ty]==TY_NAMED)(void)(gen_append_c_symbol(node_value[ty]));
...
if(node_next[a]!=0)(void)(gen_append_text("__"));
else{
}
a = node_next[a];
}
...
void gen_add_tuple_type(int ty){
if((ty==0)||(node_kind[ty]!=TY_TUPLE))return;
else{
}
int name = gen_mangled_type_symbol(ty);
int i = 0;
while(i<gen_tuple_count){
if(gen_tuple_name[i]==name)return;
else{
}
i = (i+1);
}
...
void ensure_gen_struct_state(int need){
if(need<gen_struct_state_cap)return;
else{
}
...
void ensure_gen_spec_state(int need){
if(need<gen_spec_state_cap)return;
else{
}
...
if(gen_spec_name[i]==name)return i;
else{
}
}
else{
...
if(node_kind[ty]==TY_PARAM){
int b = gen_bind_find(node_value[ty]);
if(b!=0)return gen_substitute_type(b);
else{
}
...
if(node_kind[ty]==TY_PTR)return ast_node(TY_PTR, gen_substitute_type(node_a[ty]), 0, 0, 0, 0);
else{
}
if(node_kind[ty]==TY_ARRAY)return ast_node(TY_ARRAY, gen_substitute_type(node_a[ty]), 0, 0, node_value[ty], 0);
else{
}
if(node_kind[ty]==TY_DYN_ARRAY)return ast_node(TY_DYN_ARRAY, gen_substitute_type(node_a[ty]), 0, 0, 0, 0);
else{
}
...
int gen_active_closure_capture(int name){
if(gen_active_closure==0)return 0;
else{
}
int cap_probe = node_a[gen_active_closure];
while(cap_probe!=0){
if(node_a[cap_probe]==name)return cap_probe;
else{
}
...
int gen_closure_active_env_name(void){
if(gen_active_closure==0)return gen_closure_env_local_name(0);
else{
}
...
int gen_active_param_type(int name){
if(gen_active_function==0)return 0;
else{
}
int p = node_c[gen_active_function];
while(p!=0){
if(node_a[p]==name)return node_b[p];
else{
}
p = node_next[p];
}
...
if(node_a[id]==name)return node_b[id];
else{
}
...
if(k==N_WHILE)return gen_local_decl_type(node_b[id], name);
else{
}
if(k==N_FOR){
int found_for = gen_local_decl_type(node_a[id], name);
if(found_for!=0)return found_for;
else{
}
...
if(found_arm!=0)return found_arm;
else{
}
arm = node_next[arm];
}
...
int i = 0;
while(i<gen_spec_count){
if(((gen_spec_kind[i]==kind)&&(gen_spec_decl[i]==decl))&&(gen_spec_name[i]==name))return 1;
else{
}
i = (i+1);
}
...
if((q==0)||(node_kind[q]!=TY_GENERIC))return;
else{
}
...
if(gen_spec_exists(1, decl, name)==1)return;
else{
}
...
if(node_kind[ty]==TY_PARAM)return 1;
else{
}
if(((node_kind[ty]==TY_PTR)||(node_kind[ty]==TY_ARRAY))||(node_kind[ty]==TY_DYN_ARRAY))return gen_type_has_param(node_a[ty]);
else{
}
if(node_kind[ty]==TY_GENERIC){
int a = node_a[ty];
while(a!=0){
if(gen_type_has_param(a)==1)return 1;
else{
}
a = node_next[a];
}
...
while(p!=0){
int q = gen_substitute_type(p);
if(actual==0)actual = q;
else actual = ast_link(actual, q);
p = node_next[p];
}
...
if(bt==0)bt = ast_node(TY_PARAM, 0, 0, 0, node_a[tp], 0);
else{
}
int cq = gen_substitute_type(bt);
if(typeargs==0)typeargs = cq;
else typeargs = ast_link(typeargs, cq);
tp = node_next[tp];
}
(void)(gen_bind_clear());
int restore_i = 0;
...
if(gen_spec_exists(2, decl, name)==1)return;
else{
}
(void)(ensure_gen_specs(gen_spec_count));
...
else if(((node_kind[ty]==TY_PTR)||(node_kind[ty]==TY_ARRAY))||(node_kind[ty]==TY_DYN_ARRAY)){
...
(void)(tc_expr(aa));
q = tc_result_type;
if(q==0)q = tc_type_node_from_summary(tc_kind, tc_name, tc_elem_kind, tc_elem_name);
else{
}
}
else{
}
if(q!=0)q = gen_substitute_type(q);
else{
}
if(actual==0)actual = q;
else actual = ast_link(actual, q);
...
if(gen_type_has_param(check_actual)==1)unresolved = 1;
else{
}
check_actual = node_next[check_actual];
}
if(unresolved==0)(void)(gen_add_fun_spec(f, actual));
else{
}
}
else{
...
void gen_primitive_type(int kind){
if(kind==TY_INT)(void)(code_emit(C_KW, 1));
else if(kind==TY_BOOL)(void)(code_emit(C_KW, 2));
else if(kind==TY_STRING)(void)(code_emit(C_KW, 3));
else if(kind==TY_CHAR)(void)(code_emit(C_KW, 17));
else if(kind==TY_FLOAT)(void)(code_emit(C_KW, 18));
else if(kind==TY_DOUBLE)(void)(code_emit(C_KW, 15));
else if(kind==TY_LONG)(void)(code_emit(C_KW, 19));
else if(kind==TY_LLONG)(void)(code_emit(C_KW, 20));
else if(kind==TY_U8)(void)(code_emit(C_KW, 21));
else if(kind==TY_U16)(void)(code_emit(C_KW, 22));
else if(kind==TY_U32)(void)(code_emit(C_KW, 23));
else if(kind==TY_U64)(void)(code_emit(C_KW, 24));
else if(kind==TY_I8)(void)(code_emit(C_KW, 25));
else if(kind==TY_I16)(void)(code_emit(C_KW, 26));
else if(kind==TY_I32)(void)(code_emit(C_KW, 27));
else if(kind==TY_I64)(void)(code_emit(C_KW, 28));
else if(kind==TY_USIZE)(void)(code_emit(C_KW, 29));
else if(kind==TY_VOID)(void)(code_emit(C_KW, 4));
else{
}
}
void gen_type(int kind, int child, int size){
...
if(b!=0)(void)(gen_type(node_kind[b], b, node_value[b]));
else(void)(code_emit(C_IDENT, sym_c_symbol(node_value[child])));
(void)(code_emit(C_PUNCT, 18));
}
else(void)(gen_primitive_type(kind));
}
int gen_scalar_kind(int arg){
if(arg!=0){
...
if((resolved!=0)&&(node_kind[resolved]!=TY_PARAM))return node_kind[resolved];
else{
}
}
else{
}
if(node_kind[arg]==N_INT)return TY_INT;
else{
}
if(node_kind[arg]==N_BOOL)return TY_BOOL;
else{
}
if(node_kind[arg]==N_CHAR)return TY_CHAR;
else{
}
if(node_kind[arg]==N_FLOAT)return TY_DOUBLE;
else{
}
if(node_kind[arg]==N_STRING)return TY_STRING;
else{
}
if(node_kind[arg]==N_VAR)return sym_type[node_value[arg]];
else{
}
}
else{
...
if(typed!=0){
int resolved = gen_substitute_type(typed);
if((resolved!=0)&&(node_kind[resolved]==TY_NAMED))return node_value[resolved];
else{
}
}
else{
}
if(node_kind[arg]==N_VAR)return sym_elem_name[node_value[arg]];
else{
}
}
else{
...
if(((resolved_type!=0)&&(node_kind[resolved_type]==TY_DYN_ARRAY))&&(node_a[resolved_type]!=0))return node_kind[node_a[resolved_type]];
else{
}
}
else{
...
if(((kind==TY_PARAM)&&(elem_name!=0))&&(node_kind[elem_name]==TY_PARAM))param_name = node_value[elem_name];
else{
}
if(kind==TY_PARAM){
...
if(resolved!=0)return node_kind[resolved];
else{
}
}
else{
...
if(formal_type!=0){
int resolved_type = gen_substitute_type(formal_type);
if((((resolved_type!=0)&&(node_kind[resolved_type]==TY_DYN_ARRAY))&&(node_a[resolved_type]!=0))&&(node_kind[node_a[resolved_type]]==TY_NAMED))return node_value[node_a[resolved_type]];
else{
}
}
else{
...
else if(kind==TY_NAMED)(void)(code_emit(C_IDENT, sym_c_symbol(name)));
else if(kind==TY_PTR){
(void)(code_emit(C_KW, 1));
(void)(code_emit(C_PUNCT, 1));
}
else if(kind==TY_PARAM)(void)(code_emit(C_KW, 1));
else(void)(gen_primitive_type(kind));
}
void gen_array_sizeof(int kind, int name){
...
(void)(code_emit(C_IDENT, (0-1011)));
(void)(code_emit(C_PUNCT, 4));
if((ty!=0)&&(node_kind[ty]==TY_GENERIC))(void)(code_emit(C_IDENT, gen_mangled_type_symbol(ty)));
else if(ty!=0)(void)(gen_array_elem_type(node_kind[ty], node_value[ty]));
else(void)(gen_array_elem_type(TY_INT, 0));
(void)(code_emit(C_PUNCT, 5));
}
void gen_memory_sizeof(int arg){
int ty = tc_emit_arg_type(arg);
if(ty!=0)ty = gen_substitute_type(ty);
else{
}
...
else if(ty!=0)(void)(gen_array_sizeof_node(ty));
else(void)(gen_array_sizeof_node(0));
}
void gen_memory_builtin(int id){
...
if(wty!=0)gen_witness_ty = gen_substitute_type(wty);
else{
}
}
else{
...
(void)(code_emit(C_PUNCT, 6));
if(gen_witness_ty!=0)(void)(code_emit(C_IDENT, gen_mangled_type_symbol(gen_witness_ty)));
else(void)(gen_array_elem_type(elem_kind, elem_name));
(void)(code_emit(C_PUNCT, 1));
(void)(code_emit(C_PUNCT, 8));
if(btag==BI_TC_MEM_ALLOC_ALIGNED)(void)(code_emit(C_IDENT, (0-1019)));
else(void)(code_emit(C_IDENT, (0-1016)));
(void)(code_emit(C_PUNCT, 6));
(void)(gen_expr(a));
(void)(code_emit(C_PUNCT, 7));
if(btag==BI_TC_MEM_ALLOC_ALIGNED){
...
else(void)(gen_array_sizeof(elem_kind, elem_name));
(void)(code_emit(C_PUNCT, 8));
(void)(code_emit(C_PUNCT, 8));
}
...
if(ptr_ty!=0)ptr_ty = gen_substitute_type(ptr_ty);
else{
}
if(ptr_ty==0)ptr_ty = ast_node(TY_PTR, ast_node(TY_INT, 0, 0, 0, 0, 0), 0, 0, 0, 0);
else{
}
(void)(code_emit(C_PUNCT, 6));
(void)(code_emit(C_PUNCT, 6));
...
while(arg!=0){
(void)(gen_expr(arg));
if(node_next[arg]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
arg = node_next[arg];
}
...
int gen_call_name(int id){
int f = tc_find_function_ctx(node_value[id], node_scope[id]);
if(f==0)return sym_c_symbol(node_value[id]);
else{
}
if(node_kind[f]!=N_GENERIC_FUNC)return sym_c_symbol(node_value[f]);
else{
}
int actual = 0;
int a = node_a[id];
...
if(((enum_name==0)&&(node_aux[id]!=0))&&(node_kind[node_aux[id]]==TY_NAMED))enum_name = node_value[node_aux[id]];
else{
}
...
(void)(code_emit(C_PUNCT, 11));
(void)(gen_expr(arg));
if((node_next[field]!=0)&&(node_next[arg]!=0))(void)(code_emit(C_PUNCT, 7));
else{
}
field = node_next[field];
arg = node_next[arg];
...
if(k==N_INT){
if(node_aux[id]!=0){
if(node_value[id]<0)(void)(code_emit(C_RAW_U64, node_aux[id]));
else(void)(code_emit(C_RAW, node_aux[id]));
}
else(void)(code_emit(C_INT, node_value[id]));
}
else if(k==N_BOOL)(void)(code_emit(C_INT, node_value[id]));
else if(k==N_FLOAT){
(void)(code_emit(C_IDENT, node_value[id]));
if(node_aux[id]==TY_FLOAT)(void)(code_emit(C_IDENT, (0-1021)));
else{
}
}
else if(k==N_STRING)(void)(code_emit(C_STRING, node_value[id]));
else if(k==N_CHAR)(void)(code_emit(C_INT, node_value[id]));
else if(k==N_VARIANT)(void)(gen_variant_expr(id));
else if(k==N_TUPLE){
...
(void)(code_emit(C_PUNCT, 11));
(void)(gen_expr(item));
if(node_next[item]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
item = node_next[item];
index = (index+1);
...
if((node_aux[closure_cap_expr]==2)||(node_aux[closure_cap_expr]==3))(void)(code_emit(C_PUNCT, 10));
else{
}
(void)(gen_expr(capture_var_expr));
if(node_next[closure_cap_expr]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
if(tagged==0)(void)(code_emit(C_IDENT, sym_c_symbol(sym_qualified(enum_name, node_a[member]))));
else{
(void)(code_emit(C_PUNCT, 4));
(void)(code_emit(C_IDENT, sym_c_symbol(enum_name)));
(void)(code_emit(C_PUNCT, 5));
...
else(void)(code_emit(C_IDENT, sym_c_symbol(node_value[id])));
}
else(void)(code_emit(C_IDENT, sym_c_symbol(node_value[id])));
}
else if(k==N_BINOP){
...
else if(((node_value[id]==OP_SUB)&&(gen_expr_kind(node_a[id])==TY_PTR))&&(gen_expr_kind(node_b[id])==TY_PTR)){
(void)(code_emit(C_PUNCT, 4));
(void)(code_emit(C_KW, 1));
(void)(code_emit(C_PUNCT, 5));
(void)(code_emit(C_PUNCT, 4));
...
if(node_c[id]==TY_FLOAT)(void)(code_emit(C_PUNCT, 5));
else{
}
}
else if(k==N_CALL){
int call_name = node_value[id];
int btag = bi_tag(call_name);
if((((btag==BI_TC_MEM_ALLOC)||(btag==BI_TC_MEM_ALLOC_ALIGNED))||(btag==BI_TC_MEM_RESIZE))||(btag==BI_TC_MEM_FREE))(void)(gen_memory_builtin(id));
else{
...
if(closure_arg_emit!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
while(closure_arg_emit!=0){
(void)(gen_expr(closure_arg_emit));
if(node_next[closure_arg_emit]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
(void)(gen_expr(node_a[id]));
if(gen_expr_kind(node_a[id])==TY_PTR)(void)(code_emit(C_PUNCT, 27));
else(void)(code_emit(C_PUNCT, 17));
(void)(code_emit(C_IDENT, node_value[id]));
}
else{
}
...
int gen_expr_kind(int id){
int k = node_kind[id];
if((k==N_INT)||(k==N_BOOL))return TY_INT;
else{
}
if(k==N_CLOSURE)return TY_CLOSURE;
else{
}
if(k==N_MOVE)return gen_expr_kind(node_a[id]);
else{
}
if(k==N_TUPLE)return TY_TUPLE;
else{
}
if(k==N_VARIANT){
if(node_aux[id]!=0)return node_kind[node_aux[id]];
else{
}
return TY_NAMED;
}
...
if(k==N_CHAR)return TY_CHAR;
else{
}
if(k==N_FLOAT){
if(node_aux[id]==TY_FLOAT)return TY_FLOAT;
else{
}
return TY_DOUBLE;
}
...
if(k==N_STRING)return TY_STRING;
else{
}
if(k==N_NULL)return TY_PTR;
else{
}
if(k==N_UNARY)return TY_BOOL;
else{
}
if(k==N_VAR){
...
if(formal_type!=0){
int resolved_type = gen_substitute_type(formal_type);
if(resolved_type!=0)return node_kind[resolved_type];
else{
}
}
else{
...
if(bt==TY_STRING)return TY_CHAR;
else{
}
if(bt==TY_PTR)return TY_INT;
else{
}
return TY_INT;
}
...
if(k==N_DEREF)return TY_INT;
else{
}
if(k==N_ADDRESS)return TY_PTR;
else{
}
if(k==N_FIELD_ACCESS){
int field_ty = tc_emit_field_type(id);
if(field_ty!=0)return node_kind[field_ty];
else{
}
return TY_INT;
}
...
if((k==N_CALL)||(k==N_INDIRECT_CALL))return tc_expr_kind_for_emit(id);
else{
}
if(k==N_BINOP){
if(node_value[id]==OP_CONCAT)return TY_STRING;
else{
}
if((((((((node_value[id]==OP_EQ)||(node_value[id]==OP_NEQ))||(node_value[id]==OP_LT))||(node_value[id]==OP_GT))||(node_value[id]==OP_LE))||(node_value[id]==OP_GE))||(node_value[id]==OP_AND))||(node_value[id]==OP_OR))return TY_BOOL;
else{
}
if(((node_value[id]==OP_SUB)&&(gen_expr_kind(node_a[id])==TY_PTR))&&(gen_expr_kind(node_b[id])==TY_PTR))return TY_INT;
else{
}
...
if((ak==TY_LLONG)||(bk==TY_LLONG))return TY_LLONG;
else{
}
if((ak==TY_LONG)||(bk==TY_LONG))return TY_LONG;
else{
}
return TY_INT;
}
...
if((ak==TY_DOUBLE)||(bk==TY_DOUBLE))return TY_DOUBLE;
else{
}
if((ak==TY_FLOAT)||(bk==TY_FLOAT))return TY_FLOAT;
else{
}
if((ak==TY_LLONG)||(bk==TY_LLONG))return TY_LLONG;
else{
...
if(((((node_kind[st]==TY_NAMED)||(node_kind[st]==TY_GENERIC))||(node_kind[st]==TY_ARRAY))&&(node_kind[expr]==N_INT))&&(node_value[expr]==0))(void)(code_emit(C_PUNCT, 19));
...
else(void)(gen_expr(rhs));
}
int compound_c_operator(int op){
if(op==OP_ADD)return 19;
else{
}
if(op==OP_SUB)return 20;
else{
}
if(op==OP_MUL)return 21;
else{
}
if(op==OP_DIV)return 22;
else{
}
if(op==OP_MOD)return 23;
else{
}
if(op==OP_BITAND)return 24;
else{
}
if(op==OP_BITOR)return 25;
else{
}
if(op==OP_BITXOR)return 26;
else{
}
if(op==OP_SHL)return 27;
else{
}
if(op==OP_SHR)return 28;
else{
}
return 19;
}
...
else if(node_kind[id]==N_EXPR)(void)(gen_expr(node_a[id]));
else{
}
}
void gen_fun_decl(int ty, int name){
...
if(node_next[p]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
p = node_next[p];
}
...
void gen_decl(int ty, int name){
if(node_kind[ty]==TY_FUN)(void)(gen_fun_decl(ty, name));
...
int add_newline = 0;
if(k==N_PRINTLN)add_newline = 1;
else{
}
if(pk==TY_STRING){
if(add_newline==1)(void)(code_emit(C_PUNCT, 16));
else(void)(code_emit(C_PUNCT, 32));
}
else if(pk==TY_CHAR){
if(add_newline==1)(void)(code_emit(C_PUNCT, 20));
else(void)(code_emit(C_PUNCT, 33));
}
else if((pk==TY_FLOAT)||(pk==TY_DOUBLE)){
if(add_newline==1)(void)(code_emit(C_PUNCT, 21));
else(void)(code_emit(C_PUNCT, 34));
}
else if(pk==TY_LONG){
if(add_newline==1)(void)(code_emit(C_PUNCT, 28));
else(void)(code_emit(C_PUNCT, 35));
}
else if(pk==TY_LLONG){
if(add_newline==1)(void)(code_emit(C_PUNCT, 29));
else(void)(code_emit(C_PUNCT, 36));
}
else if(pk==TY_PTR){
if(add_newline==1)(void)(code_emit(C_PUNCT, 26));
else(void)(code_emit(C_PUNCT, 37));
(void)(code_emit(C_PUNCT, 4));
(void)(code_emit(C_KW, 4));
(void)(code_emit(C_PUNCT, 1));
(void)(code_emit(C_PUNCT, 5));
...
if(add_newline==1)(void)(code_emit(C_PUNCT, 15));
else(void)(code_emit(C_PUNCT, 38));
}
(void)(gen_expr(node_a[id]));
(void)(code_emit(C_PUNCT, 8));
(void)(code_emit(C_PUNCT, 12));
...
if(elem_ty!=0)(void)(gen_type(node_kind[elem_ty], elem_ty, 0));
else(void)(code_emit(C_KW, 1));
...
if(node_a[id]!=0)(void)(gen_expr(node_a[id]));
else{
}
(void)(code_emit(C_PUNCT, 12));
(void)(code_emit(C_NEWLINE, 0));
...
if(emit_loop_depth>0)(void)(gen_emit_defer_from(emit_loop_base[(emit_loop_depth-1)]));
else{
}
...
if(emit_for_step!=0)(void)(gen_stmt(emit_for_step));
else{
}
...
if((node_kind[actual]==TY_PARAM)&&(node_value[formal]==node_value[actual]))return;
else{
}
if(gen_bind_find(node_value[formal])==0)(void)(gen_bind_add(node_value[formal], actual));
else{
}
return;
}
...
if(node_value[formal]!=node_value[actual])return;
else{
}
...
if(((node_kind[formal]==TY_FUN)||(node_kind[formal]==TY_CLOSURE))&&((node_kind[actual]==TY_FUN)||(node_kind[actual]==TY_CLOSURE))){
...
(void)(code_emit(C_PUNCT, 4));
(void)(code_emit(C_IDENT, temp));
if(tagged==1)(void)(code_emit(C_PUNCT, 17));
else{
}
if(tagged==1)(void)(code_emit(C_IDENT, sym_tag_id()));
else{
}
...
if(gen_struct_state[decl]==2)return;
else{
}
if(gen_struct_state[decl]==1)return;
else{
}
...
if(gen_spec_state[index]==2)return;
else{
}
if(gen_spec_state[index]==1)return;
else{
}
...
if(node_kind[q]==TY_DYN_ARRAY)return;
else{
}
...
if(index>(0-1))(void)(gen_emit_complete_spec(index));
else{
}
}
else{
...
if(node_next[f]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
(void)(code_emit(C_NEWLINE, 0));
f = node_next[f];
...
else(void)(gen_decl(ty, name));
}
void gen_function_signature(int id){
if(bi_has_flag(node_value[id], BI_FLAG_MAIN)==1)(void)(code_emit(C_KW, 1));
else(void)(gen_type(node_aux[id], node_b[id], 0));
(void)(code_emit(C_IDENT, sym_c_symbol(node_value[id])));
(void)(code_emit(C_PUNCT, 6));
int param = node_c[id];
if(param==0)(void)(code_emit(C_KW, 4));
else{
while(param!=0){
if(node_kind[id]==N_EXTERN)(void)(gen_extern_param(node_b[param], node_a[param]));
else(void)(gen_decl(node_b[param], node_a[param]));
if(node_next[param]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
else(void)(gen_type(node_kind[source_ty_emit], source_ty_emit, node_value[source_ty_emit]));
...
if(gen_closure_node[value_scan_emit]==closure_id)break;
else{
}
if(gen_closure_value_type_name[value_scan_emit]==value_name_emit)duplicate_value_emit = 1;
else{
}
value_scan_emit = (value_scan_emit+1);
}
if(duplicate_value_emit==1)return;
else{
}
(void)(code_emit(C_KW, 12));
...
if(param_emit!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
while(param_emit!=0){
(void)(gen_type(node_kind[param_emit], param_emit, node_value[param_emit]));
if(node_next[param_emit]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
if(param_proto!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
while(param_proto!=0){
(void)(gen_decl(node_b[param_proto], node_a[param_proto]));
if(node_next[param_proto]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
if(param_call!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
while(param_call!=0){
(void)(gen_decl(param_call, gen_closure_name(0, "arg")));
if(node_next[param_call]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
if(arg_call!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
while(arg_call!=0){
(void)(code_emit(C_IDENT, gen_closure_name(0, "arg")));
if(node_next[arg_call]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
if(cap_factory==0)(void)(code_emit(C_KW, 4));
else{
while(cap_factory!=0){
...
else(void)(gen_decl(cap_ty_factory, node_a[cap_factory]));
if(node_next[cap_factory]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
if(param_invoke!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
while(param_invoke!=0){
(void)(gen_decl(node_b[param_invoke], node_a[param_invoke]));
if(node_next[param_invoke]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
...
(void)(gen_type(node_kind[node_b[cap_invoke]], node_b[cap_invoke], node_value[node_b[cap_invoke]]));
...
closure_i = 0;
while(closure_i<gen_closure_count){
(void)(gen_closure_emit_invoke_prototype(gen_closure_serial[closure_i], gen_closure_node[closure_i]));
closure_i = (closure_i+1);
}
closure_i = 0;
while(closure_i<gen_closure_count){
...
if(gen_closure_value_type_name[helper_scan]==gen_closure_value_type_name[closure_i])helper_seen = 1;
else{
}
helper_scan = (helper_scan+1);
}
if(helper_seen==0)(void)(gen_closure_emit_call_helper(gen_closure_sig[closure_i]));
else{
}
closure_i = (closure_i+1);
}
...
if(node_a[item]!=0)(void)(ffi_header_register(node_a[item]));
else{
}
}
else{
}
if((node_kind[item]==N_GLOBAL)||(node_kind[item]==N_CONST))(void)(gen_collect_stmt(item));
...
item = node_a[id];
while(item!=0){
if(node_kind[item]==N_STRUCT)(void)(gen_emit_complete_struct(item));
else if(node_kind[item]==N_ENUM)(void)(gen_enum_decl(item));
else{
}
item = node_next[item];
}
si = 0;
while(si<gen_spec_count){
if(gen_spec_kind[si]==1)(void)(gen_emit_complete_spec(si));
else{
}
si = (si+1);
}
item = node_a[id];
while(item!=0){
if((node_kind[item]==N_FUNC)||(node_kind[item]==N_EXTERN))(void)(gen_prototype(item));
else{
}
item = node_next[item];
}
...
if(pp==0)(void)(code_emit(C_KW, 4));
else{
while(pp!=0){
int pt = gen_substitute_type(node_b[pp]);
(void)(gen_decl(pt, node_a[pp]));
if(node_next[pp]!=0)(void)(code_emit(C_PUNCT, 7));
else{
}
pp = node_next[pp];
}
...
item = node_a[id];
while(item!=0){
if((node_kind[item]==N_GLOBAL)||(node_kind[item]==N_CONST))(void)(gen_stmt(item));
else{
}
item = node_next[item];
}
si = 0;
while(si<gen_spec_count){
if(gen_spec_kind[si]==2)(void)(gen_function_specialized(gen_spec_decl[si], gen_spec_type[si], gen_spec_name[si]));
else{
}
si = (si+1);
}
item = node_a[id];
while(item!=0){
if(node_kind[item]==N_FUNC)(void)(gen_function(item));
else{
}
item = node_next[item];
}
...
int same = 1;
i = 0;
if(code_count!=first_count)same = 0;
else{
}
while(i<first_count){
if(code_kind[i]!=snapshot_kind[i])same = 0;
else{
}
if(code_value[i]!=snapshot_value[i])same = 0;
else{
}
i = (i+1);
}
...
int input_peek(void){
if(input_pos<input_count)return input_kind[input_pos];
else return T_EOF;
}
int input_payload(void){
if(input_pos<input_count)return input_value[input_pos];
else return 0;
}
int input_text_payload(void){
if(input_pos<input_count)return input_text[input_pos];
else return 0;
}
int input_take(int kind){
if(input_peek()==kind){
...
int ast_generic_params(void){
int params = 0;
if(input_take(T_LT)==0)return 0;
else{
}
while(1==1){
if(input_peek()!=T_ID)return(0-1);
else{
}
...
else params = ast_link(params, p);
if(input_take(T_COMMA)==1){
}
else break;
}
if(input_take(T_GT)==0)return(0-1);
else{
}
return params;
}
...
if(input_take(T_RPAREN)==0)return 0;
else{
}
...
if(input_take(T_RBRACK)==0)return 0;
else{
}
...
if(input_take(T_LPAREN)==0)return 0;
else{
}
int args = 0;
...
if(input_take(T_COLON)==0)return 0;
else{
}
int ret = ast_type();
if(ret==0)return 0;
...
if((input_peek()!=T_ID)&&(input_peek()!=T_ARRAY))return 0;
else{
}
...
if(input_take(T_TINT)==1)base = TY_INT;
else if(input_take(T_TBOOL)==1)base = TY_BOOL;
else if(input_take(T_TSTRING)==1)base = TY_STRING;
else if(input_take(T_TCHAR)==1)base = TY_CHAR;
else if(input_take(T_FLOAT)==1)base = TY_FLOAT;
else if(input_take(T_TDOUBLE)==1)base = TY_DOUBLE;
else if(input_take(T_TLONG)==1){
if(input_take(T_TLONG)==1)base = TY_LLONG;
else base = TY_LONG;
}
else if(input_take(T_TU8)==1)base = TY_U8;
else if(input_take(T_TU16)==1)base = TY_U16;
else if(input_take(T_TU32)==1)base = TY_U32;
else if(input_take(T_TU64)==1)base = TY_U64;
else if(input_take(T_TI8)==1)base = TY_I8;
else if(input_take(T_TI16)==1)base = TY_I16;
else if(input_take(T_TI32)==1)base = TY_I32;
else if(input_take(T_TI64)==1)base = TY_I64;
else if(input_take(T_TUSIZE)==1)base = TY_USIZE;
else if(input_take(T_TVOID)==1)base = TY_VOID;
else if(input_peek()==T_ID){
named = input_payload();
input_pos = (input_pos+1);
if(input_take(T_SCOPE)==1){
if(input_peek()!=T_ID)return 0;
else{
}
int rhs = input_payload();
input_pos = (input_pos+1);
...
else if(ast_generic_param(named)==1)ty = ast_node(TY_PARAM, 0, 0, 0, named, 0);
else ty = ast_node(TY_NAMED, 0, 0, 0, named, 0);
}
else return 0;
if(ty==0)ty = ast_node(base, 0, 0, 0, named, 0);
else{
}
}
while(input_take(T_STAR)==1){
...
if(input_take(T_RPAREN)==0)return(0-1);
else{
}
return args;
}
...
while(1==1){
int mode = 2;
if(input_take(T_MOVE)==1)mode = 1;
else if(input_take(T_BORROW)==1)mode = 2;
else if(input_take(T_BORROW_MUT)==1)mode = 3;
else{
}
if(input_peek()!=T_ID)return(0-1);
else{
...
if(captures==0)captures = capture;
else captures = ast_link(captures, capture);
if(input_take(T_COMMA)==0)break;
else{
}
}
}
...
if(input_take(T_RBRACK)==0)return(0-1);
else{
}
}
else{
}
if(input_take(T_LPAREN)==0)return(0-1);
else{
}
...
if(input_take(T_COLON)==0)return(0-1);
else{
}
int ret_ty = ast_type();
if(ret_ty==0)return(0-1);
else{
}
int body = ast_stmt();
...
if(input_take(T_NULL)==1)return ast_node(N_NULL, 0, 0, 0, 0, 0);
else{
}
...
if(input_take(T_TRUE)==1)return ast_node(N_BOOL, 0, 0, 0, 1, 0);
else{
}
if(input_take(T_FALSE)==1)return ast_node(N_BOOL, 0, 0, 0, 0, 0);
else{
}
...
if((input_peek()!=T_ID)&&(input_peek()!=T_ARRAY))return(0-1);
else{
}
int rhs = input_payload();
input_pos = (input_pos+1);
...
int ast_precedence(int kind){
if(kind==T_OR_OR)return 1;
else{
}
if(kind==T_AND_AND)return 2;
else{
}
if(kind==T_EQEQ)return 3;
else{
}
if(kind==T_NEQ)return 3;
else{
}
if(kind==T_LT)return 3;
else{
}
if(kind==T_GT)return 3;
else{
}
if(kind==T_LE)return 3;
else{
}
if(kind==T_GE)return 3;
else{
}
if(kind==T_CONCAT)return 4;
else{
}
if(kind==T_BITOR)return 5;
else{
}
if(kind==T_BITXOR)return 6;
else{
}
if(kind==T_AMP)return 7;
else{
}
if(kind==T_SHL)return 8;
else{
}
if(kind==T_SHR)return 8;
else{
}
if(kind==T_PLUS)return 9;
else{
}
if(kind==T_MINUS)return 9;
else{
}
if(kind==T_STAR)return 10;
else{
}
if(kind==T_DIVIDE)return 10;
else{
}
if(kind==T_MOD)return 10;
else{
}
return 0;
}
int ast_operator(int kind){
if(kind==T_PLUS)return OP_ADD;
else{
}
if(kind==T_MINUS)return OP_SUB;
else{
}
if(kind==T_STAR)return OP_MUL;
else{
}
if(kind==T_DIVIDE)return OP_DIV;
else{
}
if(kind==T_MOD)return OP_MOD;
else{
}
if(kind==T_EQEQ)return OP_EQ;
else{
}
if(kind==T_NEQ)return OP_NEQ;
else{
}
if(kind==T_LT)return OP_LT;
else{
}
if(kind==T_GT)return OP_GT;
else{
}
if(kind==T_LE)return OP_LE;
else{
}
if(kind==T_GE)return OP_GE;
else{
}
if(kind==T_AND_AND)return OP_AND;
else{
}
if(kind==T_OR_OR)return OP_OR;
else{
}
if(kind==T_CONCAT)return OP_CONCAT;
else{
}
if(kind==T_AMP)return OP_BITAND;
else{
}
if(kind==T_BITOR)return OP_BITOR;
else{
}
if(kind==T_BITXOR)return OP_BITXOR;
else{
}
if(kind==T_SHL)return OP_SHL;
else{
}
if(kind==T_SHR)return OP_SHR;
else{
}
return OP_GT;
}
int ast_compound_operator(int kind){
if(kind==T_PLUS_EQ)return OP_ADD;
else{
}
if(kind==T_MINUS_EQ)return OP_SUB;
else{
}
if(kind==T_STAR_EQ)return OP_MUL;
else{
}
if(kind==T_DIV_EQ)return OP_DIV;
else{
}
if(kind==T_MOD_EQ)return OP_MOD;
else{
}
if(kind==T_AMP_EQ)return OP_BITAND;
else{
}
if(kind==T_BITOR_EQ)return OP_BITOR;
else{
}
if(kind==T_BITXOR_EQ)return OP_BITXOR;
else{
}
if(kind==T_SHL_EQ)return OP_SHL;
else{
}
if(kind==T_SHR_EQ)return OP_SHR;
else{
}
return 0;
}
...
int ast_alignment(void){
if(input_take(T_ALIGNAS)==0)return 0;
else{
}
if(input_take(T_LPAREN)==0)return(0-1);
else{
}
if(input_peek()!=T_INT)return(0-1);
else{
}
int alignment = input_payload();
input_pos = (input_pos+1);
if(alignment<1)return(0-1);
else{
}
int power = 1;
while(power<alignment){
if(power>1073741824)return(0-1);
else{
}
power = (power*2);
}
if(power!=alignment)return(0-1);
else{
}
if(input_take(T_RPAREN)==0)return(0-1);
else{
...
if(input_take(T_SEMI)==0)return(0-1);
else{
}
...
if(input_take(T_LBRACE)==0)return(0-1);
else{
}
int arms = 0;
while(input_peek()!=T_RBRACE){
if((input_peek()==T_EOF)||(input_peek()!=T_ID))return(0-1);
else{
}
...
if(input_take(T_FATARROW)==0)return(0-1);
else{
}
int body = ast_stmt();
if(body<0)return(0-1);
...
if(arms==0)arms = arm;
else arms = ast_link(arms, arm);
}
if(input_take(T_RBRACE)==0)return(0-1);
else{
}
...
input_pos = (input_pos+1);
if(input_take(T_COMMA)==0)return(0-1);
else{
}
if(input_peek()!=T_ID)return(0-1);
else{
...
if(tuple_ty==0)return(0-1);
else{
}
if(input_take(T_EQUAL)==0)return(0-1);
else{
}
int tuple_value = ast_expr();
if(tuple_value<0)return(0-1);
else{
}
if(input_take(T_SEMI)==0)return(0-1);
else{
...
if(input_take(T_THEN)==0)return(0-1);
else{
}
int yes = ast_stmt();
if(yes<0)return(0-1);
...
int items = 0;
while(input_peek()!=T_RBRACE){
if(input_peek()==T_EOF)return(0-1);
else{
}
int x = ast_stmt();
if(x<0)return(0-1);
...
else items = ast_link(items, item);
}
if(input_take(T_RBRACE)==0)return(0-1);
else{
}
...
if(value!=0)node_pos[return_stmt] = node_pos[value];
else{
}
return return_stmt;
}
...
if(right_compound<0)return(0-1);
else{
}
if(input_take(T_SEMI)==0)return(0-1);
else{
...
int ast_params(void){
int params = 0;
if(input_peek()==T_RPAREN)return 0;
else{
}
while(1==1){
int mode = 0;
...
if(params==0)params = param;
else params = ast_link(params, param);
if(input_take(T_COMMA)==0)return params;
else{
}
}
}
...
if(fields==0)fields = field;
else fields = ast_link(fields, field);
}
if(input_take(T_RBRACE)==0)return(0-1);
else{
}
...
if(params==0)return ast_node(N_STRUCT, fields, 0, 0, name, 0);
else{
}
...
int field_type = ast_type();
if((field_type==0)||(input_take(T_SEMI)==0))return(0-1);
else{
}
int field = ast_node(N_FIELD, field_name, field_type, 0, 0, 0);
if(payload==0)payload = field;
else payload = ast_link(payload, field);
}
if(input_take(T_RBRACE)==0)return(0-1);
else{
}
...
if(values==0)values = item;
else values = ast_link(values, item);
ordinal = (ordinal+1);
if(input_take(T_COMMA)==0){
}
else{
...
input_pos = (input_pos+1);
int ns = raw;
if(ast_namespace_scope!=0)ns = sym_qualified(ast_namespace_scope, raw);
else{
}
if(input_take(T_LBRACE)==0)return(0-1);
else{
...
int ast_decl(void){
if(input_take(T_NAMESPACE)==1)return ast_namespace_decl();
else{
}
...
if(input_take(T_FUNC)==0)return(0-1);
else{
}
if(input_peek()!=T_ID)return(0-1);
else{
...
if(input_take(T_STRUCT)==1)return ast_struct_decl();
else{
}
if(input_take(T_ENUM)==1)return ast_enum_decl();
else{
}
if(input_take(T_LET)==1){
if(input_peek()!=T_ID)return(0-1);
...
if(generic_params<0)return(0-1);
else{
}
...
ast_generic_scope = old_scope;
if(generic_params==0)return ast_node(N_FUNC, body, ret_ty, params, name, node_kind[ret_ty]);
else{
}
...
node_next[p] = 0;
int part = p;
if(node_kind[p]==N_LIST)part = ast_flatten_decl_list(node_a[p]);
else{
}
if(part!=0){
if(head==0){
...
int flat = 0;
if(node_kind[item]==N_LIST)flat = ast_flatten_decl_list(node_a[item]);
else flat = item;
if(flat!=0){
if(items==0)items = flat;
else{
int tail = items;
while(node_next[tail]!=0){
tail = node_next[tail];
...
void ensure_source_file_names(int need){
if(need<source_file_cap)return;
else{
}
...
void ensure_source_file_text(int need){
if(need<source_file_name_text_cap)return;
else{
}
...
int same = 1;
while(i<length){
if(source_file_name_text[(source_file_name_start[id]+i)]!=path[i])same = 0;
else{
}
i = (i+1);
}
...
if(b==length)return base_id;
else{
}
int base_len = 0;
if(base_id>0)base_len = source_file_name_len[base_id];
else{
}
int prefix_len = 0;
int p = base_len;
...
int same = 1;
while(i<prefix_len){
if(source_file_name_text[(source_file_name_start[id]+i)]!=source_file_name_text[(source_file_name_start[base_id]+i)])same = 0;
else{
}
i = (i+1);
}
i = 0;
while(i<raw_len){
if(source_file_name_text[((source_file_name_start[id]+prefix_len)+i)]!=line[(a+i)])same = 0;
else{
}
i = (i+1);
}
...
if(c==10)source_active_line = (source_active_line+1);
else{
}
}
int is_space(int c){
...
int source_peek(void){
if(source_pos<source_len)return source[source_pos];
else{
}
return 0;
}
int source_take(void){
int c = source_peek();
if(source_pos<source_len)source_pos = (source_pos+1);
else{
}
...
if(h>1000000)h = (h-((h/1000000)*1000000));
else{
}
i = (i+1);
}
...
int i = 0;
while(i<length){
if(source[(a+i)]!=source[(b+i)])return 0;
else{
}
i = (i+1);
}
...
if(sym_len[i]==length){
if(sym_hash[i]==h){
if(span_equal(sym_start[i], start, length)==1)return i;
else{
}
}
else{
...
int sym_tag_id(void){
if(sym_tag_name!=0)return sym_tag_name;
else{
}
...
int ast_decl_name(int name){
if(ast_namespace_scope==0)return name;
else{
}
return sym_qualified(ast_namespace_scope, name);
}
int ast_type_name(int name){
if(ast_generic_param(name)==1)return name;
else{
}
if(ast_namespace_scope==0)return name;
else{
...
int bi_lookup(int name){
if(bi_count==0)(void)(bi_init());
else{
}
int i = 0;
while(i<bi_count){
if((sym_len[name]==bi_len[i])&&(sym_hash[name]==sym_hash[bi_name[i]]))return i;
else{
}
i = (i+1);
}
...
(void)(bi_register("memory_alloc_aligned", BI_TC_MEM_ALLOC_ALIGNED, (BI_FLAG_RESERVED+BI_FLAG_OWNED)));
...
(void)(bi_register("basalt_include_open_root", BI_TC_PTR_INT, (BI_FLAG_RESERVED+BI_FLAG_OWNED)));
(void)(bi_register("basalt_include_open_line", BI_TC_PTR_INT, (BI_FLAG_RESERVED+BI_FLAG_OWNED)));
...
(void)(bi_register("basalt_channel_make", BI_TC_CHANNEL_MAKE, (BI_FLAG_RESERVED+BI_FLAG_OWNED)));
...
(void)(bi_register("basalt_thread_spawn", BI_TC_THREAD_SPAWN, (BI_FLAG_RESERVED+BI_FLAG_OWNED)));
...
int h = span_hash(start, length);
if(length==2){
if(h==10084)return L_IF;
else{
}
if(h==9999)return L_FN;
else{
}
if((source[start]==117)&&(source[(start+1)]==56))return L_TU8;
else{
}
if((source[start]==105)&&(source[(start+1)]==56))return L_TI8;
else{
}
}
else{
...
if(length==3){
if(h==315572)return L_LET;
else{
}
if(h==312968)return L_TINT;
else{
}
if(h==310114)return L_FOR;
else{
}
if(((source[start]==117)&&(source[(start+1)]==49))&&(source[(start+2)]==54))return L_TU16;
else{
}
if(((source[start]==102)&&(source[(start+1)]==51))&&(source[(start+2)]==50))return L_TFLOAT;
else{
}
if(((source[start]==102)&&(source[(start+1)]==54))&&(source[(start+2)]==52))return L_TDOUBLE;
else{
}
if(((source[start]==117)&&(source[(start+1)]==51))&&(source[(start+2)]==50))return L_TU32;
else{
}
if(((source[start]==117)&&(source[(start+1)]==54))&&(source[(start+2)]==52))return L_TU64;
else{
}
if(((source[start]==105)&&(source[(start+1)]==49))&&(source[(start+2)]==54))return L_TI16;
else{
}
if(((source[start]==105)&&(source[(start+1)]==51))&&(source[(start+2)]==50))return L_TI32;
else{
}
if(((source[start]==105)&&(source[(start+1)]==54))&&(source[(start+2)]==52))return L_TI64;
else{
}
}
else{
...
if(length==4){
if(h==619275)return L_FUNC;
else{
}
if(h==580992)return L_ELSE;
else{
}
if(h==582984)return L_ENUM;
else{
}
if(h==33685)return L_TRUE;
else{
}
if(h==494385)return L_TBOOL;
else{
}
if(h==90011)return L_TVOID;
else{
}
if(h==23588)return L_THEN;
else{
}
if((((source[start]==109)&&(source[(start+1)]==111))&&(source[(start+2)]==118))&&(source[(start+3)]==101))return L_MOVE;
else{
}
}
else{
...
if(length==5){
if(h==339014)return L_PRINT;
else{
}
if(h==505674)return L_WHILE;
else{
}
if(h==600380)return L_FALSE;
else{
}
if(h==405464)return L_BREAK;
else{
}
if(((((source[start]==100)&&(source[(start+1)]==101))&&(source[(start+2)]==102))&&(source[(start+3)]==101))&&(source[(start+4)]==114))return L_DEFER;
else{
}
if(((((source[start]==109)&&(source[(start+1)]==97))&&(source[(start+2)]==116))&&(source[(start+3)]==99))&&(source[(start+4)]==104))return L_MATCH;
else{
}
}
else{
...
if(length==7){
if(((((((source[start]==112)&&(source[(start+1)]==114))&&(source[(start+2)]==105))&&(source[(start+3)]==110))&&(source[(start+4)]==116))&&(source[(start+5)]==108))&&(source[(start+6)]==110))return L_PRINTLN;
else{
}
}
else{
...
if(source[(start+7)]==101)return L_CONTINUE;
else{
}
}
else{
...
if(length==4){
if((((source[start]==110)&&(source[(start+1)]==117))&&(source[(start+2)]==108))&&(source[(start+3)]==108))return L_NULL;
else{
}
if((((source[start]==99)&&(source[(start+1)]==104))&&(source[(start+2)]==97))&&(source[(start+3)]==114))return L_TCHAR;
else{
}
if((((source[start]==108)&&(source[(start+1)]==111))&&(source[(start+2)]==110))&&(source[(start+3)]==103))return L_TLONG;
else{
}
}
else{
...
if(length==5){
if(((((source[start]==99)&&(source[(start+1)]==111))&&(source[(start+2)]==110))&&(source[(start+3)]==115))&&(source[(start+4)]==116))return L_CONST;
else{
}
if(((((source[start]==97)&&(source[(start+1)]==114))&&(source[(start+2)]==114))&&(source[(start+3)]==97))&&(source[(start+4)]==121))return L_ARRAY;
else{
}
}
else{
...
if(length==6){
if((((((source[start]==98)&&(source[(start+1)]==111))&&(source[(start+2)]==114))&&(source[(start+3)]==114))&&(source[(start+4)]==111))&&(source[(start+5)]==119))return L_BORROW;
else{
}
if(h==448999)return L_EXTERN;
else{
}
if(source[start]==114){
...
if(source[(start+5)]==110)return L_RETURN;
else{
}
}
else{
...
if(source[(start+5)]==116)return L_STRUCT;
else{
}
}
else{
...
if(source[(start+5)]==103)return L_TSTRING;
else{
}
}
else{
...
if(length==5){
if(((((source[start]==102)&&(source[(start+1)]==108))&&(source[(start+2)]==111))&&(source[(start+3)]==97))&&(source[(start+4)]==116))return L_TFLOAT;
else{
}
if(((((source[start]==117)&&(source[(start+1)]==115))&&(source[(start+2)]==105))&&(source[(start+3)]==122))&&(source[(start+4)]==101))return L_TUSIZE;
else{
}
}
else{
...
if(length==7){
if(((((((source[start]==97)&&(source[(start+1)]==108))&&(source[(start+2)]==105))&&(source[(start+3)]==103))&&(source[(start+4)]==110))&&(source[(start+5)]==97))&&(source[(start+6)]==115))return L_ALIGNAS;
else{
}
if(((((((source[start]==99)&&(source[(start+1)]==108))&&(source[(start+2)]==111))&&(source[(start+3)]==115))&&(source[(start+4)]==117))&&(source[(start+5)]==114))&&(source[(start+6)]==101))return L_CLOSURE;
else{
}
}
else{
...
if(length==9){
if(((((((((source[start]==110)&&(source[(start+1)]==97))&&(source[(start+2)]==109))&&(source[(start+3)]==101))&&(source[(start+4)]==115))&&(source[(start+5)]==112))&&(source[(start+6)]==97))&&(source[(start+7)]==99))&&(source[(start+8)]==101))return L_NAMESPACE;
else{
}
}
else{
...
if(length==10){
if((((((((((source[start]==98)&&(source[(start+1)]==111))&&(source[(start+2)]==114))&&(source[(start+3)]==114))&&(source[(start+4)]==111))&&(source[(start+5)]==119))&&(source[(start+6)]==95))&&(source[(start+7)]==109))&&(source[(start+8)]==117))&&(source[(start+9)]==116))return L_BORROW_MUT;
else{
}
}
else{
...
if(length==6){
if((((((source[start]==100)&&(source[(start+1)]==111))&&(source[(start+2)]==117))&&(source[(start+3)]==98))&&(source[(start+4)]==108))&&(source[(start+5)]==101))return L_TDOUBLE;
else{
}
}
else{
...
if(source_pos>(source_len-1))return;
else{
}
(void)(source_take());
}
...
if((tok_kind==L_ID)||(tok_kind==L_ARRAY))tok_value = sym_intern(tok_start, tok_length, tok_kind, 0);
else tok_value = 0;
return tok_kind;
}
else{
...
if(source_peek()==92){
(void)(source_take());
if(source_peek()!=0)(void)(source_take());
else{
}
}
else(void)(source_take());
}
(void)(source_take());
tok_kind = L_STRING;
tok_length = (source_pos-tok_start);
...
(void)(source_take());
if(overflow==0){
if(value>214748364)overflow = 1;
else if((value==214748364)&&(digit>7))overflow = 1;
...
if(overflow==1)tok_value = (0-1);
else tok_value = value;
return tok_kind;
}
else{
}
...
else if(c==40)tok_kind = L_LPAREN;
else if(c==41)tok_kind = L_RPAREN;
else if(c==123)tok_kind = L_LBRACE;
else if(c==125)tok_kind = L_RBRACE;
else if(c==58){
if(source_peek()==58){
(void)(source_take());
tok_kind = L_SCOPE;
}
else tok_kind = L_COLON;
}
else if(c==59)tok_kind = L_SEMI;
else if(c==44)tok_kind = L_COMMA;
else if(c==91)tok_kind = L_LBRACK;
else if(c==93)tok_kind = L_RBRACK;
else if(c==46)tok_kind = L_DOT;
else if(c==60){
if(source_peek()==60){
(void)(source_take());
if(source_peek()==61){
...
else tok_kind = L_BITXOR;
}
else if(c==126)tok_kind = L_BITNOT;
else if(c==61){
if(source_peek()==61){
(void)(source_take());
tok_kind = L_EQEQ;
...
if((line[(p+7)]==32)||(line[(p+7)]==9))mode = 1;
else{
}
}
else{
...
if((line[(p+8)]==32)||(line[(p+8)]==9))mode = 2;
else{
}
}
else{
...
if(child==0){
if(basalt_include_last_status()==1)include_ok = 0;
else{
}
}
else{
...
if(length>0)(void)(include_process_line(line, length));
else{
}
}
void load_source_file(char* path){
...
int map_token(int k){
if(k==L_EOF)return T_EOF;
else{
}
if(k==L_ID)return T_ID;
else{
}
if(k==L_INT)return T_INT;
else{
}
if(k==L_STRING)return T_STRING;
else{
}
if(k==L_FUNC)return T_FUNC;
else{
}
if(k==L_EXTERN)return T_EXTERN;
else{
}
if(k==L_LET)return T_LET;
else{
}
if(k==L_PRINT)return T_PRINT;
else{
}
if(k==L_PRINTLN)return T_PRINTLN;
else{
}
if(k==L_RETURN)return T_RETURN;
else{
}
if(k==L_DEFER)return T_DEFER;
else{
}
if(k==L_MATCH)return T_MATCH;
else{
}
if(k==L_FATARROW)return T_FATARROW;
else{
}
if(k==L_IF)return T_IF;
else{
}
if(k==L_ELSE)return T_ELSE;
else{
}
if(k==L_WHILE)return T_WHILE;
else{
}
if(k==L_FOR)return T_FOR;
else{
}
if(k==L_STRUCT)return T_STRUCT;
else{
}
if(k==L_ENUM)return T_ENUM;
else{
}
if(k==L_BREAK)return T_BREAK;
else{
}
if(k==L_CONTINUE)return T_CONTINUE;
else{
}
if(k==L_TRUE)return T_TRUE;
else{
}
if(k==L_FALSE)return T_FALSE;
else{
}
if(k==L_TINT)return T_TINT;
else{
}
if(k==L_TBOOL)return T_TBOOL;
else{
}
if(k==L_TSTRING)return T_TSTRING;
else{
}
if(k==L_TVOID)return T_TVOID;
else{
}
if(k==L_THEN)return T_THEN;
else{
}
if(k==L_PLUS)return T_PLUS;
else{
}
if(k==L_MINUS)return T_MINUS;
else{
}
if(k==L_STAR)return T_STAR;
else{
}
if(k==L_DIV)return T_DIVIDE;
else{
}
if(k==L_MOD)return T_MOD;
else{
}
if(k==L_TLONG)return T_TLONG;
else{
}
if(k==L_ALIGNAS)return T_ALIGNAS;
else{
}
if(k==L_TU8)return T_TU8;
else{
}
if(k==L_TU16)return T_TU16;
else{
}
if(k==L_TU32)return T_TU32;
else{
}
if(k==L_TU64)return T_TU64;
else{
}
if(k==L_TI8)return T_TI8;
else{
}
if(k==L_TI16)return T_TI16;
else{
}
if(k==L_TI32)return T_TI32;
else{
}
if(k==L_TI64)return T_TI64;
else{
}
if(k==L_TUSIZE)return T_TUSIZE;
else{
}
if(k==L_PLUS_EQ)return T_PLUS_EQ;
else{
}
if(k==L_MINUS_EQ)return T_MINUS_EQ;
else{
}
if(k==L_STAR_EQ)return T_STAR_EQ;
else{
}
if(k==L_DIV_EQ)return T_DIV_EQ;
else{
}
if(k==L_MOD_EQ)return T_MOD_EQ;
else{
}
if(k==L_AMP_EQ)return T_AMP_EQ;
else{
}
if(k==L_BITOR_EQ)return T_BITOR_EQ;
else{
}
if(k==L_BITXOR_EQ)return T_BITXOR_EQ;
else{
}
if(k==L_SHL_EQ)return T_SHL_EQ;
else{
}
if(k==L_SHR_EQ)return T_SHR_EQ;
else{
}
if(k==L_CONCAT)return T_CONCAT;
else{
}
if(k==L_AND)return T_AND_AND;
else{
}
if(k==L_OR)return T_OR_OR;
else{
}
if(k==L_EQ)return T_EQUAL;
else{
}
if(k==L_EQEQ)return T_EQEQ;
else{
}
if(k==L_NEQ)return T_NEQ;
else{
}
if(k==L_LT)return T_LT;
else{
}
if(k==L_GT)return T_GT;
else{
}
if(k==L_LE)return T_LE;
else{
}
if(k==L_GE)return T_GE;
else{
}
if(k==L_NOT)return T_NOT;
else{
}
if(k==L_MOVE)return T_MOVE;
else{
}
if(k==L_BORROW)return T_BORROW;
else{
}
if(k==L_BORROW_MUT)return T_BORROW_MUT;
else{
}
if(k==L_CLOSURE)return T_CLOSURE;
else{
}
if(k==L_COLON)return T_COLON;
else{
}
if(k==L_LPAREN)return T_LPAREN;
else{
}
if(k==L_RPAREN)return T_RPAREN;
else{
}
if(k==L_LBRACE)return T_LBRACE;
else{
}
if(k==L_RBRACE)return T_RBRACE;
else{
}
if(k==L_SEMI)return T_SEMI;
else{
}
if(k==L_COMMA)return T_COMMA;
else{
}
if(k==L_AMP)return T_AMP;
else{
}
if(k==L_LBRACK)return T_LBRACK;
else{
}
if(k==L_RBRACK)return T_RBRACK;
else{
}
if(k==L_DOT)return T_DOT;
else{
}
if(k==L_CHAR)return T_CHAR;
else{
}
if(k==L_NULL)return T_NULL;
else{
}
if(k==L_CONST)return T_CONST;
else{
}
if(k==L_TCHAR)return T_TCHAR;
else{
}
if(k==L_FLOAT)return T_FLOAT;
else{
}
if(k==L_TFLOAT)return T_FLOAT;
else{
}
if(k==L_TDOUBLE)return T_TDOUBLE;
else{
}
if(k==L_BITOR)return T_BITOR;
else{
}
if(k==L_BITXOR)return T_BITXOR;
else{
}
if(k==L_BITNOT)return T_BITNOT;
else{
}
if(k==L_SHL)return T_SHL;
else{
}
if(k==L_SHR)return T_SHR;
else{
}
if(k==L_FN)return T_FN;
else{
}
if(k==L_ARRAY)return T_ARRAY;
else{
}
if(k==L_NAMESPACE)return T_NAMESPACE;
else{
}
if(k==L_SCOPE)return T_SCOPE;
else{
}
return T_EOF;
}
...
if(tc_var_borrow_mut[source_index]>0)tc_var_borrow_mut[source_index] = (tc_var_borrow_mut[source_index]-1);
else if(tc_var_borrow_count[source_index]>0)tc_var_borrow_count[source_index] = (tc_var_borrow_count[source_index]-1);
else{
}
}
else{
...
if(tc_var_borrow_mut[closure_source]>0)tc_var_borrow_mut[closure_source] = (tc_var_borrow_mut[closure_source]-1);
else{
}
}
else{
if(tc_var_borrow_count[closure_source]>0)tc_var_borrow_count[closure_source] = (tc_var_borrow_count[closure_source]-1);
else{
}
}
}
...
tc_ok = 0;
tc_error_code = code;
if(tc_error_pos<0)tc_error_pos = current_source_pos;
else{
}
}
else{
...
if(tc_bind_name[i]==name)return tc_bind_type[i];
else{
}
i = (i+1);
}
...
int tc_is_integer_kind(int kind){
if(((((kind==TY_INT)||(kind==TY_BOOL))||(kind==TY_CHAR))||(kind==TY_LONG))||(kind==TY_LLONG))return 1;
else{
}
if((((kind==TY_U8)||(kind==TY_U16))||(kind==TY_U32))||(kind==TY_U64))return 1;
else{
}
if(((((kind==TY_I8)||(kind==TY_I16))||(kind==TY_I32))||(kind==TY_I64))||(kind==TY_USIZE))return 1;
else{
}
return 0;
}
int tc_is_numeric_kind(int kind){
if(tc_is_integer_kind(kind)==1)return 1;
else{
}
if((kind==TY_FLOAT)||(kind==TY_DOUBLE))return 1;
else{
}
return 0;
}
...
int i = 0;
while(i<n){
if(source[(sym_start[raw]+i)]<limit[i])return 1;
else{
}
if(source[(sym_start[raw]+i)]>limit[i])return 0;
else{
}
i = (i+1);
}
...
int tc_literal_fits(int id, int target_kind){
if(((id==0)||(node_kind[id]!=N_INT))||(node_aux[id]==0))return 1;
else{
}
if(target_kind==TY_INT)return tc_decimal_le(node_aux[id], "2147483647");
else{
}
if(target_kind==TY_U8)return tc_decimal_le(node_aux[id], "255");
else{
}
if(target_kind==TY_U16)return tc_decimal_le(node_aux[id], "65535");
else{
}
if(target_kind==TY_U32)return tc_decimal_le(node_aux[id], "4294967295");
else{
}
if(target_kind==TY_U64)return tc_decimal_le(node_aux[id], "18446744073709551615");
else{
}
if(target_kind==TY_I8)return tc_decimal_le(node_aux[id], "127");
else{
}
if(target_kind==TY_I16)return tc_decimal_le(node_aux[id], "32767");
else{
}
if(target_kind==TY_I32)return tc_decimal_le(node_aux[id], "2147483647");
else{
}
if(target_kind==TY_I64)return tc_decimal_le(node_aux[id], "9223372036854775807");
else{
}
if(target_kind==TY_USIZE)return tc_decimal_le(node_aux[id], "18446744073709551615");
else{
}
return 1;
}
...
if((ak==bk)&&(node_value[a]==node_value[b]))return 1;
else{
}
return 0;
}
...
if((((((((((ak==TY_INT)||(ak==TY_BOOL))||(ak==TY_STRING))||(ak==TY_CHAR))||(ak==TY_FLOAT))||(ak==TY_DOUBLE))||(ak==TY_LONG))||(ak==TY_LLONG))||(ak==TY_VOID))||(tc_is_fixed_integer_kind(ak)==1))return 1;
else{
}
if(ak==TY_NAMED){
if(node_value[a]==node_value[b])return 1;
else{
}
return 0;
}
...
if(ak==TY_VARIANT){
if((node_value[a]==node_value[b])&&(node_aux[a]==node_aux[b]))return 1;
else{
}
return 0;
}
...
if(ak==TY_PTR)return tc_type_equal(node_a[a], node_a[b]);
else{
}
if(ak==TY_ARRAY)return((node_value[a]==node_value[b])&&tc_type_equal(node_a[a], node_a[b]));
else{
}
if(ak==TY_DYN_ARRAY)return tc_type_equal(node_a[a], node_a[b]);
else{
}
if(ak==TY_GENERIC){
if(node_value[a]!=node_value[b])return 0;
else{
}
int x = node_a[a];
int y = node_a[b];
while((x!=0)&&(y!=0)){
if(tc_type_equal(x, y)==0)return 0;
else{
}
x = node_next[x];
y = node_next[y];
...
if(tc_type_equal(node_b[a], node_b[b])==0)return 0;
else{
}
int x = node_a[a];
int y = node_a[b];
...
if(ret!=0)ret = ast_node(node_kind[ret], node_a[ret], node_b[ret], node_c[ret], node_value[ret], node_aux[ret]);
else{
}
return ast_node(TY_FUN, args, ret, 0, 0, 0);
}
...
if(kind==TY_NAMED)return ast_node(TY_NAMED, 0, 0, 0, name, 0);
else{
}
if(kind==TY_VARIANT)return ast_node(TY_VARIANT, 0, 0, 0, name, elem_name);
else{
}
if(kind==TY_PTR)return ast_node(TY_PTR, tc_type_node_from_summary(elem_kind, elem_name, 0, 0), 0, 0, 0, 0);
else{
}
if(kind==TY_DYN_ARRAY)return ast_node(TY_DYN_ARRAY, tc_type_node_from_summary(elem_kind, elem_name, 0, 0), 0, 0, 0, 0);
else{
}
return ast_node(kind, 0, 0, 0, 0, 0);
}
int tc_generic_moves_array(int fun_node){
if(fun_node==0)return 0;
else{
}
int ret = node_b[fun_node];
if((ret!=0)&&(node_kind[ret]==TY_DYN_ARRAY))return 1;
else{
}
int name = node_value[fun_node];
if((name==0)||(sym_len[name]<4))return 0;
else{
}
int s = sym_start[name];
int n = sym_len[name];
if((((source[((s+n)-4)]==102)&&(source[((s+n)-3)]==114))&&(source[((s+n)-2)]==101))&&(source[((s+n)-1)]==101))return 1;
else{
}
return 0;
}
...
if(expected_kind==TY_FLOAT)node_aux[id] = TY_FLOAT;
else if(expected_kind==TY_DOUBLE)node_aux[id] = 0;
else{
}
}
...
if(((node_kind[expr]==N_INT)&&(bound!=0))&&(tc_is_integer_kind(node_kind[bound])==1))return;
else{
}
}
else{
...
if(node_kind[formal]==TY_PARAM){
if(tc_bind_add(node_value[formal], actual)==0)return;
else{
}
return;
}
...
if(tc_type_equal(formal, actual)==0)(void)(tc_fail_types(12, node_kind[formal], node_kind[actual]));
else{
}
}
int tc_substitute_type(int ty){
...
if(b!=0)return tc_substitute_type(b);
else{
}
return ast_node(TY_PARAM, node_a[ty], node_b[ty], node_c[ty], node_value[ty], node_aux[ty]);
}
...
if(node_kind[ty]==TY_PTR)return ast_node(TY_PTR, tc_substitute_type(node_a[ty]), 0, 0, 0, 0);
else{
}
if(node_kind[ty]==TY_ARRAY)return ast_node(TY_ARRAY, tc_substitute_type(node_a[ty]), 0, 0, node_value[ty], 0);
else{
}
if(node_kind[ty]==TY_DYN_ARRAY)return ast_node(TY_DYN_ARRAY, tc_substitute_type(node_a[ty]), 0, 0, 0, 0);
else{
}
if(node_kind[ty]==TY_GENERIC){
int args = 0;
...
int tc_same(int a_kind, int a_name, int b_kind, int b_name){
if((a_kind==TY_PTR)&&(b_kind==TY_PTR))return 1;
else{
}
if(a_kind==b_kind){
if(a_kind==TY_NAMED){
...
if((tc_is_integer_kind(a_kind)==1)&&(tc_is_integer_kind(b_kind)==1))return 1;
else{
}
if((a_kind==TY_VOID)&&(b_kind==TY_INT))return 1;
else{
}
if((a_kind==TY_INT)&&(b_kind==TY_VOID))return 1;
else{
}
return 0;
}
int tc_array_elem_same(int a_kind, int a_name, int b_kind, int b_name){
if((tc_is_legacy_integer_kind(a_kind)==1)&&(tc_is_legacy_integer_kind(b_kind)==1))return 1;
else{
}
if(a_kind!=b_kind)return 0;
else{
}
if(a_kind==TY_NAMED){
if(a_name==b_name)return 1;
...
if((a_kind==TY_PARAM)&&(b_kind==TY_PARAM))return 1;
else{
}
return 1;
}
...
if((a_kind==TY_GENERIC)&&(b_kind==TY_GENERIC))return tc_type_equal(a_name, b_name);
else{
}
return 0;
}
...
if((a_kind==TY_TUPLE)&&(b_kind==TY_TUPLE))return tc_type_equal(a_name, b_name);
else{
}
return 0;
}
...
if((a_kind==TY_CLOSURE)&&(b_kind==TY_CLOSURE))return tc_type_equal(a_name, b_name);
else{
}
return 0;
}
...
if((a_elem_kind==TY_VOID)||(b_elem_kind==TY_VOID))return 1;
else{
}
if(a_elem_kind!=b_elem_kind)return 0;
else{
}
if((a_elem_kind==TY_NAMED)&&(a_elem_name!=b_elem_name))return 0;
else{
}
return 1;
}
...
if(((a_kind==TY_CHAR)&&((((b_kind==TY_INT)||(b_kind==TY_BOOL))||(b_kind==TY_LONG))||(b_kind==TY_LLONG)))||((b_kind==TY_CHAR)&&((((a_kind==TY_INT)||(a_kind==TY_BOOL))||(a_kind==TY_LONG))||(a_kind==TY_LLONG))))return 1;
else{
}
if(((a_kind==TY_VOID)&&(b_kind==TY_INT))||((a_kind==TY_INT)&&(b_kind==TY_VOID)))return 1;
else{
}
return 0;
}
int tc_ptr_diff_ok(int a_kind, int a_elem_kind, int a_elem_name, int b_kind, int b_elem_kind, int b_elem_name){
if((a_kind!=TY_PTR)||(b_kind!=TY_PTR))return 0;
else{
}
if((a_elem_kind==TY_VOID)||(b_elem_kind==TY_VOID))return 0;
else{
}
if(a_elem_kind!=b_elem_kind)return 0;
else{
...
if(source[(sym_start[full]+i)]!=source[(sym_start[base]+i)])return 0;
else{
}
i = (i+1);
}
...
if(sym_len[full]<(sym_len[base]+3))return 0;
else{
}
int start = ((sym_start[full]+sym_len[full])-sym_len[base]);
if((source[(start-1)]!=58)||(source[(start-2)]!=58))return 0;
else{
}
int j = 0;
while(j<sym_len[base]){
if(source[(start+j)]!=source[(sym_start[base]+j)])return 0;
else{
}
j = (j+1);
}
...
while(item!=0){
if(((node_kind[item]==N_STRUCT)||(node_kind[item]==N_GENERIC_STRUCT))&&(node_value[item]==name))return item;
else{
}
item = node_next[item];
}
...
while(broad!=0){
if(((node_kind[broad]==N_STRUCT)||(node_kind[broad]==N_GENERIC_STRUCT))&&(sym_suffix_equal(node_value[broad], name)==1))return broad;
else{
}
...
int item = node_a[tc_root];
while(item!=0){
if((((node_kind[item]==N_STRUCT)||(node_kind[item]==N_GENERIC_STRUCT))&&(node_scope[item]==ns))&&(sym_suffix_equal(node_value[item], name)==1))return item;
else{
}
item = node_next[item];
}
...
int item = node_a[tc_root];
while(item!=0){
if((node_kind[item]==N_ENUM)&&(node_value[item]==name))return item;
else{
}
item = node_next[item];
}
...
int broad = node_a[tc_root];
while(broad!=0){
if((node_kind[broad]==N_ENUM)&&(sym_suffix_equal(node_value[broad], name)==1))return broad;
else{
}
broad = node_next[broad];
}
...
int item = node_a[tc_root];
while(item!=0){
if(((node_kind[item]==N_ENUM)&&(node_scope[item]==ns))&&(sym_suffix_equal(node_value[item], name)==1))return item;
else{
}
item = node_next[item];
}
...
int tc_generic_arity(int decl){
if((decl==0)||(node_kind[decl]!=N_GENERIC_STRUCT))return 0;
else{
}
int n = 0;
int p = node_c[decl];
...
int tc_generic_arg_count(int ty){
if((ty==0)||(node_kind[ty]!=TY_GENERIC))return 0;
else{
}
int n = 0;
int p = node_a[ty];
...
int tc_named_exists_ctx(int name, int ns){
if(tc_find_struct_ctx(name, ns)!=0)return 1;
else{
}
if(tc_find_enum_ctx(name, ns)!=0)return 1;
else{
}
return 0;
}
int tc_named_exists(int name){
if(tc_find_struct(name)!=0)return 1;
else{
}
if(tc_find_enum(name)!=0)return 1;
else{
}
return 0;
}
...
int k = node_kind[ty];
if(k==TY_NAMED){
if(tc_named_exists_ctx(node_value[ty], node_scope[ty])==0)(void)(tc_fail(2));
else{
}
}
else if(k==TY_GENERIC){
int s = tc_find_struct_ctx(node_value[ty], node_scope[ty]);
if((s==0)||(node_kind[s]!=N_GENERIC_STRUCT))(void)(tc_fail(2));
else{
if(tc_generic_arity(s)!=tc_generic_arg_count(ty))(void)(tc_fail(37));
else{
}
int a = node_a[ty];
while(a!=0){
...
else if(k==TY_PTR)(void)(tc_check_type(node_a[ty]));
else if(k==TY_ARRAY)(void)(tc_check_type(node_a[ty]));
else if(k==TY_DYN_ARRAY)(void)(tc_check_type(node_a[ty]));
else if(k==TY_TUPLE){
int item = node_a[ty];
while(item!=0){
...
if(tc_path_name[i]==name)return 1;
else{
}
i = (i+1);
}
...
int bad = 0;
while(f!=0){
if(tc_cycle_type(node_b[f])==1)bad = 1;
else{
}
f = node_next[f];
}
...
if(node_kind[ty]==TY_PTR)return 0;
else{
}
if(node_kind[ty]==TY_FUN)return 0;
else{
}
if(node_kind[ty]==TY_ARRAY)return tc_cycle_type(node_a[ty]);
else{
}
if(node_kind[ty]==TY_DYN_ARRAY)return 0;
else{
}
if(node_kind[ty]==TY_NAMED)return tc_cycle_struct(node_value[ty]);
else{
}
if(node_kind[ty]==TY_GENERIC)return 0;
else{
}
return 0;
}
int tc_release_name(int name){
if(bi_has_flag(name, BI_FLAG_CONSUME)==1)return 1;
else{
}
return 0;
}
...
if(node_kind[id]==N_CLOSURE)return 1;
else{
}
if(node_kind[id]==N_MOVE)return 1;
else{
}
if(node_kind[id]!=N_CALL)return 0;
else{
}
if(bi_has_flag(node_value[id], BI_FLAG_OWNED)==1)return 1;
else{
}
return 0;
}
int tc_is_owner_kind(int kind){
if(kind==TY_CLOSURE)return 1;
else{
}
if(kind==TY_DYN_ARRAY)return 1;
else{
}
if(kind==TY_GENERIC)return 1;
else{
}
return 0;
}
...
if(tc_is_owner_kind(node_kind[ty])==1)return 1;
else{
}
return 0;
}
...
if((index<tc_var_count)&&((tc_var_borrow_count[index]>0)||(tc_var_borrow_mut[index]>0)))return 1;
else{
}
return 0;
}
...
void tc_move_value(int id){
if((id==0)||(node_kind[id]!=N_VAR))return;
else{
}
...
if(tc_is_owner_type(tc_last_var_type)==1)(void)(tc_move_var(tc_last_var_index));
else(void)(tc_fail(58));
}
void tc_check_call_borrow(int arg, int mode){
if(mode==0)return;
else{
...
if(mode==1){
if((arg!=0)&&(node_kind[arg]==N_MOVE))return;
else{
}
(void)(tc_fail(40));
return;
...
if((arg!=0)&&(node_kind[arg]==N_VAR)){
if(tc_lookup_var(node_value[arg])==1)source_index = tc_last_var_index;
else{
}
}
else{
...
if(mode==2){
if(tc_var_borrow_mut[source_index]>0)(void)(tc_fail(37));
else{
}
}
else if(mode==3){
if((tc_var_borrow_count[source_index]>0)||(tc_var_borrow_mut[source_index]>0))(void)(tc_fail(37));
else{
}
}
else{
...
if(source_index<tc_global_count)return;
else{
}
if((source_index<tc_var_count)&&(tc_var_param[source_index]==1))return;
else{
}
(void)(tc_fail(38));
}
void tc_record_borrow(int destination, int source_index2){
if((destination<0)||(source_index2<0))return;
else{
}
...
if(tc_var_mode[tc_last_var_index]==2)(void)(tc_fail(37));
else if(tc_borrow_conflict(tc_last_var_index)==1)(void)(tc_fail(37));
else{
}
}
else{
...
else if(((node_kind[id]==N_DEREF)||(node_kind[id]==N_INDEX))||(node_kind[id]==N_FIELD_ACCESS)){
...
(void)(tc_expr(id));
if(tc_expr_borrow_source<0)return;
else{
}
(void)(tc_fail(37));
}
...
void tc_consume_call(int id){
if(((id==0)||(node_kind[id]!=N_CALL))||(tc_release_name(node_value[id])==0))return;
else{
}
int arg = node_a[id];
if((arg==0)||(node_kind[arg]!=N_VAR))return;
else{
}
...
void tc_add_var(int name, int kind, int named, int elem_kind, int elem_name, int type_node){
int begin = 0;
if(tc_scope_count>0)begin = tc_scope_start[(tc_scope_count-1)];
else{
}
int i = begin;
while(i<tc_var_count){
...
if(tc_elem_kind==TY_NAMED)tc_elem_name = node_value[node_a[tc_last_var_type]];
else{
}
}
else{
...
else if((((tc_kind==TY_GENERIC)||(tc_kind==TY_TUPLE))||(tc_kind==TY_CLOSURE))||(tc_kind==TY_PARAM)){
tc_name = ty;
}
else{
tc_name = 0;
...
if((a==b)&&(tc_is_fixed_integer_kind(a)==1))return a;
else{
}
if((a==TY_USIZE)||(b==TY_USIZE))return TY_USIZE;
else{
}
if((a==TY_U64)||(b==TY_U64))return TY_U64;
else{
}
if((a==TY_I64)||(b==TY_I64))return TY_I64;
else{
}
if((a==TY_U32)||(b==TY_U32))return TY_U32;
else{
}
if((a==TY_I32)||(b==TY_I32))return TY_I32;
else{
}
if((a==TY_U16)||(b==TY_U16))return TY_U16;
else{
}
if((a==TY_I16)||(b==TY_I16))return TY_I16;
else{
}
if((a==TY_U8)||(b==TY_U8))return TY_U8;
else{
}
if((a==TY_I8)||(b==TY_I8))return TY_I8;
else{
}
if((a==TY_LLONG)||(b==TY_LLONG))return TY_LLONG;
else{
}
...
int tc_check_variant(int id){
if(tc_find_enum_variant(node_value[id])==0)return 0;
else{
}
int arg = node_a[id];
...
if(tc_literal_fits(arg, fk)==0)(void)(tc_fail(54));
else{
}
...
if(destination>=tc_var_count)return;
else{
}
...
if(node_aux[cap]==3)tc_var_borrow_mut[cap_source] = (tc_var_borrow_mut[cap_source]+1);
else if(node_aux[cap]==2)tc_var_borrow_count[cap_source] = (tc_var_borrow_count[cap_source]+1);
else{
}
}
else{
...
tc_expr_owner_source = (0-1);
tc_expr_is_owned = 0;
if((id!=0)&&(tc_ok==1))tc_error_pos = node_pos[id];
else{
}
if(id==0){
(void)(tc_fail(4));
...
if(node_kind[node_a[id]]==N_VAR)(void)(tc_move_value(node_a[id]));
else if(tc_expr_borrow_source>=0)(void)(tc_fail(38));
else if(tc_is_owner_kind(moved_kind)==0)(void)(tc_fail(35));
else{
}
tc_expr_borrow_source = (0-1);
tc_expr_owner_source = (0-1);
...
if(tc_lookup_var(cap_name)==0)(void)(tc_fail(5));
else{
...
if(node_aux[cap]==1){
if(tc_is_owner_type(source_type)==0)(void)(tc_fail(58));
else if(tc_var_moved[source_index]==1)(void)(tc_fail(61));
else{
(void)(tc_move_var(source_index));
if(tc_var_moved[source_index]==1)tc_var_closure_moved[source_index] = 1;
else{
}
}
}
else if(node_aux[cap]==2){
if(tc_var_moved[source_index]==1)(void)(tc_fail(33));
else if(tc_var_borrow_mut[source_index]>0)(void)(tc_fail(37));
else{
}
}
else if(node_aux[cap]==3){
if(tc_var_moved[source_index]==1)(void)(tc_fail(33));
else if((tc_var_borrow_count[source_index]>0)||(tc_var_borrow_mut[source_index]>0))(void)(tc_fail(37));
else{
}
}
else{
...
(void)(tc_add_var(node_a[body_cap], body_cap_kind, body_cap_name, body_cap_elem_kind, body_cap_elem_name, body_cap_type));
tc_var_mode[tc_last_var_index] = node_aux[body_cap];
if(node_aux[body_cap]==1)tc_var_owned[tc_last_var_index] = 1;
else{
tc_var_borrow_source[tc_last_var_index] = node_c[body_cap];
if(node_aux[body_cap]==3)tc_var_borrow_mut[tc_last_var_index] = 1;
else tc_var_borrow_count[tc_last_var_index] = 1;
}
...
if(node_aux[p]==1)tc_var_owned[tc_last_var_index] = 1;
else if(node_aux[p]==3)tc_var_borrow_mut[tc_last_var_index] = 1;
else if(tc_is_owner_kind(pk)==1)tc_var_owned[tc_last_var_index] = 1;
else{
}
p = node_next[p];
}
...
int src = node_b[sp];
int q = ast_node(node_kind[src], node_a[src], node_b[src], node_c[src], node_value[src], node_aux[src]);
if(sig_args==0)sig_args = q;
else sig_args = ast_link(sig_args, q);
sp = node_next[sp];
}
...
if(elem_ty==0)elem_ty = tc_type_node_from_summary(tc_kind, tc_name, tc_elem_kind, tc_elem_name);
else{
}
if(elem_ty==0){
(void)(tc_fail(19));
...
if(types==0)types = elem_ty;
else types = ast_link(types, elem_ty);
item = node_next[item];
}
...
if(tc_lookup_var(node_value[id])==1){
if(tc_last_var_moved==1){
if(tc_var_closure_moved[tc_last_var_index]==1)(void)(tc_fail(61));
else(void)(tc_fail(33));
}
else{
}
tc_expr_borrow_source = tc_var_borrow_source[tc_last_var_index];
if(tc_last_var_owned==1)tc_expr_owner_source = tc_last_var_index;
else{
}
tc_expr_is_owned = 0;
tc_result_type = tc_last_var_type;
if(node_type[id]==0)node_type[id] = tc_last_var_type;
else{
}
...
if(k==N_ADDRESS){
int address_entry = 0;
if(node_kind[node_a[id]]==N_VAR)address_entry = tc_find_function_ctx(node_value[node_a[id]], node_scope[node_a[id]]);
else{
}
if(address_entry!=0){
tc_kind = TY_FUN;
...
if((node_kind[node_a[id]]==N_VAR)&&(tc_lookup_var(node_value[node_a[id]])==1))tc_expr_borrow_source = tc_last_var_index;
else{
}
tc_kind = TY_PTR;
tc_name = oldn;
...
if(tc_kind!=TY_PTR)(void)(tc_fail(6));
else{
...
if(tc_kind!=TY_INT)(void)(tc_fail(7));
else{
}
(void)(tc_expr(node_a[id]));
...
else if(((((node_kind[ix]==N_BINOP)&&(node_value[ix]==OP_SUB))&&(node_kind[node_a[ix]]==N_INT))&&(node_value[node_a[ix]]==0))&&(node_kind[node_b[ix]]==N_INT)){
ik = (0-node_value[node_b[ix]]);
is_const = 1;
}
else{
}
if((((is_const==1)&&(tc_result_type!=0))&&(node_kind[tc_result_type]==TY_ARRAY))&&((ik<0)||(ik>(node_value[tc_result_type]-1))))(void)(tc_fail(45));
else{
}
tc_kind = TY_INT;
tc_name = 0;
...
while((gp!=0)&&(ga!=0)){
if(tc_bind_add(node_a[gp], ga)==0)return;
else{
}
gp = node_next[gp];
ga = node_next[ga];
...
int call_name = node_value[id];
int btag = bi_tag(call_name);
if(bi_has_flag(call_name, BI_FLAG_OWNED)==1)tc_expr_is_owned = 1;
else{
}
...
if(witness_ty==0)witness_ty = tc_type_node_from_summary(wk, wn, tc_elem_kind, tc_elem_name);
else{
}
tc_kind = TY_PTR;
tc_name = 0;
...
int aa = node_a[id];
if(((((((aa==0)||(node_next[aa]==0))||(node_next[node_next[aa]]==0))||(node_next[node_next[node_next[aa]]]==0))||(node_next[node_next[node_next[node_next[aa]]]]==0))||(node_next[node_next[node_next[node_next[node_next[aa]]]]]==0))||(node_next[node_next[node_next[node_next[node_next[node_next[aa]]]]]]!=0)){
(void)(tc_fail(13));
return;
}
else{
...
if(btag==BI_TC_SYS_STRING)tc_kind = TY_STRING;
else tc_kind = TY_INT;
tc_name = 0;
tc_elem_kind = 0;
tc_elem_name = 0;
if(btag==BI_TC_SYS_STRING)tc_result_type = ast_node(TY_STRING, 0, 0, 0, 0, 0);
...
int ep = node_c[entry];
if(((((ep!=0)&&(node_next[ep]==0))&&(node_kind[node_b[ep]]==TY_PTR))&&(node_a[node_b[ep]]!=0))&&(node_kind[node_a[node_b[ep]]]==TY_VOID))callback_ok = 1;
else{
}
}
else{
...
if(((((ep!=0)&&(node_next[ep]==0))&&(node_kind[ep]==TY_PTR))&&(node_a[ep]!=0))&&(node_kind[node_a[ep]]==TY_VOID))callback_ok = 1;
else{
}
}
else{
...
if(tc_literal_fits(arg_fp, pek_fp)==0)(void)(tc_fail(54));
else{
}
if((pek_fp==TY_DYN_ARRAY)&&(ak_fp==TY_DYN_ARRAY))(void)(tc_move_value(arg_fp));
else{
}
if(tc_same_full(ak_fp, an_fp, aek_fp, aen_fp, pek_fp, pen_fp, peek_fp, peen_fp)==0)(void)(tc_fail_types(12, pek_fp, ak_fp));
else{
}
arg_fp = node_next[arg_fp];
p_fp = node_next[p_fp];
}
if((arg_fp!=0)||(p_fp!=0))(void)(tc_fail(13));
else{
}
...
if((bound_arg!=0)&&(node_kind[bound_arg]==TY_FLOAT))(void)(tc_mark_float_expr(ga, TY_FLOAT));
else if((bound_arg!=0)&&(node_kind[bound_arg]==TY_DOUBLE))(void)(tc_mark_float_expr(ga, TY_DOUBLE));
else{
}
}
else{
...
if(((generic_moves_array==1)&&(actual_kind==TY_DYN_ARRAY))&&(node_aux[gp]==0))(void)(tc_move_value(ga));
else{
}
if(actual_ty==0)actual_ty = tc_type_node_from_summary(tc_kind, tc_name, tc_elem_kind, tc_elem_name);
else{
}
...
if(tc_literal_fits(arg, pek)==0)(void)(tc_fail(54));
else{
}
if(((pek==TY_DYN_ARRAY)&&(ak==TY_DYN_ARRAY))&&(node_aux[p]==0))(void)(tc_move_value(arg));
else{
}
if(tc_same_full(ak, an, aek, aen, pek, pen, peek, peen)==0)(void)(tc_fail_types(12, pek, ak));
else{
}
if((ak==TY_FUN)&&(pek==TY_FUN)){
if(actual_type==0)(void)(tc_fail_types(12, TY_FUN, 0));
else{
}
if(formal_type==0)(void)(tc_fail_types(12, 0, TY_FUN));
else{
}
if(((actual_type!=0)&&(formal_type!=0))&&(tc_type_equal(actual_type, formal_type)==0))(void)(tc_fail_types(12, node_kind[formal_type], node_kind[actual_type]));
else{
}
}
else{
...
if(k==N_INDIRECT_CALL){
(void)(tc_expr(node_a[id]));
if(((tc_kind==TY_CLOSURE)&&(tc_result_type!=0))&&(node_kind[tc_result_type]==TY_CLOSURE)){
}
else if(((tc_kind!=TY_FUN)||(tc_result_type==0))||(node_kind[tc_result_type]!=TY_FUN)){
(void)(tc_fail_types(12, TY_FUN, tc_kind));
return;
}
else{
...
int aek = tc_elem_kind;
int aen = tc_elem_name;
if(tc_literal_fits(arg, pk)==0)(void)(tc_fail(54));
else{
}
if(tc_same_full(ak, an, aek, aen, pk, pn, pek, pen)==0)(void)(tc_fail_types(12, pk, ak));
else{
}
arg = node_next[arg];
param = node_next[param];
...
if(node_value[id]==OP_CONCAT){
if((ak!=TY_STRING)||(bk!=TY_STRING))(void)(tc_fail(14));
else{
}
tc_kind = TY_STRING;
tc_name = 0;
...
if((tc_is_integer_kind(ak)==0)||(tc_is_integer_kind(bk)==0))(void)(tc_fail(15));
else{
}
tc_kind = TY_BOOL;
tc_name = 0;
}
else if(((((node_value[id]==OP_BITAND)||(node_value[id]==OP_BITOR))||(node_value[id]==OP_BITXOR))||(node_value[id]==OP_SHL))||(node_value[id]==OP_SHR)){
if((tc_is_integer_kind(ak)==0)||(tc_is_integer_kind(bk)==0))(void)(tc_fail(32));
else{
}
...
int null_cmp = 0;
if((((ak==TY_PTR)&&(bk==TY_INT))&&(node_kind[node_b[id]]==N_INT))&&(node_value[node_b[id]]==0))null_cmp = 1;
else{
}
if((((ak==TY_INT)&&(bk==TY_PTR))&&(node_kind[node_a[id]]==N_INT))&&(node_value[node_a[id]]==0))null_cmp = 1;
else{
}
if((tc_same_full(ak, an, ae, aen, bk, bn, be, ben)==0)&&(null_cmp==0))(void)(tc_fail(16));
else{
}
tc_kind = TY_BOOL;
tc_name = 0;
}
else if((((node_value[id]==OP_LT)||(node_value[id]==OP_GT))||(node_value[id]==OP_LE))||(node_value[id]==OP_GE)){
if((tc_is_numeric_kind(ak)==0)||(tc_is_numeric_kind(bk)==0))(void)(tc_fail(17));
else{
}
tc_kind = TY_BOOL;
tc_name = 0;
...
else if(((node_value[id]==OP_MUL)||(node_value[id]==OP_DIV))||(node_value[id]==OP_MOD)){
if((tc_is_numeric_kind(ak)==0)||(tc_is_numeric_kind(bk)==0))(void)(tc_fail(18));
else{
}
tc_kind = tc_numeric_result_kind(ak, bk);
tc_name = 0;
...
int item = node_a[tc_root];
while(item!=0){
if((((node_kind[item]==N_FUNC)||(node_kind[item]==N_GENERIC_FUNC))||(node_kind[item]==N_EXTERN))&&(node_value[item]==name))return item;
else{
}
item = node_next[item];
}
...
int item = node_a[tc_root];
while(item!=0){
if(((((node_kind[item]==N_FUNC)||(node_kind[item]==N_GENERIC_FUNC))||(node_kind[item]==N_EXTERN))&&(node_scope[item]==ns))&&(sym_suffix_equal(node_value[item], name)==1))return item;
else{
}
item = node_next[item];
}
...
int f = node_a[item];
while(f!=0){
if(node_a[f]==name)return node_value[item];
else{
}
f = node_next[f];
}
...
int tc_match_enum_decl(int ty){
if((ty==0)||(node_kind[ty]!=TY_NAMED))return 0;
else{
}
int decl = tc_find_enum_ctx(node_value[ty], node_scope[ty]);
if(decl==0)decl = tc_find_enum(node_value[ty]);
else{
}
return decl;
}
int tc_match_variant_member(int decl, int name){
if((decl==0)||(node_kind[decl]!=N_ENUM))return 0;
else{
}
int field = node_a[decl];
while(field!=0){
if(node_a[field]==name)return field;
else{
}
int qualified = sym_qualified(node_value[decl], node_a[field]);
if((qualified==name)||(sym_suffix_equal(qualified, name)==1))return field;
else{
}
field = node_next[field];
}
...
int arm = head;
while(arm!=0){
if(node_aux[arm]==member)return 1;
else{
}
arm = node_next[arm];
}
...
if((field!=0)||(binding!=0))(void)(tc_fail(50));
else{
}
}
int tc_emit_field_type(int id){
if((id==0)||(node_kind[id]!=N_FIELD_ACCESS))return 0;
else{
}
...
if(node_a[variant_field]==node_value[id])return gen_substitute_type(node_b[variant_field]);
else{
}
...
else if(node_kind[base_ty]==TY_NAMED)struct_name = node_value[base_ty];
else return 0;
...
(void)(tc_bind_clear());
while((gp!=0)&&(ga!=0)){
if(tc_bind_add(node_a[gp], ga)==0)return 0;
else{
}
gp = node_next[gp];
ga = node_next[ga];
...
int f = node_a[decl];
while(f!=0){
if(node_a[f]==node_value[id])return tc_substitute_type(node_b[f]);
else{
}
f = node_next[f];
}
...
if(gen_closure_serial[closure_scan]==closure_serial_value)return gen_closure_sig[closure_scan];
else{
}
...
if(((node_kind[id]==N_FIELD_ACCESS)&&(node_aux[id]!=0))&&(node_kind[node_aux[id]]==TY_VARIANT))return node_aux[id];
else{
}
if(node_kind[id]==N_FIELD_ACCESS){
int field_ty = tc_emit_field_type(id);
if(field_ty!=0)return field_ty;
else{
}
}
else{
...
if((callee_type!=0)&&((node_kind[callee_type]==TY_FUN)||(node_kind[callee_type]==TY_CLOSURE)))return gen_substitute_type(node_b[callee_type]);
else{
}
}
else{
...
if((call_param_type!=0)&&((node_kind[call_param_type]==TY_FUN)||(node_kind[call_param_type]==TY_CLOSURE)))return gen_substitute_type(node_b[call_param_type]);
else{
}
}
else{
...
if(node_kind[id]==N_VAR){
int formal_type = gen_active_param_type(node_value[id]);
if(formal_type!=0)return gen_substitute_type(formal_type);
else{
}
int declared_type = gen_active_local_decl_type(node_value[id]);
if(declared_type!=0)return gen_substitute_type(declared_type);
else{
}
if(node_type[id]!=0){
int stable_local_type = node_type[id];
int resolved_stable_type = gen_substitute_type(stable_local_type);
if(resolved_stable_type!=0)return resolved_stable_type;
else{
}
...
if(resolved_local_type!=0)return resolved_local_type;
else{
}
return local_type;
}
...
if(node_kind[id]==N_STRING)return ast_node(TY_STRING, 0, 0, 0, 0, 0);
else{
}
if(node_kind[id]==N_INT)return ast_node(TY_INT, 0, 0, 0, 0, 0);
else{
}
if(node_kind[id]==N_BOOL)return ast_node(TY_BOOL, 0, 0, 0, 0, 0);
else{
}
if(node_kind[id]==N_CHAR)return ast_node(TY_CHAR, 0, 0, 0, 0, 0);
else{
}
if(node_kind[id]==N_FLOAT){
if(node_aux[id]==TY_FLOAT)return ast_node(TY_FLOAT, 0, 0, 0, 0, 0);
else{
}
...
if(node_kind[id]==N_NULL)return ast_node(TY_PTR, ast_node(TY_VOID, 0, 0, 0, 0, 0), 0, 0, 0, 0);
else{
}
if(node_kind[id]==N_TUPLE){
if(node_aux[id]!=0)return gen_substitute_type(node_aux[id]);
else{
}
(void)(tc_expr(id));
...
int tc_expr_kind_for_emit(int id){
if((id!=0)&&(node_kind[id]==N_CLOSURE))return TY_CLOSURE;
else{
}
if(((id!=0)&&(node_kind[id]==N_CALL))&&(node_aux[id]!=0))return node_kind[node_aux[id]];
else{
}
if((id!=0)&&(node_kind[id]==N_INDIRECT_CALL)){
int indirect_result_type = tc_emit_arg_type(id);
if(indirect_result_type!=0)return node_kind[indirect_result_type];
else{
}
return TY_INT;
}
...
int call_param_type = gen_active_param_type(node_value[id]);
if((call_param_type!=0)&&((node_kind[call_param_type]==TY_FUN)||(node_kind[call_param_type]==TY_CLOSURE))){
int call_result_type = gen_substitute_type(node_b[call_param_type]);
if(call_result_type!=0)return node_kind[call_result_type];
else{
}
}
else{
...
if(tc_literal_fits(node_c[id], ck)==0)(void)(tc_fail(54));
else{
}
if(tc_same_full(ck, cn, ce, cen, tc_kind, tc_name, tc_elem_kind, tc_elem_name)==0){
if((node_kind[node_c[id]]!=N_INT)||(node_value[node_c[id]]!=0))if(node_kind[node_c[id]]!=N_NULL)(void)(tc_fail(30));
else{
}
else{
}
...
if((tc_is_owner_kind(dk)==1)&&(node_kind[node_c[id]]==N_VAR))(void)(tc_fail(40));
else{
}
if(tc_literal_fits(node_c[id], dk)==0)(void)(tc_fail(54));
else{
}
if(tc_same_full(dk, dn, de, den, ek, en, ee, een)==0){
if(((node_kind[node_c[id]]!=N_INT)||(node_value[node_c[id]]!=0))&&(node_kind[node_c[id]]!=N_NULL))(void)(tc_fail_types(20, dk, ek));
else{
}
}
else{
}
if((dk==TY_DYN_ARRAY)&&(node_kind[node_c[id]]==N_VAR))(void)(tc_move_value(node_c[id]));
else{
}
(void)(tc_add_var(node_a[id], dk, dn, de, den, node_b[id]));
if((tc_is_owner_kind(dk)==1)||(tc_owned_initializer(node_c[id])==1))tc_var_owned[tc_last_var_index] = 1;
else{
}
if((dk==TY_CLOSURE)&&(node_kind[node_c[id]]==N_CLOSURE))(void)(tc_attach_closure_caps(tc_last_var_index, node_a[node_c[id]]));
else{
}
if(dk==TY_PTR){
if(rhs_borrow<0){
}
else(void)(tc_record_borrow(tc_last_var_index, rhs_borrow));
}
else{
}
}
else if((k==N_ASSIGN)||(k==N_COMPOUND_ASSIGN)){
if((node_kind[node_a[id]]==N_VAR)&&(sym_type[node_value[node_a[id]]]>100))(void)(tc_fail(31));
else{
}
(void)(tc_expr(node_a[id]));
int lk = tc_kind;
...
if((lk==TY_DYN_ARRAY)&&(node_kind[node_b[id]]==N_VAR))(void)(tc_fail(40));
else{
}
if(tc_literal_fits(node_b[id], lk)==0)(void)(tc_fail(54));
else{
}
if(tc_same_full(lk, ln, le, len, tc_kind, tc_name, tc_elem_kind, tc_elem_name)==0){
if(((node_kind[node_b[id]]!=N_INT)||(node_value[node_b[id]]!=0))&&(node_kind[node_b[id]]!=N_NULL))(void)(tc_fail_types(21, lk, tc_kind));
else{
}
}
else{
...
if(tc_var_borrow_mut[lhs_borrow]>0)tc_var_borrow_mut[lhs_borrow] = (tc_var_borrow_mut[lhs_borrow]-1);
else if(tc_var_borrow_count[lhs_borrow]>0)tc_var_borrow_count[lhs_borrow] = (tc_var_borrow_count[lhs_borrow]-1);
else{
}
}
else{
...
else(void)(tc_record_borrow(lhs_index, rhs_borrow_assign));
}
else{
}
}
...
(void)(tc_expr(node_a[id]));
if(tc_kind!=TY_VOID)(void)(tc_fail(46));
else{
}
}
else if(k==N_TUPLE_BIND){
...
if((declared_kind!=TY_TUPLE)||(tc_kind!=TY_TUPLE))(void)(tc_fail(52));
else if(((declared_ty==0)||(rhs_ty==0))||(tc_type_equal(declared_ty, rhs_ty)==0))(void)(tc_fail_types(20, declared_kind, tc_kind));
else{
...
if((elem!=0)||(binding!=0))(void)(tc_fail(53));
else{
}
}
}
...
int enum_decl = 0;
if((subject_kind==TY_NAMED)&&(subject_type!=0))enum_decl = tc_match_enum_decl(subject_type);
else{
}
if(enum_decl==0)(void)(tc_fail(47));
else{
int arm = node_b[id];
while(arm!=0){
int member = tc_match_variant_member(enum_decl, node_value[arm]);
if(member==0)(void)(tc_fail(48));
else{
if(tc_match_seen_variant(node_b[id], member)==1)(void)(tc_fail(49));
else{
}
node_aux[arm] = member;
(void)(tc_enter_scope());
...
while(variant!=0){
if(tc_match_seen_variant(node_b[id], variant)==0)(void)(tc_fail(51));
else{
}
...
else if((k==N_PRINT)||(k==N_PRINTLN))(void)(tc_expr(node_a[id]));
else if(k==N_EXPR){
(void)(tc_expr(node_a[id]));
if(node_kind[node_a[id]]==N_CALL)(void)(tc_consume_call(node_a[id]));
else{
}
}
else if(k==N_RETURN){
if(node_a[id]==0){
if(expected_kind!=TY_VOID)(void)(tc_fail(22));
else{
}
}
else{
...
if(tc_literal_fits(node_a[id], expected_kind)==0)(void)(tc_fail(54));
else{
}
if(expected_kind==TY_PTR)(void)(tc_check_return_escape(return_borrow));
else{
}
if(expected_kind==TY_CLOSURE)(void)(tc_check_closure_value_escape(node_a[id]));
else{
}
if(tc_same_full(expected_kind, expected_name, tc_expected_elem_kind, tc_expected_elem_name, tc_kind, tc_name, tc_elem_kind, tc_elem_name)==0)if(node_kind[node_a[id]]!=N_NULL)(void)(tc_fail_types(23, expected_kind, tc_kind));
else{
}
else{
}
...
if(tc_loop_depth==0)(void)(tc_fail(24));
else{
}
}
else if(k==N_BLOCK){
...
(void)(tc_expr(node_a[id]));
if(((tc_is_numeric_kind(tc_kind)==0)&&(tc_kind!=TY_PTR))&&(tc_kind!=TY_FUN))(void)(tc_fail(25));
else{
}
...
(void)(tc_expr(node_a[id]));
if(((tc_is_numeric_kind(tc_kind)==0)&&(tc_kind!=TY_PTR))&&(tc_kind!=TY_FUN))(void)(tc_fail(26));
else{
}
...
(void)(tc_enter_scope());
if(node_a[id]!=0)(void)(tc_stmt(node_a[id], expected_kind, expected_name));
else{
}
(void)(tc_expr(node_b[id]));
if(((tc_is_numeric_kind(tc_kind)==0)&&(tc_kind!=TY_PTR))&&(tc_kind!=TY_FUN))(void)(tc_fail(27));
else{
}
tc_loop_depth = (tc_loop_depth+1);
(void)(tc_stmt(node_c[id], expected_kind, expected_name));
tc_loop_depth = (tc_loop_depth-1);
if(node_value[id]!=0)(void)(tc_stmt(node_value[id], expected_kind, expected_name));
else{
}
(void)(tc_leave_scope());
}
...
if(pos<source_len)return source_file_at[pos];
else{
}
if(source_len>0)return source_file_at[(source_len-1)];
else{
}
return source_active_file;
}
void tc_print_source_byte(int value){
if(value==9)(void)(runtime_write_char(9));
else if(value==10)(void)(runtime_write_char(10));
else if(value==13)(void)(runtime_write_char(13));
else if(value>31){
if(value<127)(void)(runtime_write_char(tc_diag_ascii[(value-32)]));
else(void)(runtime_write_char(63));
}
else(void)(runtime_write_char(63));
}
void tc_print_source_file(int file_id){
if(file_id<1)(void)(runtime_write_string("<unknown>"));
else if(file_id>(source_file_count-1))(void)(runtime_write_string("<unknown>"));
else{
int i = 0;
...
if(begin>source_len)begin = source_len;
else{
}
...
void tc_print_type_kind(int kind){
if(kind==TY_INT)(void)(runtime_write_string("int"));
else if(kind==TY_BOOL)(void)(runtime_write_string("bool"));
else if(kind==TY_STRING)(void)(runtime_write_string("string"));
else if(kind==TY_VOID)(void)(runtime_write_string("void"));
else if(kind==TY_PTR)(void)(runtime_write_string("pointer"));
else if(kind==TY_ARRAY)(void)(runtime_write_string("array"));
else if(kind==TY_DYN_ARRAY)(void)(runtime_write_string("Array<T>"));
else if(kind==TY_NAMED)(void)(runtime_write_string("named type"));
else if(kind==TY_CHAR)(void)(runtime_write_string("char"));
else if(kind==TY_FLOAT)(void)(runtime_write_string("f32"));
else if(kind==TY_DOUBLE)(void)(runtime_write_string("f64"));
else if(kind==TY_FUN)(void)(runtime_write_string("function"));
else if(kind==TY_PARAM)(void)(runtime_write_string("type parameter"));
else if(kind==TY_GENERIC)(void)(runtime_write_string("generic type"));
else if(kind==TY_LONG)(void)(runtime_write_string("long"));
else if(kind==TY_LLONG)(void)(runtime_write_string("long long"));
else if(kind==TY_VARIANT)(void)(runtime_write_string("enum variant"));
else if(kind==TY_U8)(void)(runtime_write_string("u8"));
else if(kind==TY_U16)(void)(runtime_write_string("u16"));
else if(kind==TY_U32)(void)(runtime_write_string("u32"));
else if(kind==TY_U64)(void)(runtime_write_string("u64"));
else if(kind==TY_I8)(void)(runtime_write_string("i8"));
else if(kind==TY_I16)(void)(runtime_write_string("i16"));
else if(kind==TY_I32)(void)(runtime_write_string("i32"));
else if(kind==TY_I64)(void)(runtime_write_string("i64"));
else if(kind==TY_USIZE)(void)(runtime_write_string("usize"));
else if(kind==TY_TUPLE)(void)(runtime_write_string("tuple"));
else(void)(runtime_write_string("unknown type"));
}
int tc_diag_has_types(int code){
if(code==12)return 1;
else{
...
void tc_print_hint(int code){
if(code==12)(void)(runtime_write_string("check each argument against its parameter type"));
else if(code==20)(void)(runtime_write_string("make the initializer expression match the declared type"));
else if(code==21)(void)(runtime_write_string("make the right-hand side match the left-hand side type"));
else if(code==23)(void)(runtime_write_string("return a value matching the function return type"));
else if(code==36)(void)(runtime_write_string("use an element type matching the existing array"));
else if(code==55)(void)(runtime_write_string("use only scalar, pointer, fixed-array, or named struct types in extern parameters"));
else if(code==56)(void)(runtime_write_string("use only scalar, pointer, fixed-array, or named struct types in extern return type"));
else if(code==57)(void)(runtime_write_string("header path must contain only alphanumeric characters, '.', '/', '_', '-'"));
else if(code==58)(void)(runtime_write_string("move only an owned binding or an owned container result"));
else if(code==59)(void)(runtime_write_string("pass a tracked variable or address expression to a borrow parameter"));
else if(code==60)(void)(runtime_write_string("keep a borrowed capture within the source binding's lexical lifetime"));
else if(code==61)(void)(runtime_write_string("closure moved-capture used after move"));
else if(code==31)(void)(runtime_write_string("assign only to a mutable binding"));
else(void)(runtime_write_string("inspect the expression at the reported source location"));
}
void tc_diag(void){
if(tc_error_code==3)(void)(runtime_write_string("type error: duplicate declaration"));
else if(tc_error_code==5)(void)(runtime_write_string("type error: unknown name"));
else if(tc_error_code==12)(void)(runtime_write_string("type error: invalid function arguments"));
else if(tc_error_code==13)(void)(runtime_write_string("type error: invalid argument count"));
else if(tc_error_code==14)(void)(runtime_write_string("type error: string concatenation requires strings"));
else if(tc_error_code==17)(void)(runtime_write_string("type error: invalid built-in argument type"));
else if(tc_error_code==18)(void)(runtime_write_string("type error: invalid arithmetic operands"));
else if(tc_error_code==20)(void)(runtime_write_string("type error: initializer type mismatch"));
else if(tc_error_code==21)(void)(runtime_write_string("type error: assignment type mismatch"));
else if(tc_error_code==23)(void)(runtime_write_string("type error: return type mismatch"));
else if(tc_error_code==28)(void)(runtime_write_string("type error: recursive struct definition"));
else if(tc_error_code==31)(void)(runtime_write_string("type error: assignment to const"));
else if(tc_error_code==33)(void)(runtime_write_string("type error: use after ownership move"));
else if(tc_error_code==34)(void)(runtime_write_string("type error: use after ownership move"));
else if(tc_error_code==35)(void)(runtime_write_string("type error: release requires an owned value"));
else if(tc_error_code==36)(void)(runtime_write_string("type error: array element type mismatch"));
else if(tc_error_code==37)(void)(runtime_write_string("type error: cannot mutate or move while borrowed"));
else if(tc_error_code==38)(void)(runtime_write_string("type error: borrowed reference escapes its owner"));
else if(tc_error_code==40)(void)(runtime_write_string("type error: owned value copy requires an explicit move"));
else if(tc_error_code==41)(void)(runtime_write_string("type error: unknown function"));
else if(tc_error_code==42)(void)(runtime_write_string("type error: cannot call non-function value"));
else if(tc_error_code==43)(void)(runtime_write_string("type error: function name is reserved by the C runtime"));
else if(tc_error_code==45)(void)(runtime_write_string("type error: array index out of bounds"));
else if(tc_error_code==46)(void)(runtime_write_string("type error: defer expression must return void"));
else if(tc_error_code==47)(void)(runtime_write_string("type error: match subject must be an enum"));
else if(tc_error_code==48)(void)(runtime_write_string("type error: unknown variant in match arm"));
else if(tc_error_code==49)(void)(runtime_write_string("type error: duplicate variant in match"));
else if(tc_error_code==50)(void)(runtime_write_string("type error: match arm payload arity mismatch"));
else if(tc_error_code==51)(void)(runtime_write_string("type error: match is not exhaustive"));
else if(tc_error_code==52)(void)(runtime_write_string("type error: tuple destructuring requires tuple type"));
else if(tc_error_code==53)(void)(runtime_write_string("type error: tuple binding count mismatch"));
else if(tc_error_code==54)(void)(runtime_write_string("type error: integer literal is out of range for its target type"));
else if(tc_error_code==55)(void)(runtime_write_string("type error: extern parameter type is not FFI-safe"));
else if(tc_error_code==56)(void)(runtime_write_string("type error: extern return type is not FFI-safe"));
else if(tc_error_code==57)(void)(runtime_write_string("type error: extern header path contains invalid characters"));
else if(tc_error_code==58)(void)(runtime_write_string("type error: explicit move requires an owned value"));
else if(tc_error_code==59)(void)(runtime_write_string("type error: borrow parameter requires a tracked source"));
else if(tc_error_code==60)(void)(runtime_write_string("type error: closure capture escapes its source lifetime"));
else if(tc_error_code==61)(void)(runtime_write_string("type error: closure moved-capture used after move"));
else if(tc_error_code==44)(void)(runtime_write_string("type error: distinct functions collide after C mangling"));
else(void)(runtime_write_string("type error: invalid expression"));
(void)(runtime_write_char(10));
...
int tc_reserved_function(int name){
if(bi_has_flag(name, BI_FLAG_RESERVED)==1)return 1;
else{
}
return 0;
}
...
int k = node_kind[ty];
if((((k==TY_INT)||(k==TY_BOOL))||(k==TY_CHAR))||(k==TY_STRING))return 1;
else{
}
if(((k==TY_VOID)||(k==TY_FLOAT))||(k==TY_DOUBLE))return 1;
else{
}
if((k==TY_LONG)||(k==TY_LLONG))return 1;
else{
}
if(tc_is_fixed_integer_kind(k)==1)return 1;
else{
}
if(k==TY_PTR)return 1;
else{
}
if(k==TY_ARRAY)return 1;
else{
}
if(k==TY_NAMED)return 1;
else{
}
return 0;
}
...
if(tc_cycle_struct(node_value[item])==1)(void)(tc_fail(28));
else{
}
}
else{
...
if(node_kind[item]==N_CONST)sym_type[node_a[item]] = (gk+100);
else{
}
(void)(tc_mark_float_expr(node_c[item], gk));
(void)(tc_expr(node_c[item]));
if(tc_literal_fits(node_c[item], gk)==0)(void)(tc_fail(54));
else{
}
if(((tc_same_full(gk, gn, ge, gen, tc_kind, tc_name, tc_elem_kind, tc_elem_name)==0)&&((node_kind[node_c[item]]!=N_INT)||(node_value[node_c[item]]!=0)))&&(node_kind[node_c[item]]!=N_NULL))(void)(tc_fail(29));
else{
}
}
else{
...
if(input_peek()!=T_EOF)parsed = 0;
else{
}
if(include_ok==0)parsed = 0;
else{
}
if(parsed==1){
if(tc_program(root)==0)parsed = 0;
else{
}
}
else{
...
(void)(code_reset());
(void)(gen_program(root));
if(code_count!=stable_count)parsed = 0;
else{
}
}
else{
...
(void)(runtime_write_char(10));
(void)(runtime_write_string("diagnostic.hint=check the syntax near the reported source location\n"));
(void)(runtime_write_string("diagnostic.excerpt="));
...
else(void)(write_char(out, c));
i = (i+1);
}
}
void emit_source_line(int*out, int pos){
if((pos<0)||(pos>(source_len-1)))return;
else{
}
int file_id = source_file_at[pos];
if((file_id<1)||(file_id>(source_file_count-1)))return;
else{
}
...
if((((((kind==C_KW)||(kind==C_IDENT))||(kind==C_INT))||(kind==C_STRING))||(kind==C_RAW))||(kind==C_RAW_U64))(void)(write_char(out, 32));
else{
}
...
if(value==(0-1001))(void)(write_string(out, "printf"));
else if(value==(0-1002))(void)(write_string(out, "runtime_string_concat"));
else if(value==(0-1011))(void)(write_string(out, "sizeof"));
else if(value==(0-1012))(void)(write_string(out, "len"));
else if(value==(0-1013))(void)(write_string(out, "cap"));
else if(value==(0-1015))(void)(write_string(out, "data"));
else if(value==(0-1016))(void)(write_string(out, "basalt_memory_alloc"));
else if(value==(0-1017))(void)(write_string(out, "basalt_memory_resize"));
else if(value==(0-1018))(void)(write_string(out, "basalt_memory_free"));
else if(value==(0-1019))(void)(write_string(out, "basalt_memory_alloc_aligned"));
else if(value==(0-1020))(void)(write_string(out, "_Alignas"));
else if(value==(0-1021))(void)(write_string(out, "f"));
else(void)(emit_symbol(out, value));
}
else if(kind==C_INT)(void)(emit_int_text(out, value));
else if(kind==C_RAW)(void)(emit_symbol(out, value));
else if(kind==C_RAW_U64){
(void)(emit_symbol(out, value));
(void)(write_string(out, "ULL"));
}
else if(kind==C_STRING)(void)(emit_string(out, value));
else if(kind==C_OP){
if(value==1)(void)(write_string(out, "+"));
else if(value==2)(void)(write_string(out, "-"));
else if(value==3)(void)(write_string(out, "*"));
else if(value==4)(void)(write_string(out, "/"));
else if(value==5)(void)(write_string(out, "=="));
else if(value==6)(void)(write_string(out, "!="));
else if(value==7)(void)(write_string(out, "<"));
else if(value==8)(void)(write_string(out, ">"));
else if(value==9)(void)(write_string(out, "&&"));
else if(value==10)(void)(write_string(out, "||"));
else if(value==11)(void)(write_string(out, "++"));
else if(value==12)(void)(write_string(out, "&"));
else if(value==13)(void)(write_string(out, "|"));
else if(value==14)(void)(write_string(out, "^"));
else if(value==15)(void)(write_string(out, "<<"));
else if(value==16)(void)(write_string(out, ">>"));
else if(value==17)(void)(write_string(out, "%"));
else if(value==18)(void)(write_string(out, ":"));
else if(value==19)(void)(write_string(out, "+="));
else if(value==20)(void)(write_string(out, "-="));
else if(value==21)(void)(write_string(out, "*="));
else if(value==22)(void)(write_string(out, "/="));
else if(value==23)(void)(write_string(out, "%="));
else if(value==24)(void)(write_string(out, "&="));
else if(value==25)(void)(write_string(out, "|="));
else if(value==26)(void)(write_string(out, "^="));
else if(value==27)(void)(write_string(out, "<<="));
else if(value==28)(void)(write_string(out, ">>="));
else if(value==29)(void)(write_string(out, "!"));
else if(value==30)(void)(write_string(out, "<="));
else if(value==31)(void)(write_string(out, ">="));
else{
}
}
...
else if(value==15)(void)(emit_print_prefix(out));
else if(value==16){
...
else if(value==19)(void)(write_string(out, "{0}"));
else if(value==22)(void)(write_char(out, 32));
else if(value==23){
(void)(write_char(out, 40));
(void)(write_char(out, 34));
(void)(write_string(out, "%d"));
...
else if(kind==C_NEWLINE)(void)(write_char(out, 10));
else{
}
}
void emit_runtime(int*out){
(void)(write_string(out, "#if defined(_WIN32)\n#include <direct.h>\n#else\n#define _POSIX_C_SOURCE 200809L\n#define _XOPEN_SOURCE 700\n#endif\n#include <stdio.h>\n#include <stdlib.h>\n#include <string.h>\n#include <stdint.h>\n#include <stddef.h>\n#include <limits.h>\n#include <errno.h>\n#include <stdatomic.h>\n#if defined(_WIN32)\n#include <windows.h>\n#include <process.h>\n#else\n#include <unistd.h>\n#include <fcntl.h>\n#include <poll.h>\n#include <sys/types.h>\n#include <sys/wait.h>\n#endif\n#if defined(_WIN32) && defined(__MINGW32__) && !defined(BASALT_USE_NATIVE_C11_THREADS)\n#include <pthread.h>\n#include <sched.h>\ntypedef pthread_t thrd_t;\ntypedef int (*basalt_thrd_start_t)(void*);\ntypedef struct basalt_thrd_start_context { basalt_thrd_start_t entry; void* argument; int result; } basalt_thrd_start_context;\nstatic void* basalt_thrd_start(void* raw){basalt_thrd_start_context* context=(basalt_thrd_start_context*)raw;context->result=context->entry(context->argument);return raw;}\nstatic int basalt_thrd_create(thrd_t* thread,basalt_thrd_start_t entry,void* argument){basalt_thrd_start_context* context;int status;context=(basalt_thrd_start_context*)malloc(sizeof(*context));if(!context)return 1;context->entry=entry;context->argument=argument;context->result=0;status=pthread_create(thread,NULL,basalt_thrd_start,context);if(status!=0)free(context);return status;}\nstatic int basalt_thrd_join(thrd_t thread,int* result){void* raw=NULL;int status=pthread_join(thread,&raw);if(status==0){basalt_thrd_start_context* context=(basalt_thrd_start_context*)raw;if(result)*result=context->result;free(context);}return status;}\n#define thrd_success 0\n#define thrd_create(thread,entry,argument) basalt_thrd_create((thread),(entry),(argument))\n#define thrd_join(thread,result) basalt_thrd_join((thread),(result))\n#define thrd_yield() sched_yield()\n#else\n#include <threads.h>\n#endif\n#if defined(_WIN32) && defined(__MINGW32__)\n#ifndef BASALT_ALIGNED_ALLOC_DECLARED\n#define BASALT_ALIGNED_ALLOC_DECLARED 1\nvoid *aligned_alloc(size_t alignment,size_t size);\n#endif\n#endif\n#if defined(__GNUC__) || defined(__clang__)\n#define BASALT_UNUSED __attribute__((unused))\n#else\n#define BASALT_UNUSED\n#endif\nstatic void basalt_panic(int code){(void)code;exit(2);}\nstatic size_t basalt_checked_bytes(int count,size_t elem_size){if(count<0)basalt_panic(1);if(elem_size!=0&&(size_t)count>(size_t)-1/elem_size)basalt_panic(1);return(size_t)count*elem_size;}\nstatic void* basalt_track(void*);static void basalt_release(void*);\n"));
(void)(write_string(out, "typedef struct basalt_atomic_int { atomic_int value; } basalt_atomic_int;\nstatic BASALT_UNUSED void* basalt_atomic_make(int initial){basalt_atomic_int*a=(basalt_atomic_int*)calloc(1,sizeof(*a));if(!a)basalt_panic(5);atomic_init(&a->value,initial);return basalt_track(a);}\nstatic BASALT_UNUSED int basalt_atomic_load(void*p){basalt_atomic_int*a=(basalt_atomic_int*)p;if(!a)basalt_panic(4);return atomic_load_explicit(&a->value,memory_order_acquire);}\nstatic BASALT_UNUSED void basalt_atomic_store(void*p,int value){basalt_atomic_int*a=(basalt_atomic_int*)p;if(!a)basalt_panic(4);atomic_store_explicit(&a->value,value,memory_order_release);}\nstatic BASALT_UNUSED int basalt_atomic_fetch_add(void*p,int delta){basalt_atomic_int*a=(basalt_atomic_int*)p;if(!a)basalt_panic(4);return atomic_fetch_add_explicit(&a->value,delta,memory_order_acq_rel);}\nstatic BASALT_UNUSED int basalt_atomic_compare_exchange(void*p,int expected,int desired){basalt_atomic_int*a=(basalt_atomic_int*)p;int old;if(!a)basalt_panic(4);old=expected;return atomic_compare_exchange_strong_explicit(&a->value,&old,desired,memory_order_acq_rel,memory_order_acquire);}\nstatic BASALT_UNUSED void basalt_atomic_free(void*p){basalt_release(p);}\ntypedef struct basalt_channel { _Atomic size_t head; _Atomic size_t tail; atomic_int closed; size_t capacity; int data[]; } basalt_channel;\nstatic BASALT_UNUSED void* basalt_channel_make(int requested){size_t cap=2;size_t bytes;basalt_channel*c;if(requested<1||requested>1073741824)basalt_panic(7);while(cap<(size_t)requested){if(cap>(size_t)-1/2)basalt_panic(1);cap*=2;}if(cap>(size_t)-1/sizeof(int))basalt_panic(1);bytes=sizeof(*c)+cap*sizeof(int);if(bytes<sizeof(*c))basalt_panic(1);c=(basalt_channel*)calloc(1,bytes);if(!c)basalt_panic(5);c->capacity=cap;atomic_init(&c->head,0);atomic_init(&c->tail,0);atomic_init(&c->closed,0);return basalt_track(c);}\nstatic BASALT_UNUSED int basalt_channel_send(void*p,int value){basalt_channel*c=(basalt_channel*)p;size_t head,tail;if(!c)basalt_panic(4);if(atomic_load_explicit(&c->closed,memory_order_acquire)!=0)return -1;head=atomic_load_explicit(&c->head,memory_order_relaxed);tail=atomic_load_explicit(&c->tail,memory_order_acquire);if(head-tail>=c->capacity)return 0;c->data[head&(c->capacity-1)]=value;atomic_store_explicit(&c->head,head+1,memory_order_release);return 1;}\nstatic BASALT_UNUSED int basalt_channel_recv(void*p,int*out){basalt_channel*c=(basalt_channel*)p;size_t head,tail;if(!c||!out)basalt_panic(4);tail=atomic_load_explicit(&c->tail,memory_order_relaxed);head=atomic_load_explicit(&c->head,memory_order_acquire);if(tail==head){if(atomic_load_explicit(&c->closed,memory_order_acquire)!=0)return -1;return 0;}*out=c->data[tail&(c->capacity-1)];atomic_store_explicit(&c->tail,tail+1,memory_order_release);return 1;}\nstatic BASALT_UNUSED void basalt_channel_close(void*p){basalt_channel*c=(basalt_channel*)p;if(!c)basalt_panic(4);atomic_store_explicit(&c->closed,1,memory_order_release);}\nstatic BASALT_UNUSED void basalt_channel_free(void*p){basalt_release(p);}\ntypedef struct basalt_thread_handle { thrd_t thread; } basalt_thread_handle;\nstatic BASALT_UNUSED void* basalt_thread_spawn(int(*entry)(void*),void*arg){basalt_thread_handle*h=(basalt_thread_handle*)calloc(1,sizeof(*h));if(!h)basalt_panic(5);if(thrd_create(&h->thread,entry,arg)!=thrd_success){free(h);return NULL;}return basalt_track(h);}\nstatic BASALT_UNUSED int basalt_thread_join(void*p){basalt_thread_handle*h=(basalt_thread_handle*)p;int result;if(!h)basalt_panic(4);if(thrd_join(h->thread,&result)!=thrd_success)basalt_panic(8);basalt_release(h);return result;}\nstatic BASALT_UNUSED void basalt_thread_yield(void){thrd_yield();}\n"));
(void)(write_string(out, "static char** basalt_inc_active=NULL;static size_t basalt_inc_active_n=0,basalt_inc_active_cap=0;static char** basalt_inc_loaded=NULL;static size_t basalt_inc_loaded_n=0,basalt_inc_loaded_cap=0;static int basalt_inc_status=0;\n"));
(void)(write_string(out, "static BASALT_UNUSED int basalt_inc_eq(const char*a,const char*b){return strcmp(a,b)==0;}\n"));
(void)(write_string(out, "static BASALT_UNUSED size_t basalt_inc_find(char**v,size_t n,const char*p){size_t i;for(i=0;i<n;i++)if(basalt_inc_eq(v[i],p))return i;return (size_t)-1;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void basalt_inc_add(char***vp,size_t*np,size_t*cp,char*p){size_t c;char**q;if(*np==*cp){c=*cp?*cp*2:16;q=(char**)realloc(*vp,c*sizeof(char*));if(!q)exit(2);*vp=q;*cp=c;}(*vp)[(*np)++]=p;}\n"));
(void)(write_string(out, "static BASALT_UNUSED char* basalt_inc_strdup(const char*p){size_t n=strlen(p);char*q=(char*)malloc(n+1);if(!q)exit(2);memcpy(q,p,n+1);return(char*)basalt_track(q);}\n"));
(void)(write_string(out, "static BASALT_UNUSED char* basalt_inc_realpath(const char*p){if(p&&p[0]==0&&basalt_inc_active_n)return basalt_inc_active[basalt_inc_active_n-1];\n#if defined(_WIN32)\nchar*q=_fullpath(NULL,p,0);if(q)return(char*)basalt_track(q);\n#else\nchar*q=realpath(p,NULL);if(q)return(char*)basalt_track(q);\n#endif\nreturn basalt_inc_strdup(p);}\n"));
(void)(write_string(out, "static BASALT_UNUSED int basalt_inc_begin(char*p){if(basalt_inc_find(basalt_inc_active,basalt_inc_active_n,p)!=(size_t)-1){basalt_inc_status=1;return 0;}if(basalt_inc_find(basalt_inc_loaded,basalt_inc_loaded_n,p)!=(size_t)-1){basalt_inc_status=2;return 0;}basalt_inc_add(&basalt_inc_active,&basalt_inc_active_n,&basalt_inc_active_cap,p);basalt_inc_status=0;return 1;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void basalt_include_close(void){if(basalt_inc_active_n){char*p=basalt_inc_active[--basalt_inc_active_n];if(basalt_inc_find(basalt_inc_loaded,basalt_inc_loaded_n,p)==(size_t)-1)basalt_inc_add(&basalt_inc_loaded,&basalt_inc_loaded_n,&basalt_inc_loaded_cap,p);}}\n"));
(void)(write_string(out, "static BASALT_UNUSED char* basalt_inc_join(const char*base,const char*raw){const char*s=strrchr(base,'/');size_t n=s?(size_t)(s-base+1):0;size_t m=strlen(raw);char*q=(char*)malloc(n+m+1);if(!q)exit(2);if(n)memcpy(q,base,n);memcpy(q+n,raw,m+1);return(char*)basalt_track(q);}\n"));
(void)(write_string(out, "static BASALT_UNUSED int basalt_include_line_mode(int*line,int n){int i=0,j;while(i<n&&(line[i]==' '||line[i]==9))i++;if(i+7<=n&&line[i]=='i'&&line[i+1]=='n'&&line[i+2]=='c'&&line[i+3]=='l'&&line[i+4]=='u'&&line[i+5]=='d'&&line[i+6]=='e'){j=i+7;while(j<n&&(line[j]==' '||line[j]==9))j++;if(j<n&&line[j]==34)return 1;}if(i+8<=n&&line[i]=='i'&&line[i+1]=='n'&&line[i+2]=='c'&&line[i+3]=='l'&&line[i+4]=='u'&&line[i+5]=='d'&&line[i+6]=='e'&&line[i+7]=='c'){j=i+8;while(j<n&&(line[j]==' '||line[j]==9))j++;if(j<n&&line[j]==34)return 2;}return 0;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void* basalt_include_open_root(const char*path){char*p=basalt_inc_realpath(path);FILE*f;if(!basalt_inc_begin(p))return NULL;f=fopen(p,(const char[]){114,0});if(!f){basalt_inc_status=3;basalt_include_close();return NULL;}return(void*)f;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void* basalt_include_open_line(int*line,int n,int mode){int i=0,a,b,j;char*raw,*joined,*canon;FILE*f;(void)mode;while(i<n&&line[i]!=34)i++;if(i>=n)return NULL;a=++i;while(i<n&&line[i]!=34)i++;if(i>=n)return NULL;b=i;raw=(char*)malloc((size_t)(b-a)+1);if(!raw)exit(2);{int k;for(k=0;k<b-a;k++)raw[k]=(char)line[a+k];}raw[b-a]=0;raw=(char*)basalt_track(raw);j=i+1;while(j<n&&(line[j]==32||line[j]==9))j++;if(j<n&&line[j]==59)j++;while(j<n&&(line[j]==32||line[j]==9))j++;if(j!=n)return NULL;joined=basalt_inc_join(basalt_inc_active[basalt_inc_active_n-1],raw);canon=basalt_inc_realpath(joined);if(!basalt_inc_begin(canon))return NULL;f=fopen(canon,(const char[]){114,0});if(!f){basalt_inc_status=3;basalt_include_close();return NULL;}return(void*)f;}\n"));
(void)(write_string(out, "static BASALT_UNUSED int basalt_include_last_status(void){return basalt_inc_status;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void basalt_include_reset_session(void){basalt_inc_active_n=0;basalt_inc_loaded_n=0;basalt_inc_status=0;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void* open_file(const char* p,const char* m){return (void*)fopen(p,m);}\n"));
(void)(write_string(out, "static BASALT_UNUSED int read_char(void* h){int c=fgetc((FILE*)h);return c==EOF?-1:c;}\n"));
(void)(write_string(out, "static BASALT_UNUSED int close_file(void* h){return fclose((FILE*)h);}\n"));
(void)(write_string(out, "static BASALT_UNUSED int write_char(void* h,int c){return fputc(c,(FILE*)h);}\n"));
(void)(write_string(out, "static BASALT_UNUSED int write_string(void* h,const char* s){return fputs(s,(FILE*)h);}\n"));
(void)(write_string(out, "static void** basalt_live=NULL;static size_t basalt_live_n=0,basalt_live_cap=0;\n"));
(void)(write_string(out, "static BASALT_UNUSED size_t basalt_find(void* p){size_t i;for(i=0;i<basalt_live_n;i++)if(basalt_live[i]==p)return i;return (size_t)-1;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void basalt_validate(void){size_t i,j;for(i=0;i<basalt_live_n;i++){if(!basalt_live[i])basalt_panic(2);for(j=i+1;j<basalt_live_n;j++)if(basalt_live[i]==basalt_live[j])basalt_panic(2);}}\n"));
(void)(write_string(out, "static BASALT_UNUSED void basalt_cleanup(void){size_t i;basalt_validate();for(i=0;i<basalt_live_n;i++)free(basalt_live[i]);free(basalt_live);basalt_live=NULL;basalt_live_n=basalt_live_cap=0;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void* basalt_track(void* p){size_t c;void**q;if(!p)return NULL;if(basalt_find(p)!=(size_t)-1)basalt_panic(2);if(basalt_live_n==basalt_live_cap){if(basalt_live_cap>(size_t)-1/2)c=(size_t)-1;else c=basalt_live_cap?basalt_live_cap*2:32;if(c>(size_t)-1/sizeof(void*))basalt_panic(2);q=(void**)realloc(basalt_live,c*sizeof(void*));if(!q)basalt_panic(2);basalt_live=q;basalt_live_cap=c;}basalt_live[basalt_live_n++]=p;atexit(basalt_cleanup);return p;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void basalt_release(void* p){size_t i;if(!p)return;i=basalt_find(p);if(i==(size_t)-1)basalt_panic(2);free(p);basalt_live[i]=basalt_live[--basalt_live_n];}\n"));
(void)(write_string(out, "static int basalt_io_status=0;\n"));
(void)(write_string(out, "static BASALT_UNUSED int runtime_io_status(void){return basalt_io_status;}\n"));
(void)(write_string(out, "static BASALT_UNUSED char* runtime_read_line(int max_len){size_t n=0;int c=EOF;char*p;if(max_len<2||max_len>1048576)basalt_panic(7);p=(char*)malloc((size_t)max_len);if(!p)basalt_panic(5);while(n+1<(size_t)max_len){c=fgetc(stdin);if(c==EOF)break;if(c=='\\n')break;p[n++]=(char)c;}p[n]=0;if(c!=EOF&&c!='\\n'&&n+1==(size_t)max_len){basalt_io_status=3;do{c=fgetc(stdin);}while(c!=EOF&&c!='\\n');}else if(c==EOF&&n==0)basalt_io_status=1;else basalt_io_status=0;return(char*)basalt_track(p);}\n"));
(void)(write_string(out, "static BASALT_UNUSED int runtime_read_int(int fallback){char buf[128];size_t n=0;int c=EOF;char*end;long v;while(n+1<sizeof(buf)){c=fgetc(stdin);if(c==EOF||c=='\\n')break;if(c!='\\r')buf[n++]=(char)c;}buf[n]=0;if(c!=EOF&&c!='\\n'&&n+1==sizeof(buf)){basalt_io_status=3;do{c=fgetc(stdin);}while(c!=EOF&&c!='\\n');return fallback;}if(c==EOF&&n==0){basalt_io_status=1;return fallback;}errno=0;v=strtol(buf,&end,10);while(*end==' '||*end=='\\t'||*end=='\\r')end++;if(end==buf||*end!=0){basalt_io_status=2;return fallback;}if(errno==ERANGE||v<(long)INT_MIN||v>(long)INT_MAX){basalt_io_status=4;return fallback;}basalt_io_status=0;return(int)v;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void runtime_write_string(const char*s){if(!s)basalt_panic(4);if(fputs(s,stdout)==EOF||fflush(stdout)!=0)basalt_panic(8);}\n"));
(void)(write_string(out, "static BASALT_UNUSED void runtime_write_line(const char*s){if(!s)basalt_panic(4);if(fputs(s,stdout)==EOF||fputc('\\n',stdout)==EOF||fflush(stdout)!=0)basalt_panic(8);}\n"));
(void)(write_string(out, "static BASALT_UNUSED void runtime_write_int(int value){if(fprintf(stdout,\"%d\",value)<0||fflush(stdout)!=0)basalt_panic(8);}\n"));
(void)(write_string(out, "static BASALT_UNUSED void runtime_write_char(char value){if(fputc((unsigned char)value,stdout)==EOF||fflush(stdout)!=0)basalt_panic(8);}\n"));
(void)(write_string(out, "static BASALT_UNUSED void* basalt_memory_alloc(int count,size_t elem_size){size_t bytes=basalt_checked_bytes(count,elem_size);void*p=calloc(1,bytes?bytes:1);if(!p)basalt_panic(5);return basalt_track(p);}\n"));
(void)(write_string(out, "static BASALT_UNUSED void* basalt_memory_alloc_aligned(int count,int alignment,size_t elem_size){size_t bytes,rounded,a;void*p;if(count<0||alignment<1)basalt_panic(1);a=(size_t)alignment;if((a&(a-1))!=0)basalt_panic(1);if(a<sizeof(void*))a=sizeof(void*);bytes=basalt_checked_bytes(count,elem_size);if(bytes==0)bytes=1;if(bytes>(size_t)-1-(a-1))basalt_panic(1);rounded=(bytes+a-1)&~(a-1);p=aligned_alloc(a,rounded);if(!p)basalt_panic(5);return basalt_track(p);}\n"));
(void)(write_string(out, "static BASALT_UNUSED void* basalt_memory_resize(void* old,int old_count,int new_count,size_t elem_size){size_t slot=(size_t)-1;size_t old_bytes;size_t new_bytes;void*p;if(old_count<0||new_count<0||new_count<old_count)basalt_panic(1);if(old){slot=basalt_find(old);if(slot==(size_t)-1)basalt_panic(2);}old_bytes=basalt_checked_bytes(old_count,elem_size);new_bytes=basalt_checked_bytes(new_count,elem_size);p=realloc(old,new_bytes?new_bytes:1);if(!p)basalt_panic(6);if(slot==(size_t)-1)basalt_track(p);else basalt_live[slot]=p;if(new_bytes>old_bytes)memset((char*)p+old_bytes,0,new_bytes-old_bytes);return p;}\n"));
(void)(write_string(out, "static BASALT_UNUSED void basalt_memory_free(void*p){basalt_release(p);}\n"));
(void)(write_string(out, "static BASALT_UNUSED char* runtime_string_concat(const char* a,const char* b){size_t na,nb,total;char* p;if(!a||!b)basalt_panic(4);na=strlen(a);nb=strlen(b);if(na>(size_t)-1-nb-1)basalt_panic(1);total=na+nb+1;p=(char*)malloc(total);if(!p)basalt_panic(5);memcpy(p,a,na);memcpy(p+na,b,nb);p[na+nb]=0;return(char*)basalt_track(p);}\n"));
(void)(write_string(out, "static char* basalt_sys_out=NULL;static char* basalt_sys_err=NULL;static int basalt_sys_status_value=0;static int basalt_sys_ok_value=0;static int basalt_sys_truncated_value=0;static int basalt_sys_spawn_error_value=0;static int basalt_sys_cleanup_registered=0;static void basalt_sys_free_buffers(void){if(basalt_sys_out){free(basalt_sys_out);basalt_sys_out=NULL;}if(basalt_sys_err){free(basalt_sys_err);basalt_sys_err=NULL;}}\n"));
(void)(write_string(out, "static BASALT_UNUSED char* basalt_sys_empty(void){char*p=(char*)malloc(1);if(!p)basalt_panic(5);p[0]=0;return(char*)basalt_track(p);}\n"));
(void)(write_string(out, "static BASALT_UNUSED char* basalt_sys_raw_empty(void){char*p=(char*)malloc(1);if(!p)basalt_panic(5);p[0]=0;return p;}\n"));
(void)(write_string(out, "static BASALT_UNUSED char* basalt_sys_copy_n(const char*s,size_t n){char*p=(char*)malloc(n+1);if(!p)basalt_panic(5);if(n)memcpy(p,s,n);p[n]=0;return(char*)basalt_track(p);}\n"));
(void)(write_string(out, "#if defined(_WIN32)\nstatic BASALT_UNUSED void basalt_sys_drain(int fd,char*buf,size_t cap,size_t*len,int*truncated,int*open_flag){(void)fd;(void)buf;(void)cap;(void)len;(void)truncated;(void)open_flag;}\n#else\nstatic BASALT_UNUSED void basalt_sys_drain(int fd,char*buf,size_t cap,size_t*len,int*truncated,int*open_flag){char chunk[4096];ssize_t n;while((n=read(fd,chunk,sizeof(chunk)))>0){size_t take=0;if(*len<cap){take=(size_t)n;if(take>cap-*len)take=cap-*len;if(take)memcpy(buf+*len,chunk,take);*len+=take;}if((size_t)n>take)*truncated=1;}if(n==0||(n<0&&errno!=EAGAIN&&errno!=EWOULDBLOCK)){close(fd);*open_flag=0;}}\n#endif\n"));
(void)(write_string(out, "int basalt_sys_run(const char*executable,char**args,int arg_count,int max_output){size_t cap;size_t out_len=0,err_len=0;int i;basalt_sys_free_buffers();if(!basalt_sys_cleanup_registered){atexit(basalt_sys_free_buffers);basalt_sys_cleanup_registered=1;}basalt_sys_status_value=-1;basalt_sys_ok_value=0;basalt_sys_truncated_value=0;basalt_sys_spawn_error_value=0;if(!executable||arg_count<0||max_output<0||max_output>16777216){basalt_sys_spawn_error_value=EINVAL;basalt_sys_out=basalt_sys_raw_empty();basalt_sys_err=basalt_sys_raw_empty();return -1;}cap=(size_t)max_output;basalt_sys_out=(char*)malloc(cap+1);basalt_sys_err=(char*)malloc(cap+1);if(!basalt_sys_out||!basalt_sys_err)basalt_panic(5);basalt_sys_out[0]=0;basalt_sys_err[0]=0;\n#if defined(_WIN32)\n{char**av=(char**)calloc((size_t)arg_count+2,sizeof(char*));int status;if(!av)basalt_panic(5);av[0]=(char*)executable;for(i=0;i<arg_count;i++)av[i+1]=args[i];av[arg_count+1]=NULL;status=_spawnvp(_P_WAIT,executable,(const char* const*)av);free(av);if(status<0){basalt_sys_spawn_error_value=errno;basalt_sys_status_value=-1;}else{basalt_sys_status_value=status;basalt_sys_ok_value=status==0;}basalt_sys_out[0]=0;basalt_sys_err[0]=0;return basalt_sys_status_value;}\n#else\n{int out_pipe[2],err_pipe[2],exec_pipe[2];pid_t child;int status;int out_open=1,err_open=1;int exec_error=0;ssize_t exec_read;if(pipe(out_pipe)!=0||pipe(err_pipe)!=0||pipe(exec_pipe)!=0){basalt_sys_spawn_error_value=errno;return -1;}if(fcntl(exec_pipe[1],F_SETFD,FD_CLOEXEC)<0){basalt_sys_spawn_error_value=errno;close(out_pipe[0]);close(out_pipe[1]);close(err_pipe[0]);close(err_pipe[1]);close(exec_pipe[0]);close(exec_pipe[1]);return -1;}child=fork();if(child<0){basalt_sys_spawn_error_value=errno;close(out_pipe[0]);close(out_pipe[1]);close(err_pipe[0]);close(err_pipe[1]);close(exec_pipe[0]);close(exec_pipe[1]);basalt_sys_out=basalt_sys_raw_empty();basalt_sys_err=basalt_sys_raw_empty();return -1;}if(child==0){char**av=(char**)calloc((size_t)arg_count+2,sizeof(char*));if(!av)_exit(127);av[0]=(char*)executable;for(i=0;i<arg_count;i++)av[i+1]=args[i];av[arg_count+1]=NULL;close(out_pipe[0]);close(err_pipe[0]);close(exec_pipe[0]);if(dup2(out_pipe[1],STDOUT_FILENO)<0||dup2(err_pipe[1],STDERR_FILENO)<0){int e=errno;if(write(exec_pipe[1],&e,sizeof(e))<0){}_exit(127);}close(out_pipe[1]);close(err_pipe[1]);execvp(executable,av);{int e=errno;if(write(exec_pipe[1],&e,sizeof(e))<0){}}_exit(127);}close(out_pipe[1]);close(err_pipe[1]);close(exec_pipe[1]);if(fcntl(out_pipe[0],F_SETFL,O_NONBLOCK)<0||fcntl(err_pipe[0],F_SETFL,O_NONBLOCK)<0){basalt_sys_spawn_error_value=errno;}while(out_open||err_open){struct pollfd pf[2];int polled;pf[0].fd=out_open?out_pipe[0]:-1;pf[0].events=POLLIN;pf[1].fd=err_open?err_pipe[0]:-1;pf[1].events=POLLIN;polled=poll(pf,2,-1);if(polled<0){if(errno==EINTR)continue;break;}if(out_open)basalt_sys_drain(out_pipe[0],basalt_sys_out,cap,&out_len,&basalt_sys_truncated_value,&out_open);if(err_open)basalt_sys_drain(err_pipe[0],basalt_sys_err,cap,&err_len,&basalt_sys_truncated_value,&err_open);}if(out_open){close(out_pipe[0]);out_open=0;}if(err_open){close(err_pipe[0]);err_open=0;}waitpid(child,&status,0);exec_read=read(exec_pipe[0],&exec_error,sizeof(exec_error));close(exec_pipe[0]);basalt_sys_out[out_len]=0;basalt_sys_err[err_len]=0;if(exec_read>0){basalt_sys_spawn_error_value=exec_error;basalt_sys_status_value=-1;}else if(WIFEXITED(status))basalt_sys_status_value=WEXITSTATUS(status);else if(WIFSIGNALED(status))basalt_sys_status_value=0-WTERMSIG(status);else basalt_sys_status_value=-1;basalt_sys_ok_value=(basalt_sys_status_value==0&&basalt_sys_spawn_error_value==0);return basalt_sys_status_value;}\n#endif\n}\n"));
(void)(write_string(out, "int basalt_sys_run_status(void){return basalt_sys_ok_value;}\nchar* basalt_sys_stdout(void){return basalt_sys_out?basalt_sys_out:basalt_sys_empty();}\nchar* basalt_sys_stderr(void){return basalt_sys_err?basalt_sys_err:basalt_sys_empty();}\nint basalt_sys_truncated(void){return basalt_sys_truncated_value;}\nint basalt_sys_spawn_error(void){return basalt_sys_spawn_error_value;}\n"));
(void)(write_string(out, "int basalt_compile_argv(const char*cc,const char*input_c,const char*output_bin,char**all,int start,int extra_count){char**av;int i,status;if(!cc||!input_c||!output_bin||!all||start<0||extra_count<0)return -1;av=(char**)calloc((size_t)extra_count+4,sizeof(char*));if(!av)basalt_panic(5);for(i=0;i<extra_count;i++)av[i]=all[start+i];av[extra_count]=(char*)input_c;av[extra_count+1]=(char*)\"-o\";av[extra_count+2]=(char*)output_bin;av[extra_count+3]=NULL;status=basalt_sys_run(cc,av,extra_count+3,65536);if(status!=0&&basalt_sys_stderr()[0]!=0)fputs(basalt_sys_stderr(),stderr);free(av);return status;}\n"));
(void)(write_string(out, "static BASALT_UNUSED int* alloc_ints(int n){int* p;if(n<0)basalt_panic(1);if(n<1)n=1;basalt_checked_bytes(n,sizeof(int));p=(int*)calloc((size_t)n,sizeof(int));if(!p)basalt_panic(5);return(int*)basalt_track(p);}\n"));
(void)(write_string(out, "static BASALT_UNUSED void free_ints(int* p){basalt_release(p);}\n"));
(void)(write_string(out, "static BASALT_UNUSED int* grow_ints(int* p,int old,int n){size_t slot=(size_t)-1;int* q;if(old<0||n<0)basalt_panic(1);if(n<=old)return p;if(p){slot=basalt_find(p);if(slot==(size_t)-1)basalt_panic(2);}basalt_checked_bytes(n,sizeof(int));q=(int*)realloc(p,(size_t)n*sizeof(int));if(!q)basalt_panic(6);if(p)basalt_live[slot]=q;else basalt_track(q);memset(q+old,0,(size_t)(n-old)*sizeof(int));return q;}\n"));
(void)(write_string(out, "extern int* payload_int; extern int* payload_name; extern int* payload_string;\n"));
(void)(write_string(out, "extern int* code_kind; extern int* code_value; extern int* input_kind; extern int* input_value;\n"));
(void)(write_string(out, "extern int* source; extern int* sym_start; extern int* sym_len; extern int* sym_hash; extern int* sym_kind; extern int* sym_type; extern int* sym_scope;\n\n"));
}
void emit_c_file(char* path){
...
if(auto_compile==1){
if(has_binary_path==0)binary_path = runtime_string_concat(argv[input_index], ".out");
else{
}
