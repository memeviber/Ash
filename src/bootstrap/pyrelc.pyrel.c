...
void gen_array_builtin(int id) {
  ...
  if ((((call_len == 10) && (call_hash == 790299)) && ((node_next)[a] != 0))) {
    {
      int witness = (node_next)[a];
      int wt = tc_emit_arg_type(witness);
      if ((wt != 0)) {
        wt = gen_substitute_type(wt);
      } else {
        {
        }
      }
      ...
      gen_array_make_expr(a, wk, wn, witness);
    }
  } else {
    ...
    if ((((((node_kind)[st] == TY_DYN_ARRAY) && ((node_kind)[expr] == N_CALL)) && ((sym_len)[(node_value)[expr]] == 10)) && ((sym_hash)[(node_value)[expr]] == 790299))) {
      {
        ...
        int witness = 0;
        if (((node_a)[expr] != 0)) {
          witness = (node_next)[(node_a)[expr]];
        } else {
          {
          }
        }
        gen_array_make_expr((node_a)[expr], (node_kind)[elem], (node_value)[elem], witness);
      }
    } else {
      gen_expr(expr);
    }
...
void include_process_line(int* line, int length) {
  ...
  if ((mode == 0)) {
    ...
    {
      int* child = pyrel_include_open_line(line, length, mode);
      if ((child == 0)) {
        {
          if ((pyrel_include_last_status() == 1)) {
            include_ok = 0;
          } else {
            {
            }
...
void load_tokens_from_file(char* path) {
  ...
  c_source_reset();
  include_ok = 1;
  int* handle = pyrel_include_open_root(path);
  if ((handle == 0)) {
    include_ok = 0;
  } else {
    {
...
int tc_owned_initializer(int id) {
  ...
  if ((((sym_len)[(node_value)[id]] == 23) && ((sym_hash)[(node_value)[id]] == 859196))) {
    return 1;
  } else {
    {
    }
  }
  if ((((sym_len)[(node_value)[id]] == 23) && ((sym_hash)[(node_value)[id]] == 43754))) {
    return 1;
  } else {
    {
    }
      ...
      if (((call_len == 10) && (call_hash == 790299))) {
        {
          ...
          if (((node_next)[aa] != 0)) {
            {
              ...
              int ek_make = tc_kind;
              int en_make = tc_name;
              int eek_make = tc_elem_kind;
              int een_make = tc_elem_name;
              if ((ek_make == TY_VOID)) {
                {
                  tc_fail(17);
                  return;
              ...
              int witness_ty = tc_result_type;
              if ((witness_ty == 0)) {
                witness_ty = tc_type_node_from_summary(ek_make, en_make, eek_make, een_make);
              } else {
                {
                }
              }
              ...
              tc_name = 0;
              tc_elem_kind = ek_make;
              tc_elem_name = en_make;
              tc_result_type = ast_node(TY_DYN_ARRAY, witness_ty, 0, 0, 0, 0);
              return;
            }
          } else {
            {
          ...
          tc_elem_kind = TY_INT;
          tc_elem_name = 0;
          tc_result_type = ast_node(TY_DYN_ARRAY, ast_node(TY_INT, 0, 0, 0, 0, 0), 0, 0, 0, 0);
          return;
        }
      } else {
        {
      ...
      if (((call_len == 13) && (call_hash == 333999))) {
        {
          ...
          aa = (node_next)[aa];
          tc_expr(aa);
          if ((((((((((tc_kind != TY_INT) && (tc_kind != TY_BOOL)) && (tc_kind != TY_CHAR)) && (tc_kind != TY_FLOAT)) && (tc_kind != TY_DOUBLE)) && (tc_kind != TY_STRING)) && (tc_kind != TY_PTR)) && (tc_kind != TY_NAMED)) && (tc_kind != TY_PARAM))) {
            tc_fail(17);
          } else {
            {
            }
      ...
      if ((fun_node == 0)) {
        {
          ...
          if ((((sym_len)[(node_value)[id]] == 25) && ((sym_hash)[(node_value)[id]] == 737390))) {
            {
              tc_kind = TY_INT;
              tc_name = 0;
              return;
          ...
          if ((((sym_len)[(node_value)[id]] == 19) && ((sym_hash)[(node_value)[id]] == 551467))) {
            {
              tc_kind = TY_VOID;
              tc_name = 0;
              return;
          ...
          if ((((sym_len)[(node_value)[id]] == 27) && ((sym_hash)[(node_value)[id]] == 667417))) {
            {
              tc_kind = TY_VOID;
              tc_name = 0;
              return;
...
void emit_runtime(int* out) {
  write_string(out, "#if defined(_WIN32)\n#include <direct.h>\n#else\n#define _POSIX_C_SOURCE 200809L\n#define _XOPEN_SOURCE 700\n#endif\n#include <stdio.h>\n#include <stdlib.h>\n#include <string.h>\n#include <limits.h>\n#if defined(__GNUC__) || defined(__clang__)\n#define PYREL_UNUSED __attribute__((unused))\n#else\n#define PYREL_UNUSED\n#endif\nstatic void pyrel_panic(int code){(void)code;exit(2);}\nstatic size_t pyrel_checked_bytes(int count,size_t elem_size){if(count<0)pyrel_panic(1);if(elem_size!=0&&(size_t)count>(size_t)-1/elem_size)pyrel_panic(1);return(size_t)count*elem_size;}\nstatic void* pyrel_track(void*);\n");
  write_string(out, "static char** pyrel_inc_active=NULL;static size_t pyrel_inc_active_n=0,pyrel_inc_active_cap=0;static char** pyrel_inc_loaded=NULL;static size_t pyrel_inc_loaded_n=0,pyrel_inc_loaded_cap=0;static int pyrel_inc_status=0;\n");
  write_string(out, "static PYREL_UNUSED int pyrel_inc_eq(const char*a,const char*b){return strcmp(a,b)==0;}\n");
  write_string(out, "static PYREL_UNUSED size_t pyrel_inc_find(char**v,size_t n,const char*p){size_t i;for(i=0;i<n;i++)if(pyrel_inc_eq(v[i],p))return i;return (size_t)-1;}\n");
  write_string(out, "static PYREL_UNUSED void pyrel_inc_add(char***vp,size_t*np,size_t*cp,char*p){size_t c;char**q;if(*np==*cp){c=*cp?*cp*2:16;q=(char**)realloc(*vp,c*sizeof(char*));if(!q)exit(2);*vp=q;*cp=c;}(*vp)[(*np)++]=p;}\n");
  write_string(out, "static PYREL_UNUSED char* pyrel_inc_strdup(const char*p){size_t n=strlen(p);char*q=(char*)malloc(n+1);if(!q)exit(2);memcpy(q,p,n+1);return(char*)pyrel_track(q);}\n");
  write_string(out, "static PYREL_UNUSED char* pyrel_inc_realpath(const char*p){\n#if defined(_WIN32)\nchar*q=_fullpath(NULL,p,0);if(q)return(char*)pyrel_track(q);\n#else\nchar*q=realpath(p,NULL);if(q)return(char*)pyrel_track(q);\n#endif\nreturn pyrel_inc_strdup(p);}\n");
  write_string(out, "static PYREL_UNUSED int pyrel_inc_begin(char*p){if(pyrel_inc_find(pyrel_inc_active,pyrel_inc_active_n,p)!=(size_t)-1){pyrel_inc_status=1;return 0;}if(pyrel_inc_find(pyrel_inc_loaded,pyrel_inc_loaded_n,p)!=(size_t)-1){pyrel_inc_status=2;return 0;}pyrel_inc_add(&pyrel_inc_active,&pyrel_inc_active_n,&pyrel_inc_active_cap,p);pyrel_inc_status=0;return 1;}\n");
  write_string(out, "static PYREL_UNUSED void pyrel_include_close(void){if(pyrel_inc_active_n){char*p=pyrel_inc_active[--pyrel_inc_active_n];if(pyrel_inc_find(pyrel_inc_loaded,pyrel_inc_loaded_n,p)==(size_t)-1)pyrel_inc_add(&pyrel_inc_loaded,&pyrel_inc_loaded_n,&pyrel_inc_loaded_cap,p);}}\n");
  write_string(out, "static PYREL_UNUSED char* pyrel_inc_join(const char*base,const char*raw){const char*s=strrchr(base,'/');size_t n=s?(size_t)(s-base+1):0;size_t m=strlen(raw);char*q=(char*)malloc(n+m+1);if(!q)exit(2);if(n)memcpy(q,base,n);memcpy(q+n,raw,m+1);return(char*)pyrel_track(q);}\n");
  write_string(out, "static PYREL_UNUSED int pyrel_include_line_mode(int*line,int n){int i=0,j;while(i<n&&(line[i]==' '||line[i]==9))i++;if(i+7<=n&&line[i]=='i'&&line[i+1]=='n'&&line[i+2]=='c'&&line[i+3]=='l'&&line[i+4]=='u'&&line[i+5]=='d'&&line[i+6]=='e'){j=i+7;while(j<n&&(line[j]==' '||line[j]==9))j++;if(j<n&&line[j]==34)return 1;}if(i+8<=n&&line[i]=='i'&&line[i+1]=='n'&&line[i+2]=='c'&&line[i+3]=='l'&&line[i+4]=='u'&&line[i+5]=='d'&&line[i+6]=='e'&&line[i+7]=='c'){j=i+8;while(j<n&&(line[j]==' '||line[j]==9))j++;if(j<n&&line[j]==34)return 2;}return 0;}\n");
  write_string(out, "static PYREL_UNUSED void* pyrel_include_open_root(const char*path){char*p=pyrel_inc_realpath(path);FILE*f;if(!pyrel_inc_begin(p))return NULL;f=fopen(p,(const char[]){114,0});if(!f){pyrel_inc_status=3;pyrel_include_close();return NULL;}return(void*)f;}\n");
  write_string(out, "static PYREL_UNUSED void* pyrel_include_open_line(int*line,int n,int mode){int i=0,a,b,j;char*raw,*joined,*canon;FILE*f;(void)mode;while(i<n&&line[i]!=34)i++;if(i>=n)return NULL;a=++i;while(i<n&&line[i]!=34)i++;if(i>=n)return NULL;b=i;raw=(char*)malloc((size_t)(b-a)+1);if(!raw)exit(2);{int k;for(k=0;k<b-a;k++)raw[k]=(char)line[a+k];}raw[b-a]=0;raw=(char*)pyrel_track(raw);j=i+1;while(j<n&&(line[j]==32||line[j]==9))j++;if(j<n&&line[j]==59)j++;while(j<n&&(line[j]==32||line[j]==9))j++;if(j!=n)return NULL;joined=pyrel_inc_join(pyrel_inc_active[pyrel_inc_active_n-1],raw);canon=pyrel_inc_realpath(joined);if(!pyrel_inc_begin(canon))return NULL;f=fopen(canon,(const char[]){114,0});if(!f){pyrel_inc_status=3;pyrel_include_close();return NULL;}return(void*)f;}\n");
  write_string(out, "static PYREL_UNUSED int pyrel_include_last_status(void){return pyrel_inc_status;}\n");
  write_string(out, "static PYREL_UNUSED void pyrel_include_reset_session(void){pyrel_inc_active_n=0;pyrel_inc_loaded_n=0;pyrel_inc_status=0;}\n");
  write_string(out, "static PYREL_UNUSED void* open_file(const char* p,const char* m){return (void*)fopen(p,m);}\n");
  write_string(out, "static PYREL_UNUSED int read_char(void* h){int c=fgetc((FILE*)h);return c==EOF?-1:c;}\n");
  write_string(out, "static PYREL_UNUSED int close_file(void* h){return fclose((FILE*)h);}\n");
  write_string(out, "static PYREL_UNUSED int write_char(void* h,int c){return fputc(c,(FILE*)h);}\n");
  write_string(out, "static PYREL_UNUSED int write_string(void* h,const char* s){return fputs(s,(FILE*)h);}\n");
  write_string(out, "static void** pyrel_live=NULL;static size_t pyrel_live_n=0,pyrel_live_cap=0;\n");
  write_string(out, "static PYREL_UNUSED size_t pyrel_find(void* p){size_t i;for(i=0;i<pyrel_live_n;i++)if(pyrel_live[i]==p)return i;return (size_t)-1;}\n");
  write_string(out, "static PYREL_UNUSED void pyrel_validate(void){size_t i,j;for(i=0;i<pyrel_live_n;i++){if(!pyrel_live[i])pyrel_panic(2);for(j=i+1;j<pyrel_live_n;j++)if(pyrel_live[i]==pyrel_live[j])pyrel_panic(2);}}\n");
  ...
  write_string(out, "static PYREL_UNUSED PyrelArray runtime_array_make(int capacity,size_t elem_size){PyrelArray a;if(capacity<0)pyrel_panic(1);if(capacity<1)capacity=4;pyrel_checked_bytes(capacity,elem_size);a.data=calloc((size_t)capacity,elem_size);if(!a.data)pyrel_panic(5);a.len=0;a.cap=capacity;a.data=pyrel_track(a.data);return a;}\n");
  write_string(out, "static PYREL_UNUSED PyrelArray runtime_array_reserve(PyrelArray* a,int minimum,size_t elem_size){void*p;pyrel_array_check(a);if(minimum<0)pyrel_panic(1);if(minimum<=a->cap)return *a;pyrel_checked_bytes(minimum,elem_size);p=calloc((size_t)minimum,elem_size);if(!p)pyrel_panic(5);if(a->data){memcpy(p,a->data,pyrel_checked_bytes(a->len,elem_size));pyrel_release(a->data);}a->data=pyrel_track(p);a->cap=minimum;return *a;}\n");
  write_string(out, "static PYREL_UNUSED PyrelArray runtime_array_push(PyrelArray* a,const void* value,size_t elem_size){int next;pyrel_array_check(a);if(!value)pyrel_panic(4);if(a->len>=a->cap){if(a->cap>2147483647/2)pyrel_panic(1);next=a->cap>0?a->cap*2:4;runtime_array_reserve(a,next,elem_size);}memcpy((char*)a->data+(size_t)a->len*elem_size,value,elem_size);a->len+=1;return *a;}\n");
  write_string(out, "static PYREL_UNUSED void runtime_array_set(PyrelArray* a,int index,const void* value,size_t elem_size){pyrel_array_check(a);if(index<0||index>=a->len)pyrel_panic(3);if(!value)pyrel_panic(4);memcpy((char*)a->data+pyrel_checked_bytes(index,elem_size),value,elem_size);}\n");
  write_string(out, "static PYREL_UNUSED PyrelArray runtime_array_clear(PyrelArray* a){pyrel_array_check(a);a->len=0;return *a;}\n");
  write_string(out, "static PYREL_UNUSED void runtime_array_free(PyrelArray* a){if(!a)pyrel_panic(4);if(a->data){if(pyrel_find(a->data)==(size_t)-1)pyrel_panic(2);pyrel_release(a->data);}a->data=NULL;a->len=0;a->cap=0;}\n");
  write_string(out, "static PYREL_UNUSED void* runtime_array_get_raw(PyrelArray* a,int index,size_t elem_size){pyrel_array_check(a);if(index<0||index>=a->len)pyrel_panic(3);return(char*)a->data+pyrel_checked_bytes(index,elem_size);}\n");
  write_string(out, "static PYREL_UNUSED int* alloc_ints(int n){int* p;if(n<0)pyrel_panic(1);if(n<1)n=1;pyrel_checked_bytes(n,sizeof(int));p=(int*)calloc((size_t)n,sizeof(int));if(!p)pyrel_panic(5);return(int*)pyrel_track(p);}\n");
  write_string(out, "static PYREL_UNUSED void free_ints(int* p){pyrel_release(p);}\n");
  write_string(out, "static PYREL_UNUSED int* grow_ints(int* p,int old,int n){size_t slot=(size_t)-1;int* q;if(old<0||n<0)pyrel_panic(1);if(n<=old)return p;if(p){slot=pyrel_find(p);if(slot==(size_t)-1)pyrel_panic(2);}pyrel_checked_bytes(n,sizeof(int));q=(int*)realloc(p,(size_t)n*sizeof(int));if(!q)pyrel_panic(6);if(p)pyrel_live[slot]=q;else pyrel_track(q);memset(q+old,0,(size_t)(n-old)*sizeof(int));return q;}\n");
