...
if ((node_kind[ty]==TY_INT))gen_append_text("int");
else if ((node_kind[ty]==TY_BOOL))gen_append_text("bool");
else if ((node_kind[ty]==TY_STRING))gen_append_text("char_ptr");
else if ((node_kind[ty]==TY_CHAR))gen_append_text("char");
else if ((node_kind[ty]==TY_FLOAT))gen_append_text("float");
else if ((node_kind[ty]==TY_DOUBLE))gen_append_text("double");
else if ((node_kind[ty]==TY_VOID))gen_append_text("void");
else if ((node_kind[ty]==TY_NAMED))gen_append_c_symbol(node_value[ty]);
...
while ((a!=0)){
gen_mangle_type(a);
if ((node_next[a]!=0))gen_append_text("__");
else {
}
a = node_next[a];
}
...
if ((unresolved==0))gen_add_fun_spec(f, actual);
else {
}
}
else {
...
void gen_type(int kind, int child, int size){
if ((kind==TY_INT))code_emit(C_KW, 1);
else if ((kind==TY_BOOL))code_emit(C_KW, 2);
else if ((kind==TY_STRING))code_emit(C_KW, 3);
else if ((kind==TY_CHAR))code_emit(C_KW, 17);
else if ((kind==TY_FLOAT))code_emit(C_KW, 18);
else if ((kind==TY_DOUBLE))code_emit(C_KW, 15);
else if ((kind==TY_LONG))code_emit(C_KW, 19);
else if ((kind==TY_LLONG))code_emit(C_KW, 20);
else if ((kind==TY_VOID))code_emit(C_KW, 4);
else if ((kind==TY_PTR)){
...
if ((b!=0))gen_type(node_kind[b], b, node_value[b]);
...
else if ((kind==TY_NAMED))code_emit(C_IDENT, sym_c_symbol(name));
else if ((kind==TY_PTR)){
code_emit(C_KW, 1);
code_emit(C_PUNCT, 1);
}
...
code_emit(C_IDENT, (0-1011));
code_emit(C_PUNCT, 4);
if (((ty!=0)&&(node_kind[ty]==TY_GENERIC)))code_emit(C_IDENT, gen_mangled_type_symbol(ty));
else if ((ty!=0))gen_array_elem_type(node_kind[ty], node_value[ty]);
...
else if ((ty!=0))gen_array_sizeof_node(ty);
...
code_emit(C_PUNCT, 7);
code_emit(C_PUNCT, 6);
if ((gen_witness_ty!=0))code_emit(C_IDENT, gen_mangled_type_symbol(gen_witness_ty));
...
while ((arg!=0)){
gen_expr(arg);
if ((node_next[arg]!=0))code_emit(C_PUNCT, 7);
else {
}
arg = node_next[arg];
}
...
void gen_expr(int id){
int k = node_kind[id];
if ((k==N_INT))code_emit(C_INT, node_value[id]);
else if ((k==N_BOOL))code_emit(C_INT, node_value[id]);
else if ((k==N_FLOAT))code_emit(C_IDENT, node_value[id]);
else if ((k==N_STRING))code_emit(C_STRING, node_value[id]);
else if ((k==N_CHAR))code_emit(C_INT, node_value[id]);
else if ((k==N_NULL)){
code_emit(C_INT, 0);
}
else if ((k==N_VAR))code_emit(C_IDENT, sym_c_symbol(node_value[id]));
else if ((k==N_BINOP)){
...
int call_name = node_value[id];
int btag = bi_tag(call_name);
if ((((btag==BI_TC_MEM_ALLOC)||(btag==BI_TC_MEM_RESIZE))||(btag==BI_TC_MEM_FREE)))gen_memory_builtin(id);
else {
...
gen_expr(node_a[id]);
if ((gen_expr_kind(node_a[id])==TY_PTR))code_emit(C_PUNCT, 27);
else code_emit(C_PUNCT, 17);
...
if ((k==N_FIELD_ACCESS))return TY_INT;
else {
}
...
if (((((((node_value[id]==OP_EQ)||(node_value[id]==OP_NEQ))||(node_value[id]==OP_LT))||(node_value[id]==OP_GT))||(node_value[id]==OP_AND))||(node_value[id]==OP_OR)))return TY_BOOL;
else {
}
...
if ((((((node_kind[st]==TY_NAMED)||(node_kind[st]==TY_GENERIC))||(node_kind[st]==TY_ARRAY))&&(node_kind[expr]==N_INT))&&(node_value[expr]==0)))code_emit(C_PUNCT, 19);
...
else if ((node_kind[id]==N_EXPR))gen_expr(node_a[id]);
else {
}
}
void gen_fun_decl(int ty, int name){
...
if ((node_next[p]!=0))code_emit(C_PUNCT, 7);
else {
}
p = node_next[p];
}
...
void gen_decl(int ty, int name){
if ((node_kind[ty]==TY_FUN))gen_fun_decl(ty, name);
...
if ((pk==TY_STRING))code_emit(C_PUNCT, 16);
else if ((pk==TY_CHAR))code_emit(C_PUNCT, 20);
else if (((pk==TY_FLOAT)||(pk==TY_DOUBLE)))code_emit(C_PUNCT, 21);
else if ((pk==TY_LONG))code_emit(C_PUNCT, 28);
else if ((pk==TY_LLONG))code_emit(C_PUNCT, 29);
else if ((pk==TY_PTR)){
...
code_emit(C_KW, 5);
if ((node_a[id]!=0))gen_expr(node_a[id]);
else {
}
code_emit(C_PUNCT, 12);
code_emit(C_NEWLINE, 0);
...
if ((emit_for_step!=0))gen_stmt(emit_for_step);
else {
}
code_emit(C_KW, 10);
code_emit(C_PUNCT, 12);
...
if ((gen_bind_find(node_value[formal])==0))gen_bind_add(node_value[formal], actual);
else {
}
return ;
}
...
if ((node_next[f]!=0))code_emit(C_PUNCT, 7);
else {
}
code_emit(C_NEWLINE, 0);
f = node_next[f];
...
void gen_function_signature(int id){
if ((bi_has_flag(node_value[id], BI_FLAG_MAIN)==1))code_emit(C_KW, 1);
else gen_type(node_aux[id], node_b[id], 0);
code_emit(C_IDENT, sym_c_symbol(node_value[id]));
code_emit(C_PUNCT, 6);
int param = node_c[id];
while ((param!=0)){
if ((node_kind[id]==N_EXTERN))gen_extern_param(node_b[param], node_a[param]);
else gen_decl(node_b[param], node_a[param]);
if ((node_next[param]!=0))code_emit(C_PUNCT, 7);
else {
}
...
int item = node_a[id];
while ((item!=0)){
if (((node_kind[item]==N_GLOBAL)||(node_kind[item]==N_CONST)))gen_collect_stmt(item);
...
item = node_a[id];
while ((item!=0)){
if ((node_kind[item]==N_STRUCT))gen_struct_decl(item);
else if ((node_kind[item]==N_ENUM))gen_enum_decl(item);
else {
}
item = node_next[item];
}
si = 0;
while ((si<gen_spec_count)){
if ((gen_spec_kind[si]==1))gen_struct_decl_specialized(gen_spec_decl[si], gen_spec_type[si], gen_spec_name[si]);
else {
}
si = (si+1);
}
item = node_a[id];
while ((item!=0)){
if (((node_kind[item]==N_FUNC)||(node_kind[item]==N_EXTERN)))gen_prototype(item);
else {
}
item = node_next[item];
}
...
if ((node_next[pp]!=0))code_emit(C_PUNCT, 7);
else {
}
pp = node_next[pp];
}
...
item = node_a[id];
while ((item!=0)){
if (((node_kind[item]==N_GLOBAL)||(node_kind[item]==N_CONST)))gen_stmt(item);
else {
}
item = node_next[item];
}
si = 0;
while ((si<gen_spec_count)){
if ((gen_spec_kind[si]==2))gen_function_specialized(gen_spec_decl[si], gen_spec_type[si], gen_spec_name[si]);
else {
}
si = (si+1);
}
item = node_a[id];
while ((item!=0)){
if ((node_kind[item]==N_FUNC))gen_function(item);
else {
}
item = node_next[item];
}
...
if ((length>0))include_process_line(line, length);
else {
}
}
void load_source_file(char* path){
...
else if (((source_index<begin)&&(tc_var_borrow_count[source_index]>0)))tc_var_borrow_count[source_index] = (tc_var_borrow_count[source_index]-1);
else {
}
i = (i+1);
}
...
if ((((((((((ak==TY_INT)||(ak==TY_BOOL))||(ak==TY_STRING))||(ak==TY_CHAR))||(ak==TY_FLOAT))||(ak==TY_DOUBLE))||(ak==TY_LONG))||(ak==TY_LLONG))||(ak==TY_VOID)))return 1;
else {
}
if ((ak==TY_NAMED)){
...
int tc_type_node_from_summary(int kind, int name, int elem_kind, int elem_name){
if (((kind==TY_GENERIC)&&(name!=0)))return name;
else {
}
...
tc_type_node(formal);
if ((tc_type_equal(formal, actual)==0))tc_fail(12);
else {
}
}
int tc_substitute_type(int ty){
...
if ((((a_kind==TY_FLOAT)||(a_kind==TY_DOUBLE))&&((b_kind==TY_FLOAT)||(b_kind==TY_DOUBLE))))return 1;
else {
}
if ((((((a_kind==TY_LONG)||(a_kind==TY_LLONG))||(a_kind==TY_INT))||(a_kind==TY_BOOL))&&((((b_kind==TY_LONG)||(b_kind==TY_LLONG))||(b_kind==TY_INT))||(b_kind==TY_BOOL))))return 1;
else {
}
...
int k = node_kind[ty];
if ((k==TY_NAMED)){
if ((tc_named_exists_ctx(node_value[ty], node_scope[ty])==0))tc_fail(2);
else {
}
}
else if ((k==TY_GENERIC)){
int s = tc_find_struct_ctx(node_value[ty], node_scope[ty]);
if (((s==0)||(node_kind[s]!=N_GENERIC_STRUCT)))tc_fail(2);
else {
if ((tc_generic_arity(s)!=tc_generic_arg_count(ty)))tc_fail(37);
else {
}
int a = node_a[ty];
while ((a!=0)){
...
else if ((k==TY_PTR))tc_check_type(node_a[ty]);
else if ((k==TY_ARRAY))tc_check_type(node_a[ty]);
else if ((k==TY_DYN_ARRAY))tc_check_type(node_a[ty]);
else if ((k==TY_FUN)){
int p = node_a[ty];
while ((p!=0)){
tc_check_type(p);
...
int tc_owned_initializer(int id){
if (((id==0)||(node_kind[id]!=N_CALL)))return 0;
else {
}
...
if (((index<tc_var_count)&&(tc_var_borrow_count[index]>0)))return 1;
else {
}
return 0;
}
...
if ((tc_is_owner_kind(tc_kind)==1))tc_move_var(tc_last_var_index);
else {
}
}
void tc_record_borrow(int destination, int source_index2){
...
if (((tc_lookup_var(node_value[id])==1)&&(tc_borrow_conflict(tc_last_var_index)==1)))tc_fail(37);
else {
}
}
...
else if (((tc_kind==TY_GENERIC)||(tc_kind==TY_PARAM))){
tc_name = ty;
}
else {
tc_name = 0;
...
tc_result_type = 0;
tc_expr_borrow_source = (0-1);
if ((id!=0))tc_error_pos = node_pos[id];
else {
}
if ((id==0)){
tc_fail(4);
...
if ((tc_last_var_moved==1))tc_fail(33);
else {
}
...
if (((((is_const==1)&&(tc_result_type!=0))&&(node_kind[tc_result_type]==TY_ARRAY))&&((ik<0)||(ik>(node_value[tc_result_type]-1)))))tc_fail(45);
else {
}
tc_kind = TY_INT;
tc_name = 0;
...
if (((pek_fp==TY_DYN_ARRAY)&&(ak_fp==TY_DYN_ARRAY)))tc_move_value(arg_fp);
else {
}
if ((tc_same_full(ak_fp, an_fp, aek_fp, aen_fp, pek_fp, pen_fp, peek_fp, peen_fp)==0))tc_fail(12);
else {
}
arg_fp = node_next[arg_fp];
p_fp = node_next[p_fp];
}
if (((arg_fp!=0)||(p_fp!=0)))tc_fail(13);
else {
}
...
if (((generic_moves_array==1)&&(actual_kind==TY_DYN_ARRAY)))tc_move_value(ga);
else {
}
...
if (((pek==TY_DYN_ARRAY)&&(ak==TY_DYN_ARRAY)))tc_move_value(arg);
else {
}
if ((tc_same_full(ak, an, aek, aen, pek, pen, peek, peen)==0))tc_fail(12);
else {
}
arg = node_next[arg];
p = node_next[p];
}
if (((arg!=0)||(p!=0)))tc_fail(13);
else {
}
...
if ((tc_same_full(ak, an, aek, aen, pk, pn, pek, pen)==0))tc_fail(12);
else {
}
arg = node_next[arg];
param = node_next[param];
...
if ((node_value[id]==OP_CONCAT)){
if (((ak!=TY_STRING)||(bk!=TY_STRING)))tc_fail(14);
else {
}
tc_kind = TY_STRING;
tc_name = 0;
...
if ((((ak!=TY_BOOL)&&(ak!=TY_INT))||((bk!=TY_BOOL)&&(bk!=TY_INT))))tc_fail(15);
else {
}
tc_kind = TY_BOOL;
tc_name = 0;
...
if ((((((ak!=TY_INT)&&(ak!=TY_CHAR))&&(ak!=TY_LONG))&&(ak!=TY_LLONG))||((((bk!=TY_INT)&&(bk!=TY_CHAR))&&(bk!=TY_LONG))&&(bk!=TY_LLONG))))tc_fail(32);
else {
}
...
if (((tc_same_full(ak, an, ae, aen, bk, bn, be, ben)==0)&&(null_cmp==0)))tc_fail(16);
else {
}
tc_kind = TY_BOOL;
tc_name = 0;
...
if ((((((((ak!=TY_INT)&&(ak!=TY_CHAR))&&(ak!=TY_LONG))&&(ak!=TY_LLONG))&&(ak!=TY_FLOAT))&&(ak!=TY_DOUBLE))||((((((bk!=TY_INT)&&(bk!=TY_CHAR))&&(bk!=TY_LONG))&&(bk!=TY_LLONG))&&(bk!=TY_FLOAT))&&(bk!=TY_DOUBLE))))tc_fail(17);
else {
}
tc_kind = TY_BOOL;
tc_name = 0;
...
else if ((((ak==TY_INT)&&(bk==TY_PTR))&&(be!=TY_VOID))){
tc_kind = TY_PTR;
tc_name = bn;
tc_elem_kind = be;
tc_elem_name = ben;
}
else if ((((((((ak==TY_INT)||(ak==TY_CHAR))||(ak==TY_LONG))||(ak==TY_LLONG))||(ak==TY_FLOAT))||(ak==TY_DOUBLE))&&((((((bk==TY_INT)||(bk==TY_CHAR))||(bk==TY_LONG))||(bk==TY_LLONG))||(bk==TY_FLOAT))||(bk==TY_DOUBLE)))){
...
if ((((((((ak!=TY_INT)&&(ak!=TY_CHAR))&&(ak!=TY_LONG))&&(ak!=TY_LLONG))&&(ak!=TY_FLOAT))&&(ak!=TY_DOUBLE))||((((((bk!=TY_INT)&&(bk!=TY_CHAR))&&(bk!=TY_LONG))&&(bk!=TY_LLONG))&&(bk!=TY_FLOAT))&&(bk!=TY_DOUBLE))))tc_fail(18);
else {
}
tc_kind = tc_numeric_result_kind(ak, bk);
tc_name = 0;
...
int f = node_a[decl];
while ((f!=0)){
if ((node_a[f]==node_value[id]))return gen_substitute_type(node_b[f]);
else {
}
f = node_next[f];
}
...
if ((node_aux[id]!=0))return node_aux[id];
else {
}
}
else {
...
if ((node_kind[id]==N_FLOAT))return ast_node(TY_DOUBLE, 0, 0, 0, 0, 0);
else {
}
...
if (((node_kind[node_c[id]]!=N_INT)||(node_value[node_c[id]]!=0)))if ((node_kind[node_c[id]]!=N_NULL))tc_fail(30);
else {
}
else {
}
...
if (((tc_is_owner_kind(dk)==1)&&(node_kind[node_c[id]]==N_VAR)))tc_fail(40);
else {
}
if ((tc_same_full(dk, dn, de, den, ek, en, ee, een)==0)){
if ((((node_kind[node_c[id]]!=N_INT)||(node_value[node_c[id]]!=0))&&(node_kind[node_c[id]]!=N_NULL)))tc_fail(20);
else {
}
}
else {
}
if (((dk==TY_DYN_ARRAY)&&(node_kind[node_c[id]]==N_VAR)))tc_move_value(node_c[id]);
else {
}
tc_add_var(node_a[id], dk, dn, de, den, node_b[id]);
if (((dk==TY_DYN_ARRAY)||(tc_owned_initializer(node_c[id])==1)))tc_var_owned[tc_last_var_index] = 1;
else {
}
if ((dk==TY_PTR)){
if ((rhs_borrow<0)){
}
else tc_record_borrow(tc_last_var_index, rhs_borrow);
}
else {
}
}
else if ((k==N_ASSIGN)){
if (((node_kind[node_a[id]]==N_VAR)&&(sym_type[node_value[node_a[id]]]>100)))tc_fail(31);
else {
}
tc_expr(node_a[id]);
int lk = tc_kind;
...
if (((lk==TY_DYN_ARRAY)&&(node_kind[node_b[id]]==N_VAR)))tc_fail(40);
else {
}
if ((tc_same_full(lk, ln, le, len, tc_kind, tc_name, tc_elem_kind, tc_elem_name)==0)){
if ((((node_kind[node_b[id]]!=N_INT)||(node_value[node_b[id]]!=0))&&(node_kind[node_b[id]]!=N_NULL)))tc_fail(21);
else {
}
}
else {
...
else if (((lhs_borrow<tc_var_count)&&(tc_var_borrow_count[lhs_borrow]>0)))tc_var_borrow_count[lhs_borrow] = (tc_var_borrow_count[lhs_borrow]-1);
else {
}
tc_var_borrow_source[lhs_index] = (0-1);
if ((rhs_borrow_assign<0)){
}
else tc_record_borrow(lhs_index, rhs_borrow_assign);
}
else {
}
}
else if ((k==N_PRINT))tc_expr(node_a[id]);
else if ((k==N_EXPR)){
tc_expr(node_a[id]);
if ((node_kind[node_a[id]]==N_CALL))tc_consume_call(node_a[id]);
else {
}
}
else if ((k==N_RETURN)){
if ((node_a[id]==0)){
if ((expected_kind!=TY_VOID))tc_fail(22);
else {
}
}
else {
...
if ((tc_same_full(expected_kind, expected_name, tc_expected_elem_kind, tc_expected_elem_name, tc_kind, tc_name, tc_elem_kind, tc_elem_name)==0))if ((node_kind[node_a[id]]!=N_NULL))tc_fail(23);
else {
}
else {
}
...
tc_expr(node_a[id]);
if ((((((((tc_kind!=TY_BOOL)&&(tc_kind!=TY_INT))&&(tc_kind!=TY_CHAR))&&(tc_kind!=TY_FLOAT))&&(tc_kind!=TY_DOUBLE))&&(tc_kind!=TY_PTR))&&(tc_kind!=TY_FUN)))tc_fail(25);
else {
}
...
tc_expr(node_a[id]);
if ((((((((tc_kind!=TY_BOOL)&&(tc_kind!=TY_INT))&&(tc_kind!=TY_CHAR))&&(tc_kind!=TY_FLOAT))&&(tc_kind!=TY_DOUBLE))&&(tc_kind!=TY_PTR))&&(tc_kind!=TY_FUN)))tc_fail(26);
else {
}
...
tc_enter_scope();
if ((node_a[id]!=0))tc_stmt(node_a[id], expected_kind, expected_name);
else {
}
tc_expr(node_b[id]);
if ((((((((tc_kind!=TY_BOOL)&&(tc_kind!=TY_INT))&&(tc_kind!=TY_CHAR))&&(tc_kind!=TY_FLOAT))&&(tc_kind!=TY_DOUBLE))&&(tc_kind!=TY_PTR))&&(tc_kind!=TY_FUN)))tc_fail(27);
else {
}
tc_loop_depth = (tc_loop_depth+1);
tc_stmt(node_c[id], expected_kind, expected_name);
tc_loop_depth = (tc_loop_depth-1);
if ((node_value[id]!=0))tc_stmt(node_value[id], expected_kind, expected_name);
else {
}
tc_leave_scope();
}
...
if ((tc_cycle_struct(node_value[item])==1))tc_fail(28);
else {
}
}
else {
...
tc_expr(node_c[item]);
if ((((tc_same_full(gk, gn, ge, gen, tc_kind, tc_name, tc_elem_kind, tc_elem_name)==0)&&((node_kind[node_c[item]]!=N_INT)||(node_value[node_c[item]]!=0)))&&(node_kind[node_c[item]]!=N_NULL)))tc_fail(29);
else {
}
}
else {
...
if ((pk==TY_DYN_ARRAY))tc_var_owned[tc_last_var_index] = 1;
else {
}
p = node_next[p];
}
...
void emit_c_token(int *out, int kind, int value){
if ((kind==C_KW)){
if ((value==1))write_string(out, "int ");
else if ((value==2))write_string(out, "int ");
else if ((value==3))write_string(out, "char* ");
else if ((value==4))write_string(out, "void ");
else if ((value==5))write_string(out, "return ");
else if ((value==6))write_string(out, "if ");
else if ((value==7))write_string(out, "else ");
else if ((value==8))write_string(out, "while ");
else if ((value==9))write_string(out, "break ");
else if ((value==10))write_string(out, "continue ");
else if ((value==11))write_string(out, "for ");
else if ((value==12))write_string(out, "struct ");
else if ((value==13))write_string(out, "enum ");
else if ((value==14))write_string(out, "typedef ");
else if ((value==15))write_string(out, "double ");
else if ((value==16))write_string(out, "const ");
else if ((value==18))write_string(out, "float ");
else if ((value==19))write_string(out, "long ");
else if ((value==20))write_string(out, "long long ");
else if ((value==17))write_string(out, "char ");
else {
}
}
else if ((kind==C_IDENT)){
if ((value==(0-1001)))write_string(out, "printf");
else if ((value==(0-1002)))write_string(out, "runtime_string_concat");
else if ((value==(0-1011)))write_string(out, "sizeof");
else if ((value==(0-1012)))write_string(out, "len");
else if ((value==(0-1013)))write_string(out, "cap");
else if ((value==(0-1015)))write_string(out, "data");
else if ((value==(0-1016)))write_string(out, "basalt_memory_alloc");
else if ((value==(0-1017)))write_string(out, "basalt_memory_resize");
else if ((value==(0-1018)))write_string(out, "basalt_memory_free");
else emit_symbol(out, value);
}
else if ((kind==C_INT))emit_int_text(out, value);
else if ((kind==C_STRING))emit_string(out, value);
else if ((kind==C_OP)){
if ((value==1))write_string(out, "+");
else if ((value==2))write_string(out, "-");
else if ((value==3))write_string(out, "*");
else if ((value==4))write_string(out, "/");
else if ((value==5))write_string(out, "==");
else if ((value==6))write_string(out, "!=");
else if ((value==7))write_string(out, "<");
else if ((value==8))write_string(out, ">");
else if ((value==9))write_string(out, "&&");
else if ((value==10))write_string(out, "||");
else if ((value==11))write_string(out, "++");
else if ((value==12))write_string(out, "&");
else if ((value==13))write_string(out, "|");
else if ((value==14))write_string(out, "^");
else if ((value==15))write_string(out, "<<");
else if ((value==16))write_string(out, ">>");
else if ((value==17))write_string(out, "%");
else if ((value==18))write_string(out, ":");
else {
}
}
...
else if ((value==15))emit_print_prefix(out);
else if ((value==16)){
write_char(out, 40);
write_char(out, 34);
write_string(out, "%s");
...
else if ((value==19))write_string(out, "{0}");
else if ((value==22))write_char(out, 32);
else if ((value==23)){
write_char(out, 40);
write_char(out, 34);
write_string(out, "%d");
...
else if ((value==24))write_string(out, "{");
else if ((value==25))write_string(out, "}");
else if ((value==27))write_string(out, "->");
else if ((value==28)){
write_char(out, 40);
write_char(out, 34);
write_string(out, "%ld");
...
else if ((kind==C_NEWLINE))write_char(out, 10);
else {
}
}
void emit_runtime(int *out){
