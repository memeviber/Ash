// Ash bootstrap v1/v2 driver: AST payload -> C-token IR -> deterministic second pass.
//
// Ash has no records or dynamic allocation yet, so the AST is an arena of
...
func gen_append_char(c: int): void { ensure_source(source_len); source[source_len] = c; source_len = source_len + 1; gen_mangle_len = gen_mangle_len + 1; }
func gen_append_char_text(c: char): void { ensure_source(source_len); source[source_len] = c; source_len = source_len + 1; gen_mangle_len = gen_mangle_len + 1; }
...
func sym_c_symbol(id: int): int {
  gen_mangle_start = source_len; gen_mangle_len = 0; gen_append_c_symbol(id);
...
func gen_mangled_type_symbol(ty: int): int {
  gen_mangle_start = source_len; gen_mangle_len = 0; gen_mangle_type(ty);
  return sym_intern(gen_mangle_start, gen_mangle_len, 0, 0);
}
func gen_mangled_function_symbol(base: int, args: int): int {
  gen_mangle_start = source_len; gen_mangle_len = 0; gen_append_c_symbol(base); gen_append_text("__");
...
func gen_collect_expr(id: int): void {
  ...
  if k == N_CALL then {
    ...
    if f != 0 && node_kind[f] == N_GENERIC_FUNC then {
      ...
      while aa != 0 { let q: int = 0; if node_kind[aa] == N_VAR && node_aux[aa] != 0 then q = node_aux[aa]; else if node_kind[aa] == N_STRING then q = ast_node(TY_STRING, 0, 0, 0, 0, 0); else { tc_expr(aa); q = tc_result_type; if q == 0 then q = tc_type_node_from_summary(tc_kind, tc_name, tc_elem_kind, tc_elem_name); } if q != 0 then q = gen_substitute_type(q); if actual == 0 then actual = q; else actual = ast_link(actual, q); gen_collect_type(q); aa = node_next[aa]; }
...
func gen_array_elem_kind(arg: int): int {
  if arg != 0 && node_kind[arg] == N_VAR then return sym_elem_kind[node_value[arg]];
  return TY_INT;
}

func gen_array_elem_name(arg: int): int {
  if arg != 0 && node_kind[arg] == N_VAR then return sym_elem_name[node_value[arg]];
  return 0;
}
...
func gen_array_elem_type(kind: int, name: int): void {
  ...
  else if kind == TY_NAMED then code_emit(C_IDENT, name);
...
func gen_array_make_expr(capacity: int, kind: int, name: int): void {
...
func gen_array_checked_get(arr: int, pos: int, kind: int, name: int): void {
  code_emit(C_PUNCT, 9);
  code_emit(C_PUNCT, 6);
  code_emit(C_PUNCT, 6);
  gen_array_elem_type(kind, name);
...
func gen_array_builtin(id: int): void {

  let call_name: int = node_value[id];
  let call_len: int = sym_len[call_name];
  let call_hash: int = sym_hash[call_name];
  let a: int = node_a[id];
  let ek: int = gen_array_elem_kind(a);
  let en: int = gen_array_elem_name(a);
  if call_len == 10 && call_hash == 790299 then {
    code_emit(C_IDENT, 1004);
    code_emit(C_PUNCT, 6);
    gen_expr(a);
    code_emit(C_PUNCT, 7);
    ...
    code_emit(C_PUNCT, 8);
  } else if call_len == 13 && call_hash == 333999 then {
    ...
    gen_array_sizeof(ek, en);
    code_emit(C_PUNCT, 8);
  } else if call_len == 10 && call_hash == 899143 then {
    ...
    gen_array_sizeof(ek, en);
    code_emit(C_PUNCT, 8);
  } else if call_len == 9 && call_hash == 890825 then {
    gen_array_checked_get(a, node_next[a], ek, en);
  } else if call_len == 9 && call_hash == 902357 then {
    ...
    gen_array_sizeof(ek, en);
    code_emit(C_PUNCT, 8);
  } else if call_len == 11 && call_hash == 585984 then {
    code_emit(C_IDENT, 1009);
    code_emit(C_PUNCT, 6);
    code_emit(C_PUNCT, 10);
    gen_expr(a);
    code_emit(C_PUNCT, 8);
  } else if call_len == 10 && call_hash == 597913 then {
...
func gen_call_name(id: int): int {
  let f: int = tc_find_function_ctx(node_value[id], node_scope[id]);
  if f == 0 || node_kind[f] != N_GENERIC_FUNC then return sym_c_symbol(node_value[id]);
  let actual: int = 0; let a: int = node_a[id];
  while a != 0 {
    let q: int = 0;
      if node_kind[a] == N_VAR && node_aux[a] != 0 then q = node_aux[a];
    else if node_kind[a] == N_STRING then q = ast_node(TY_STRING, 0, 0, 0, 0, 0);
    else { tc_expr(a); q = tc_result_type; if q == 0 then q = tc_type_node_from_summary(tc_kind, tc_name, tc_elem_kind, tc_elem_name); }
...
func gen_expr(id: int): void {
  ...
  if k == N_INT then code_emit(C_INT, node_value[id]);
    ...
    let call_len: int = sym_len[call_name];
    let call_hash: int = sym_hash[call_name];
    if (call_len == 10 && call_hash == 790299) || (call_len == 13 && call_hash == 333999) || (call_len == 10 && call_hash == 899143) || (call_len == 9 && call_hash == 890825) || (call_len == 9 && call_hash == 902357) || (call_len == 11 && call_hash == 585984) || (call_len == 10 && call_hash == 597913) then gen_array_builtin(id);
    ...
    let base_kind: int = gen_expr_kind(node_a[id]);
    if base_kind == TY_DYN_ARRAY then gen_array_checked_get(node_a[id], node_b[id], gen_array_elem_kind(node_a[id]), gen_array_elem_name(node_a[id]));
    else { gen_expr(node_a[id]); code_emit(C_PUNCT, 2); gen_expr(node_b[id]); code_emit(C_PUNCT, 3); }
  }
...
func gen_expr_kind(id: int): int {
  ...
  if k == N_CALL then {
    let call_name: int = node_value[id];
    if sym_len[call_name] == 9 && sym_hash[call_name] == 890825 then return gen_array_elem_kind(node_a[id]);
  ...
  if k == N_BINOP then {
    ...
    if node_value[id] == OP_SUB && gen_expr_kind(node_a[id]) == TY_PTR && gen_expr_kind(node_b[id]) == TY_PTR then return TY_PTRDIFF;
...
func gen_initializer(ty: int, expr: int): void {
  ...
  else if node_kind[st] == TY_DYN_ARRAY && node_kind[expr] == N_CALL && sym_len[node_value[expr]] == 10 && sym_hash[node_value[expr]] == 790299 then {
    let elem: int = node_a[ty];
...
func gen_decl(ty: int, name: int): void {
  if node_kind[ty] == TY_FUN then gen_fun_decl(ty, name);
    ...
    code_emit(C_IDENT, name); code_emit(C_PUNCT, 2); code_emit(C_INT, node_value[ty]); code_emit(C_PUNCT, 3);
  } else {
...
func gen_stmt(id: int): void {
  ...
  if k == N_GLOBAL then {
    ...
    else if pk == TY_PTRDIFF then code_emit(C_PUNCT, 23);
...
func gen_unify_formal(formal: int, actual: int): void {
  ...
  if node_kind[formal] == TY_PARAM then { if gen_bind_find(node_value[formal]) == 0 then gen_bind_add(node_value[formal], actual); return; }
...
func gen_function_specialized(decl: int, inst: int, cname: int): void {
  ...
  code_emit(C_PUNCT, 8); gen_stmt(node_a[decl]);
  gen_bind_clear();
}
...
func gen_program(id: int): void {
  ...
  while item != 0 {
    if node_kind[item] == N_STRUCT then {
      code_emit(C_KW, 14); code_emit(C_KW, 12); code_emit(C_IDENT, node_value[item]); code_emit(C_PUNCT, 22); code_emit(C_IDENT, node_value[item]); code_emit(C_PUNCT, 12); code_emit(C_NEWLINE, 0);
    }
    item = node_next[item];
  }
  let si: int = 0;
...
func ast_type(): int {
  ...
  if input_take(T_ARRAY) == 1 then {
    if input_take(T_LT) == 0 then return 0;
    let elem: int = ast_type();
    if elem == 0 then return 0;
...
func ast_program(): int {
  ...
  while input_peek() != T_EOF {
    let item: int = ast_decl();
    if item < 0 then return (0 - 1);
    if node_kind[item] == N_LIST then {
      let nested: int = node_a[item];
      if nested != 0 then {
        if items == 0 then items = nested;
        else {
          let tail: int = items;
    ...
    } else if items == 0 then items = item; else items = ast_link(items, item);
  }
...
func lexer_next(): int {
  ...
  if is_alpha(c) == 1 then {
    ...
    if tok_kind == L_ID then tok_value = sym_intern(tok_start, tok_length, L_ID, 0);
    else tok_value = 0;
    return tok_kind;
  }
  if c == 39 then {
...
func include_process_line(line: int*, length: int): void {
  ...
  if mode == 0 then {
    ...
    let child: int* = ash_include_open_line(line, length, mode);
    if child == 0 then {
      if ash_include_last_status() == 1 then include_ok = 0;
    } else {
      if mode == 1 then {
...
func load_tokens_from_file(path: string): void {
  ...
  c_source_reset();
  include_ok = 1;
  let handle: int* = ash_include_open_root(path);
...
func tc_name_is_array_free(name: int): int {
  if sym_len[name] != 10 then return 0;
  let s: int = sym_start[name];
  if source[s] != 97 || source[s + 1] != 114 || source[s + 2] != 114 || source[s + 3] != 97 then return 0;
  if source[s + 4] != 121 || source[s + 5] != 95 || source[s + 6] != 102 || source[s + 7] != 114 then return 0;
  if source[s + 8] != 101 || source[s + 9] != 101 then return 0;
  return 1;
}
func tc_release_name(name: int): int {
  ...
  if tc_name_is_array_free(name) == 1 then return 1;
  return 0;
}
 ...
 { tc_kind = TY_FUN; tc_name = 0; return; }
  ...
  if k == N_CALL then {
    ...
    if call_len == 10 && call_hash == 790299 then {
      ...
      tc_expr(aa);
      if tc_kind != TY_INT && tc_kind != TY_BOOL && tc_kind != TY_CHAR then tc_fail(17);
      tc_kind = TY_DYN_ARRAY; tc_name = 0; tc_elem_kind = TY_INT; tc_elem_name = 0; return;
    }
    if call_len == 13 && call_hash == 333999 then {
      let aa: int = node_a[id];
      if aa == 0 || node_next[aa] == 0 || node_next[node_next[aa]] != 0 then { tc_fail(13); return; }
      tc_expr(aa);
      if tc_kind != TY_DYN_ARRAY then { tc_fail(8); return; }
      tc_require_mutable(aa);
      let ek: int = tc_elem_kind; let en: int = tc_elem_name;
      aa = node_next[aa]; tc_expr(aa);
      if tc_kind != TY_INT && tc_kind != TY_BOOL && tc_kind != TY_CHAR && tc_kind != TY_FLOAT && tc_kind != TY_DOUBLE && tc_kind != TY_STRING && tc_kind != TY_PTR && tc_kind != TY_NAMED then tc_fail(17);
      tc_kind = TY_DYN_ARRAY; tc_name = 0; tc_elem_kind = ek; tc_elem_name = en; return;
    }
    if call_len == 10 && call_hash == 899143 then {
      let aa: int = node_a[id];
      if aa == 0 || node_next[aa] == 0 || node_next[node_next[aa]] != 0 then { tc_fail(13); return; }
      tc_expr(aa);
      if tc_kind != TY_DYN_ARRAY then { tc_fail(8); return; }
      ...
      let ek: int = tc_elem_kind; let en: int = tc_elem_name;
      aa = node_next[aa]; tc_expr(aa);
      if tc_array_elem_same(ek, en, tc_kind, tc_name) == 0 then tc_fail(36);
      tc_kind = TY_DYN_ARRAY; tc_name = 0; tc_elem_kind = ek; tc_elem_name = en; return;
    }
    if call_len == 9 && call_hash == 890825 then {
      let aa: int = node_a[id];
      if aa == 0 || node_next[aa] == 0 || node_next[node_next[aa]] != 0 then { tc_fail(13); return; }
      tc_expr(aa);
      if tc_kind != TY_DYN_ARRAY then { tc_fail(8); return; }
      ...
      aa = node_next[aa]; tc_expr(aa);
      if tc_kind != TY_INT && tc_kind != TY_BOOL && tc_kind != TY_CHAR && tc_kind != TY_FLOAT && tc_kind != TY_DOUBLE && tc_kind != TY_STRING && tc_kind != TY_PTR && tc_kind != TY_NAMED then tc_fail(17);
      tc_kind = ek; tc_name = en; tc_elem_kind = 0; tc_elem_name = 0;
      if ek == TY_PTR && tc_last_var_type != 0 && node_a[tc_last_var_type] != 0 && node_a[node_a[tc_last_var_type]] != 0 then {
        let pointee: int = node_a[node_a[tc_last_var_type]];
        tc_elem_kind = node_kind[pointee];
        if tc_elem_kind == TY_NAMED then tc_elem_name = node_value[pointee];
      }
      return;
    }
    if call_len == 9 && call_hash == 902357 then {
      let aa: int = node_a[id];
      ...
      let ek: int = tc_elem_kind; let en: int = tc_elem_name;
      aa = node_next[aa]; tc_expr(aa);
      if tc_kind != TY_INT && tc_kind != TY_BOOL && tc_kind != TY_CHAR && tc_kind != TY_FLOAT && tc_kind != TY_DOUBLE && tc_kind != TY_STRING && tc_kind != TY_PTR && tc_kind != TY_NAMED then { tc_fail(17); return; }
      aa = node_next[aa]; tc_expr(aa);
      if tc_array_elem_same(ek, en, tc_kind, tc_name) == 0 then tc_fail(36);
      tc_kind = TY_VOID; tc_name = 0; tc_elem_kind = 0; tc_elem_name = 0; return;
    }
    if call_len == 11 && call_hash == 585984 then {
      let aa: int = node_a[id];
      if aa == 0 || node_next[aa] != 0 then { tc_fail(13); return; }
      tc_expr(aa);
      if tc_kind != TY_DYN_ARRAY then { tc_fail(8); return; }
    ...
    if call_len == 10 && call_hash == 597913 then {
      let aa: int = node_a[id];
      if aa == 0 || node_next[aa] != 0 then { tc_fail(13); return; }
      tc_expr(aa);
      if tc_kind != TY_DYN_ARRAY then tc_fail(8);
      tc_require_mutable(aa);
      tc_kind = TY_VOID; tc_name = 0; tc_elem_kind = 0; tc_elem_name = 0; return;
    }
    ...
    if fun_node == 0 then {
      if tc_lookup_var(node_value[id]) == 1 && tc_kind == TY_FUN then {
        ...
        if fty == 0 || node_kind[fty] != TY_FUN then { tc_kind = TY_INT; tc_name = 0; return; }
      ...
      if sym_len[node_value[id]] == 9 && sym_hash[node_value[id]] == 776147 then { tc_kind = TY_STRING; tc_name = 0; return; }
      ...
      if sym_len[node_value[id]] == 21 && sym_hash[node_value[id]] == 375664 then { tc_kind = TY_PTR; tc_name = 0; tc_elem_kind = TY_VOID; tc_elem_name = 0; return; }
      if sym_len[node_value[id]] == 21 && sym_hash[node_value[id]] == 191106 then { tc_kind = TY_PTR; tc_name = 0; tc_elem_kind = TY_VOID; tc_elem_name = 0; return; }
      if sym_len[node_value[id]] == 23 && sym_hash[node_value[id]] == 702900 then { tc_kind = TY_INT; tc_name = 0; return; }
      if sym_len[node_value[id]] == 17 && sym_hash[node_value[id]] == 344177 then { tc_kind = TY_VOID; tc_name = 0; return; }
      if sym_len[node_value[id]] == 25 && sym_hash[node_value[id]] == 522527 then { tc_kind = TY_VOID; tc_name = 0; return; }
      if sym_len[node_value[id]] == 21 && sym_hash[node_value[id]] == 686023 then { tc_kind = TY_INT; tc_name = 0; return; }
      if sym_len[node_value[id]] == 16 && sym_hash[node_value[id]] == 904440 then { tc_kind = TY_STRING; tc_name = 0; return; }
      tc_kind = TY_INT; tc_name = 0; return;
    }
    if node_kind[fun_node] == N_GENERIC_FUNC then {
      ...
      while ga != 0 && gp != 0 {
        ...
        if actual_kind == TY_DYN_ARRAY then tc_move_value(ga);
...
func tc_emit_arg_type(id: int): int {
  ...
  if node_kind[id] == N_VAR && node_aux[id] != 0 then return node_aux[id];
...
func tc_stmt(id: int, expected_kind: int, expected_name: int): void {
  ...
  if k == N_CONST then {
    ...
    if tc_same_full(dk, dn, de, den, ek, en, ee, een) == 0 then {
      let polymorphic_make: int = 0;
      if node_kind[node_c[id]] == N_CALL && sym_len[node_value[node_c[id]]] == 10 && sym_hash[node_value[node_c[id]]] == 790299 then polymorphic_make = 1;
      if polymorphic_make == 0 && (node_kind[node_c[id]] != N_INT || node_value[node_c[id]] != 0) && node_kind[node_c[id]] != N_NULL then tc_fail(20);
    }
    ...
    tc_expr(node_a[id]); if tc_kind != TY_BOOL && tc_kind != TY_INT && tc_kind != TY_CHAR && tc_kind != TY_FLOAT && tc_kind != TY_DOUBLE && tc_kind != TY_PTR && tc_kind != TY_FUN then tc_fail(25);
    tc_stmt(node_b[id], expected_kind, expected_name); tc_stmt(node_c[id], expected_kind, expected_name);
  } else if k == N_WHILE then {
    tc_expr(node_a[id]); if tc_kind != TY_BOOL && tc_kind != TY_INT && tc_kind != TY_CHAR && tc_kind != TY_FLOAT && tc_kind != TY_DOUBLE && tc_kind != TY_PTR && tc_kind != TY_FUN then tc_fail(26);
    ...
    tc_expr(node_b[id]); if tc_kind != TY_BOOL && tc_kind != TY_INT && tc_kind != TY_CHAR && tc_kind != TY_FLOAT && tc_kind != TY_DOUBLE && tc_kind != TY_PTR && tc_kind != TY_FUN then tc_fail(27);
...
func tc_program(root: int): int {
  ...
  item = node_a[root];
    while item != 0 { if node_kind[item] == N_GLOBAL || node_kind[item] == N_CONST then { tc_type_node(node_b[item]); let gk: int = tc_kind; let gn: int = tc_name; let ge: int = tc_elem_kind; let gen: int = tc_elem_name; tc_add_var(node_a[item], gk, gn, ge, gen, node_b[item]); if node_kind[item] == N_CONST then sym_type[node_a[item]] = gk + 100; tc_expr(node_c[item]);     if tc_same_full(gk, gn, ge, gen, tc_kind, tc_name, tc_elem_kind, tc_elem_name) == 0 then { let polymorphic_make: int = 0; if node_kind[node_c[item]] == N_CALL && sym_len[node_value[node_c[item]]] == 10 && sym_hash[node_value[node_c[item]]] == 790299 then polymorphic_make = 1; if polymorphic_make == 0 && (node_kind[node_c[item]] != N_INT || node_value[node_c[item]] != 0) && node_kind[node_c[item]] != N_NULL then tc_fail(29); } } item = node_next[item]; }
...
func emit_c_token(out: int*, kind: int, value: int): void {
  if kind == C_KW then {
    ...
    else if value == 1003 then write_string(out, "AshArray");
    else if value == 1004 then write_string(out, "runtime_array_make");
    else if value == 1005 then write_string(out, "runtime_array_push");
    else if value == 1006 then write_string(out, "runtime_array_get");
    else if value == 1007 then write_string(out, "runtime_array_set");
    else if value == 1008 then write_string(out, "runtime_array_reserve");
    else if value == 1009 then write_string(out, "runtime_array_clear");
    else if value == 1010 then write_string(out, "runtime_array_free");
    ...
    else if value == 1014 then write_string(out, "runtime_array_get_raw");
    ...
    else if value == 23 then { write_char(out, 40); write_char(out, 34); write_string(out, "%td"); write_char(out, 92); write_char(out, 110); write_char(out, 34); write_string(out, ", "); }
