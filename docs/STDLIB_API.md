...
### Module and FFI boundary

A standard-library module is included relative to the source file that names it. The Bootstrap loader canonicalizes that path before dependency-graph registration, deduplicates repeated canonical modules, and rejects active cycles with diagnostic 62. Missing targets use 63 and malformed directives use 64; callers should preserve `diagnostic.target` when presenting these failures. Namespace references are lexical and exact: a sibling module is not selected by a matching suffix, so public APIs should use their complete namespace when ambiguity is possible.

C interoperability uses the controlled `extern` surface rather than inferring declarations from raw `includec`. Only the platform C ABI and default compiler calling convention are supported. Scalar mappings, recursively safe pointers/fixed arrays, plain non-generic structs, and payload-free enums may cross the boundary. Dynamic arrays, generic/type-parameter nodes, tuples, closures, function types, tagged variants, and unsafe nested fields may not. `move` is not a valid extern parameter mode; `borrow` and `borrow_mut` are restricted to pointer-compatible or string inputs. Extern pointer and string returns are non-owning provider-owned values, and standard-library code must copy them before retaining them or pass them to `memory_free`; attempting to release them is diagnostic 67.
