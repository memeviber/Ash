# Báo cáo technical debt của Basalt
...
Nền tảng container hiện đã thống nhất theo hướng generic và dogfooding bằng Basalt. Commit trước đó đã hoàn thiện auto-growth cho `Array<T>`, `Slice<T>` và `HashMap<K,V>`, bổ sung regression cho nhiều primitive type, đồng thời sửa lỗi Bootstrap suy luận witness type của generic local/field để Host và Bootstrap sinh C tương thích [2].
...
Host typechecker nằm trong `src/compiler/lib/typechecker.ml`, trong khi Bootstrap có một bản port Basalt tương ứng. Hai implementation này vẫn phải đồng bộ thủ công. Sự khác biệt giữa `sym_type`, `tc_var_type`, `node_aux` và substituted generic type từng trực tiếp gây ra code C sai kiểu trong `map::reserve<K,V>` [2] [8].
...
Đợt cleanup đã xử lý đúng nhóm debt gây nhiễu và trùng lặp nhất: các API container chuyên biệt cho `int` và các compatibility implementation không còn cần thiết. Thiết kế hiện tại đã tiến gần hơn tới một standard library generic, thuần Basalt và có regression protection rõ ràng.
...
[1]: https://github.com/memeviber/Basalt/commit/46f601b "Remove obsolete container compatibility code"

[2]: https://github.com/memeviber/Basalt/commit/7ec8baa "Add generic auto-growing containers and fix Bootstrap witnesses"

[3]: https://github.com/memeviber/Basalt/commit/bab8c31 "Remove legacy array built-ins and add unary minus"

[4]: https://github.com/memeviber/Basalt/commit/c262a7d "Remove redundant memory stdlib wrapper"

[5]: https://github.com/memeviber/Basalt/commit/d03c592 "Rewrite array stdlib as pure Basalt with memory boundary"
...
| Điểm phát sinh prologue | `basaltc.ml` và `compiler.ml` | `compiler.ml` |
...
[13]: https://github.com/memeviber/Basalt/blob/main/src/compiler/lib/compiler.ml "Host C emitter and centralized runtime prologue"

[14]: https://github.com/memeviber/Basalt/blob/main/scripts/run_regression.sh "Regression checks for Host/Bootstrap C prologue parity"
