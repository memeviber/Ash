...
Generated C uses stable mangling: namespace separators and generic specialization separators become `__`, while type names use explicit, deterministic components. Mangling is an implementation detail and is not part of the Basalt source-level API. Because `__` is also legal in a source identifier, the compiler validates all top-level functions before emission: distinct source functions that would map to the same C symbol are rejected with a compile-time diagnostic. Names reserved by the Basalt runtime or the C ABI are rejected as function definitions as well. Namespace qualification therefore never silently overwrites a runtime helper or another function.
