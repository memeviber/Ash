...
`extern` declares a C-provided function with an Ash signature. `includec "file.h";` injects or includes raw C material under the compiler's controlled emission path. `include "module.ash";` loads another Ash source file. Recursive include processing tracks canonical paths, rejects active include cycles, and avoids duplicate loaded modules.
...
Ash emits portable C11. Release validation uses:
