...
The old `result::Option<T>` surface has been removed. New modules and examples MUST include `option.basalt` and use `option::Option<T>`; references to `result::Option<T>` or its legacy helpers are expected to fail at compile time. For fallible standard-library APIs with `E=int`, use `error::success` (`0`) for success and a nonzero module-documented category for failure. Use `Option` for expected absence, `Result` for operational failure, and documented fallback values only for total lookup/accessor APIs.
