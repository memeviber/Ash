...
> **Implementation note:** Ash emits C11 and validates generated programs with strict GCC diagnostics. Floating-point arithmetic follows the current compiler rules; `%` is intended for integer operands in portable generated C.
...
The script invokes Dune in `src/compiler/` and produces the Host executable at `src/compiler/_build/default/bin/main.exe`. The compiler accepts an Ash source path. It writes the generated C file beside the source file, using the source filename with `.c` appended.

## Building and using the Bootstrap compiler

The canonical Bootstrap source is `src/bootstrap/ashc.ash`. The standard sequence is:
...
The modulo-specific checks compile and execute the same Ash program through both compilers. They cover ordinary residues, zero and one, self-modulo, loop accumulation, precedence, nested expressions, a negative-value simulation, a generic `Result` context, and rejection of string operands. The complete stress corpus currently reports 164/164 passing cases before the dedicated modulo cases are added to the release repository.
...
Ash is distributed under the MIT License. See [`LICENSE`](LICENSE).
